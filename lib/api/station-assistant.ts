...
import type { 
  ...
  TableData,
  ActionSuggestion
} from '../types/station-assistant'

// ========== Mock 数据 ==========

const mockConfig: StationAssistantConfig = {
  name: '站长助理',
  avatar: '/placeholder.svg?height=80&width=80',
  welcomeMessage: '您好，我是您的专属站长运营助理。我可以帮您分析运营数据、提供推广策略建议、总结团队业绩等。有什么我可以帮您的吗？',
  suggestions: [
 ...
 * 获取助理配置
 */
export async function getStationAssistantConfig(): Promise<ApiResponse<StationAssistantConfig>> {
  if (useMock()) {
    await new Promise(resolve => setTimeout(resolve, 300))
    return { code: 200, data: mockConfig, message: 'success' }
  }
  return apiGet<StationAssistantConfig>('/station/assistant/config')
}

/**
 * 获取会话历史
 */
export async function getAssistantSession(): Promise<ApiResponse<AssistantSession>> {
  if (useMock()) {
  ...
  return apiGet<AssistantSession>('/station/assistant/session')
}
...
export async function sendAssistantMessage(
  ...
  if (content.includes('数据') || content.includes('总结')) {
  ...
  } else if (content.includes('活跃') || content.includes('提升')) {
    response = {
      text: '## 提升用户活跃度的策略建议\n\n' +
  ...
  } else if (content.includes('推广') || content.includes('策略')) {
    response = {
      text: '## 推广策略建议\n\n' +
  ...
  } else if (content.includes('团队') || content.includes('排行')) {
    response = {
      text: '## 团队业绩分析\n\n' +
        '截至今日，您的团队整体表现如下：\n\n' +
        '### 团队概况\n' +
  ...
  } else if (content.includes('待处理') || content.includes('事项')) {
    response = {
      text: '## 待处理事项提醒\n\n' +
        '您目前有以下事项需要处理：\n\n' +
        '### 紧急事项 🔴\n' +
    ...
    response = {
      text: '好的，我来为您分析一下。\n\n' +
        ...
        '- 团队业绩分析',
      actions: [
        { title: '查看数据概览', description: '了解分站整体运营情况', link: '/station/dashboard', priority: 'medium' },
      ]
    }
  }
  ...
  await new Promise<void>((resolve) => {
    const interval = setInterval(() => {
      if (currentIndex < fullText.length) {
        const chunk = fullText.slice(currentIndex, currentIndex + Math.floor(Math.random() * 5) + 2)
        ...
        // 发送额外数据
        if (response.chart) {
          setTimeout(() => callbacks.onChart?.(response.chart!), 100)
        }
        if (response.table) {
          setTimeout(() => callbacks.onTable?.(response.table!), 200)
        }
        if (response.actions) {
          setTimeout(() => callbacks.onActions?.(response.actions!), 300)
        }
 ...
 * 语音转文字
 */
export async function voiceToText(audioBlob: Blob): Promise<ApiResponse<{ text: string }>> {
  if (useMock()) {
    await new Promise(resolve => setTimeout(resolve, 1000))
    return {
      code: 200,
      data: { text: '请帮我分析一下本周的数据' },
      message: 'success',
    }
  }
  const formData = new FormData()
  formData.append('audio', audioBlob)
  return apiPost<{ text: string }>('/station/assistant/voice-to-text', formData)
}

/**
 * 清除会话
 */
export async function clearAssistantSession(): Promise<ApiResponse<{ success: boolean }>> {
  if (useMock()) {
    return { code: 200, data: { success: true }, message: '会话已清除' }
  }
  return apiPost<{ success: boolean }>('/station/assistant/session/clear')
}
