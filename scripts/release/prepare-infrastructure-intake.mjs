...
const upgradeExisting = hasFlag("--upgrade-existing");
...
if (existsSync(outputPath) && !upgradeExisting) {
...
function mergeMissingFields(defaults, existing) {
  ...
  for (const [key, defaultValue] of Object.entries(defaults)) {
    ...
    if (isPlainObject(defaultValue) && isPlainObject(existing[key])) {
      merged[key] = mergeMissingFields(defaultValue, existing[key]);
    }
  }
  return merged;
}
...
if (upgradeExisting) {
  ...
  if (existing.schemaVersion !== intake.schemaVersion || existing.kind !== intake.kind) {
    console.error("错误：现有清单契约不受支持，请人工迁移到最新模板后再升级");
    process.exit(1);
  }
  if (existing.deployTarget !== deployTarget) {
    console.error(
      `错误：现有清单部署架构为 ${existing.deployTarget}，与请求的 ${deployTarget} 不一致`,
    );
    process.exit(1);
  }
  ...
  output = mergeMissingFields(intake, existing);
}
...
console.log(`${upgradeExisting ? "已升级" : "已创建"}新基础设施私有接入清单：${outputPath}`);
if (backupPath) {
  console.log(`升级前备份：${backupPath}`);
}
