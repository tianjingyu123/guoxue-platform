...
test('App 收银台先检查 SDK，再请求服务端参数，不把 SDK success 当到账', () => {
  ...
  assert.match(app, /shopApi\.getWechatMiniPaymentLink\(orderId\.value\)/)
  assert.match(app, /launchWechatMiniPayment\(\{ orderId: orderId\.value, amount: amount\.value, scheme \}\)/)
  assert.match(app, /status\.value = 'confirming'/)
