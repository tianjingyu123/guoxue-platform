...
function load(){
 ...
 const source=fs.readFileSync('apps/mobile/src/lib/legacy-paipan-data.ts','utf8').replace(/^import .*;\r?\n/m,'').replace(/export /g,'');
 ...
 vm.runInNewContext(ts.transpileModule(source+';globalThis.api={stageLegacyPaipanEntry,consumeLegacyPaipanEntry,readLegacyPaipanContext,legacyPaipanContextPath,requestLegacyPaipanEntry}',{compilerOptions:{target:ts.ScriptTarget.ES2022}}).outputText,c);
