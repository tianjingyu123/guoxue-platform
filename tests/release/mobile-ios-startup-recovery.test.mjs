...
test("iOS 启动恢复编译进 APP-PLUS，运行时仅限 iOS，并且成功后按构建号去重", async () => {
  ...
  assert.match(source, /needsIosStartupRecovery/);
...
test("启动更新检查不依赖 URLSearchParams，且 218 版本号已冻结", async () => {
  ...
  assert.equal(JSON.parse(manifestRaw).versionCode, "218");
});
