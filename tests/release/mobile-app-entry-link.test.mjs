...
test('App Link 拒绝非 HTTPS、非受信主机、非默认端口和 URL 凭据', () => {
  ...
  assert.equal(
    parseAppEntryLink('HTTPS://API.REBUGX.CN:443/h5/pkg-live/manage/index'),
...
test('App Link 拒绝越界路径、编码分隔符、敏感凭据和异常输入', () => {
  ...
  assert.equal(parseAppEntryLink('https://api.rebugx.cn/h5/pkg-live/%2e%2e/manage/index'), null)
  assert.equal(parseAppEntryLink('https://api.rebugx.cn/h5/pkg-live%3Ftoken=hidden'), null)
  assert.equal(parseAppEntryLink('https://api.rebugx.cn/h5/paipan/luopan?access_token=secret'), null)
  assert.equal(parseAppEntryLink('https://api.rebugx.cn/h5/paipan/luopan?access%5Ftoken=secret'), null)
