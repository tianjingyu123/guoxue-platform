...
test('App 短视频使用 WebView 同层播放器，不再由原生 video 覆盖互动区', () => {
  ...
  assert.match(player, /video\.preload = 'auto'/u)
  assert.match(player, /video\.style\.opacity = '0'/u)
  assert.match(player, /video\.addEventListener\('loadeddata', \(\) => this\.revealDecodedFrame\(\)\)/u)
  assert.match(player, /video\.addEventListener\('playing', \(\) => this\.revealDecodedFrame\(\)\)/u)
  assert.match(player, /Number\(video\.readyState\) < 2/u)
  assert.match(player, /this\.video\.style\.opacity = '1'/u)
  assert.match(player, /this\.emitToOwner\('firstframe'/u)
  assert.match(player, /hostId: this\.playerId/u)
  assert.match(player, /document\.getElementById\(this\.hostId\)/u)
  assert.match(player, /video\.requestVideoFrameCallback\(showFrame\)/u)
  assert.match(player, /if \(!host\.isConnected\) this\.disposeVideo\(\)/u)
