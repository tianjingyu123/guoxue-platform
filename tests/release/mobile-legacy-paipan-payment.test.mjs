...
test('双桥支付不再跳微信内网页，iOS 的 webkit 支付消息也走同一交易号通道', () => {
  ...
  for (const script of [preload, late]) {
    ...
    window.webkit.messageHandlers.payWX.postMessage({ tradeNo: 'old-order-object-1', amount: 999, mchId: 'ignored' })
    assert.equal(assigned.at(-1), 'rebu://legacy-payment?trade_no=old-order-object-1')
    window.webviewJS.payWX({ order_no: 'old-order-object-2', url: 'https://attacker.example' })
    assert.equal(assigned.at(-1), 'rebu://legacy-payment?trade_no=old-order-object-2')
...
test('支付回调早于 App 恢复时延后刷新，回到原旧页面只处理一次', async () => {
  ...
  assert.match(actions.find(([name]) => name === 'toast')[1], /正在确认支付结果/u)
