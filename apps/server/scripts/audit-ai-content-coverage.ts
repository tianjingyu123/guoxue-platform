...
export async function collectCoverage(prisma: PrismaClient, now = new Date()): Promise<Row[]> {
  ...
  return [
    ...
    { type: "CONTENT", total: contentTotal, publicEligible: null, missingTitle: contentTitle, missingSummary: contentSummary, missingCover: contentCover, missingTags: contentTags,
      note: `状态候选 ${contentPublic} 条，但当前无安全且可按 Content.id 打开的公开详情页；不计入可推荐数` },
