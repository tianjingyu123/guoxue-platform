...
const CATEGORY_TILES = [
  ...
  },
  {
    ...
    name: "子部",
    desc: "诸子百家",
    intro: "百家争鸣，纵横捭阖，思想智慧的浩瀚星河。",
    icon: "lightbulb",
    from: "#3f8560",
    to: "#27543b",
    cover: "green",
    subCats: ["全部", "儒家", "道家", "法家", "兵家", "杂家", "小说"],
  },
  {
    id: "ji",
    cats: ["集"],
...
const HOT_SEARCH = [
  ...
  { keyword: "史记", isHot: false },
  { keyword: "黄帝内经", isHot: true },
  { keyword: "孙子兵法", isHot: false },
  { keyword: "庄子", isHot: false },
  { keyword: "资治通鉴", isHot: false },
];
...
type ClassicCopyrightRow = NonNullable<BookRow["copyrights"]>[number];
...
export class ClassicsBffService {
  ...
  /**
   * 公开端只返回履行许可和帮助用户理解所必需的信息。
   * 公版独立数字化的详细来源、哈希和处理记录保留在后台版权表/auditNote，
   * 不把内部审计材料暴露给普通读者；开放许可文本仍保留来源与许可链接。
   */
  private toPublicCopyright(copyright: ClassicCopyrightRow | undefined) {
    if (!copyright) return null;
    if (copyright.license === "AUTHORIZED") {
      return {
        sourceName: "经授权古籍内容",
        sourceUrl: null,
        license: "经权利方授权使用",
    ...
    if (copyright.license === "PUBLIC-DOMAIN") {
      return {
        sourceName: "公版古籍原文",
        sourceUrl: null,
        license: "公版内容",
  ...
  private toRankBook(b: BookRow, rank: number) {
    return {
      ...
      author: b.author || "佚名",
      dynasty: b.dynasty || "",
      views: fmtReads(b.viewCount),
      desc: this.cleanIntro(b.intro, b.title),
      reads: b.viewCount,
      rating: 0,
      category: b.category, // rating 保留字段兼容类型，前端已不展示古籍星级
    };
  }
  ...
  private toSearchResult(b: BookRow, i: number) {
    return {
      ...
      author: b.author || "佚名",
      dynasty: b.dynasty || "—",
      description: this.cleanIntro(b.intro, b.title),
      reads: b.viewCount,
      rating: 4.7,
      isFree: true,
      color: pickColor(i),
  ...
  async getHome() {
    const [grouped, chapterTotal, topRes, listsFull] = await Promise.all([
      ...
      this.prisma.classicChapter.count({
        where: { deletedAt: null, book: PUBLIC_CLASSIC_BOOK_WHERE },
      }),
  ...
  async getDetail(id: string) {
    ...
    const related = await this.classic.listBooks({
    ...
    const info = {
      ...
      reads: book.viewCount,
      rating: 0, // 古籍不做星级评分
      totalChapters: book.chapterCount,
      hasAI: true,
      hasAudio: true,
      hasTranslation: true, // AI 阅读(文白对照/查词) + AI 语音听书
      isFree: true,
      isInBookshelf: false,
      copyright: this.toPublicCopyright(book.copyrights?.[0]),
      color: pickColor(0),
    ...
    return Promise.all(
      lists.map(async (l) => {
        ...
        return {
          id: l.id,
          title: l.title,
          author: l.curator || "国学编辑部",
          bookCount: orderedCovers.length,
          likes: l.likeCount,
          desc: l.description || "",
  ...
  async getCollectionDetail(id: string) {
    ...
    const ordered = ids.map((bid) => books.find((b) => b.id === bid)).filter(Boolean) as {
      id: string;
      title: string;
      author: string | null;
      dynasty: string | null;
