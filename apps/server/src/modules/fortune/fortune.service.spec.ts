...
describe("FortuneService", () => {
  ...
  describe("分页入参加固（P2-4）", () => {
    it("adminListRecords: 非法 page(NaN) 归一化第1页·skip 不为 NaN", async () => {
      prisma.fortuneRecord.findMany.mockResolvedValue([]);
      prisma.fortuneRecord.count.mockResolvedValue(0);
      await svc.adminListRecords("abc" as any, 20);
      const arg = prisma.fortuneRecord.findMany.mock.calls.at(-1)![0];
