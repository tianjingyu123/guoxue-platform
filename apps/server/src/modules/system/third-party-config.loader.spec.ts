...
describe("ThirdPartyConfigLoader", () => {
  ...
  beforeEach(() => {
    ...
    for (const key of [
      ...
      "WECHAT_PAY_SERIAL_NO", "WECHAT_PAY_PRIVATE_KEY", "WECHAT_PAY_PUBLIC_KEY", "WECHAT_PAY_PUBLIC_KEY_ID",
  ...
  it("微信支付公钥模式缺少公钥 ID 时保持未就绪", async () => {
    const stored = encrypt(JSON.stringify({
      appId: "wx-current",
      mchId: "1748964663",
      allowedMchId: "1748964663",
    ...
    expect(wechat.configurationStatus).toBe("INCOMPLETE");
    expect(wechat.missingFields).toContain("微信支付公钥 ID");
