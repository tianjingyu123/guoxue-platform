...
describe("ToolCalculationService", () => {
  ...
  beforeEach(async () => {
    const mod: TestingModule = await Test.createTestingModule({
      providers: [ToolCalculationService],
    }).compile();
  ...
  describe("calculate", () => {
    it("已知 toolId 返回计算结果", () => {
      const result = svc.calculate({ toolId: "bazi", input: { datetime: "2025-01-01T12:00:00" } });
    ...
    it("未知 toolId 抛出异常", () => {
      expect(() => svc.calculate({ toolId: "unknown-tool", input: {} })).toThrow("暂未实现");
    });
