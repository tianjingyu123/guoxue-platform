 ...
 */
import { computeQimen, PALACE_NAMES, type QimenOptions, type QimenPalace } from '@guoxue/shared/paipan'
import { getJieqiRange } from '@guoxue/shared/paipan'
import { trueSolarTime } from '@guoxue/shared/paipan'
import type { QimenGong, QimenInput, QimenResult, QimenMeta } from './qimen-types'
...
export function computeQimenLocal(input: QimenInput): QimenResult {
  ...
  const options: QimenOptions = {
    ...
    // ★48（2026-09-21）：飞宫方式此前从未传给引擎，飞盘永远按「阴阳皆顺」排——输入页的「阳顺阴逆」是假开关。
    // 且两边取值不同：页面 'yangshun' | 'yinyang'，引擎 'shunni' | 'yinyang'。转盘不受此项影响。
    flyMethod: input.flyMethod === 'yangshun' ? 'shunni' : 'yinyang',
  ...
  const meta: QimenMeta = {
    ...
    // 引擎的 kongwang 每项为 { zhi: '辰巳', label: '年' }：zhi 是空亡地支、label 是柱名。
    // 此处要的是地支——原先取了 label，结果页遂显示成「年 年 / 月 月 / 日 日 / 时 时」，
    // 四柱旬空全部看不到（result.vue 模板是 `{{ k.label }} {{ k.zhi }}`，label 由它自己补）。
    kongWang: {
      nian: r.kongwang[0]?.zhi ?? '',
      yue: r.kongwang[1]?.zhi ?? '',
      ri: r.kongwang[2]?.zhi ?? '',
      shi: r.kongwang[3]?.zhi ?? '',
    },
    maXingZhi: r.maXing,
    jieQi: {
      name: jq.prev.name,
