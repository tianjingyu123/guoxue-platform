...
function wuyinOf(py: string): { yin: string; desc: string } {
  ...
  /**
   * 🔴 2026-09-20：原先这里直接 `return { yin: '宫', desc: '喉音，五行属土' }`。
   *
   * 表里**没有 j / q / x 的规则**，于是这三个声母全部落进这条兜底，
   * 被判成「宫·喉音·土」。实测拼音表 20856 字，**3954 字（19.0%）中招**
   * （j=1610、x=1325、q=1019）；抽 26 个常用起名字
   * （江健杰家君金晶嘉佳静秋清强青琴乾小新雪贤心欣宣轩旭学）**全部中招**。
   *
   * 而「宫·喉音」对应的是 y/w 与零声母，j/q/x **在任何一派下都不是喉音**——
   * 换句话说这不是流派之争，是兜底把「没规则」变成了「看着有答案」。
   *
   * 那 j/q/x 到底属哪一音？**从今音判不出来**：
   * 它们由中古**见系**（牙音·角·木，如 g/k/h 的细音）与**精系**
   * （齿音·商·金，如 z/c/s 的细音）两路合并而来，现代拼音已不分。
   * 通行姓名学表多归牙音（角·木），也有归齿音（商·金）的。
   * 旧版「字典查询」页**不提供五音字段**，拿不到外部基准。
   *
   * 所以这里**不替决策人选派**，如实标明未定；五音只用于展示（`zidian/index.vue`
   * 只读 `wuyin.yin`，不进任何评分），改成这样不影响其它计算。
   */
  if (/^[jqx]/.test(clean)) {
    return {
      yin: '待定',
      desc: 'j/q/x 声母：中古见系（牙音·角·木）与精系（齿音·商·金）两源在今音已合并，无法由拼音判定',
    }
  }
...
/**
 * 选字广场：从起名字库（手写精选 159 字 + 规范字标注为「宜」的约 900 字，含字义/诗词出处）按五行/笔画/吉凶/性别/结构/部首筛选。
