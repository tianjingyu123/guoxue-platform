...
import { GANS, ZHIS, fourPillars, kongWang } from '@guoxue/shared/paipan'
// 🔴 掐指推算的算法真源已迁至 packages/shared/src/paipan/xiaoliuren-engine.ts（2026-09-18）。
// 迁的原因与 ganzhi.ts 顶部记的是同一件事：算法散在两处，早晚漂移成两个盘。
// 后端要出小六壬报告，用的就是 shared 那份；这里只作转发与形态适配，**不要在此另写一份**。
import { computeXiaoliuren, type XlrSchool } from '@guoxue/shared/paipan'
...
export function paiPan(opts: {
  ...
  // 推算全部交给 shared，这里只把前端的 SizhuLite（带索引）转成引擎要的 {gan,zhi}。
  // 出参字段与引擎一致，页面无需改动。
  const { sizhu } = opts
  const p = (x: PillarLite) => ({ gan: x.g, zhi: x.z })
  const r = computeXiaoliuren({
    school: (opts.school as XlrSchool) || 'daojia',
    lunarMonth: opts.lunarMonth,
    lunarDay: opts.lunarDay,
    hourNum: opts.hourNum,
    numbers: opts.numbers,
    sizhu: { year: p(sizhu.year), month: p(sizhu.month), day: p(sizhu.day), hour: p(sizhu.hour) },
  })
  return {
    palaces: r.palaces.map((x) => ({
      name: x.name, liushen: x.liushen, star: x.star, starKong: x.starKong,
      gan: x.gan, zhi: x.zhi, qin: x.qin, markers: x.markers,
    })),
    monthPalace: r.monthPalace,
    dayPalace: r.dayPalace,
    hourPalace: r.hourPalace,
  }
}
