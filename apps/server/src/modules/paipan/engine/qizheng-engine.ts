// @ts-nocheck —— 内置 astronomy 以 .ts 收录后类型只能从 JS 推断（原配的 index.d.ts 无法挂上），
// 这里的 Body/AstroTime 等会被误报。本文件与前端原件逐字相同（前端一直带完整类型检查），行为由金样对照守着。
/**
...
import { AstroTime, Body, GeoVector, Ecliptic, Observer, SearchRiseSet } from "./vendor/astronomy"
