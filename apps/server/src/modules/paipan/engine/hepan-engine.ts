...
/**
 * 地支三刑（不含自刑，自刑见 ZI_XING）。
 *
 * 🔴 2026-09-20 修：原来是一张单向表
 * `new Set(["寅巳","巳申","申寅","丑戌","戌未","未丑","子卯","卯子"])`，
 * 调用处写的是 `ZHI_XING.has(yzA + yzB)` —— **只查一个方向**。
 * 十四个有序组合里漏了六个：巳寅、申巳、寅申、戌丑、未戌、丑未。
 *
 * 相刑是**对称**关系，而这是合盘：把两个人的录入顺序对调，
 * 同一对八字就会从「相刑，相处须存敬让」变成无事——分数、断语、风险提示全不一样。
 * 而且它在 `else if` 链里，漏判直接落到「无特殊关系」，是**少扣分、少提醒**那一侧。
 *
 * 同文件里 `ganHe`/`zhiLiuHe` 都写了 `a+b ?? b+a`，`GAN_CHONG`/`ZHI_HAI` 都列了双向，
 * 唯独这里没有——**同一文件内的风格不一致本身就是气味**。
 *
 * 改为按三刑组现判，不再维护有序字符串表（表总有人会漏一半）。
 */
...
function zhiXing(a: string, b: string): boolean {
  if (a === b) return false   // 同支属自刑，另由 ZI_XING 判
...
export function computeHepan(scene: string, inputA: HepanPersonInput, inputB: HepanPersonInput): HepanResult {
  ...
  } else if (zhiXing(yzA, yzB) || (yzA === yzB && ZI_XING.has(yzA))) {
    sxScore -= 10
