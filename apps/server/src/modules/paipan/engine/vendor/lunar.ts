...
/* eslint-disable */
/**
 * lunar-javascript 内置副本 —— 与前端 apps/mobile/src/pkg-paipan/lib/lunar/index.js 逐字节同源（仅加本头注释）。
 * 为什么不用 npm 的 lunar-javascript：前端引擎一直跑的是这份内置副本（ESM 形态），
 * 迁到后端必须是同一份代码，对等性才由构造保证。npm 版是 UMD 打包、来源版本未核，不能混用。
 */
