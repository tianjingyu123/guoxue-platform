...
import { BAGUA_LINES, BAGUA_WX, GUACI, HEX_NAMES } from "@guoxue/shared/paipan"
...
export function paiXiaoChengTu(opts: XctOptions): XctResult {
  ...
  switch (opts.method) {
    ...
    default: {
      // ★47：自动起卦的随机结果由输入页定下、随 payload 传入，保证从排盘记录重开是同一卦；
      // 未传时才现场随机（兼容此前未带 u/l/dong 的旧记录）
      upperN = opts.upper ?? rnd(8) + 1
      lowerN = opts.lower ?? rnd(8) + 1
      dong = opts.dong ?? rnd(6) + 1
