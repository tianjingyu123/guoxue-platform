/**
 * 诸葛神数引擎（2026-09-21 自前端 pkg-paipan2/lib/zhuge-engine.ts 迁入，算法与签文库只在服务端）
 *
 * 与前端原版的唯一差别：康熙笔画改取 shared 的 `strokeOfOrNull`——
 * shared 的 kangxi-strokes.json 与前端那份逐字节相同，查法 `TABLE[ch]?.[0]` 也相同；
 * 由 test/paipan-engine-golden.spec.ts 对表内全部 20,992 字逐字验证。
 */
...
const SIGNS = signsData as unknown as Record<string, ZhugeSign>
