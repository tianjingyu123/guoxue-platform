...
export function paiFeigong(input: {
  ...
  {
    // 🔴 2026-09-20 修：原实现只在「合干排在中宫之后」时才正确。
    //    原写法 `pulled.add(mate)` 只能阻止**后续**布干；当合干已经布在前面的宫里时，
    //    它既不被跳过、也不从原宫移走 —— 于是合干同时出现在两处，
    //    而十干循环多走一步回绕到青龙宫，第 10 个干被 `ganAt[p][0]` 静默丢弃。
    //    实测：青龙宫落 6/7/8/9 宫（时支 戌亥酉丑寅午，**半数时辰**）时，
    //    必有一干重复、且**癸恒定消失**。
    //    改法依文件头自己的口诀「五合干**抽出**同居中宫」——抽出即不给它单独的宫，
    //    无论它排在中宫之前还是之后。故先定出落中宫之干，再飞。
    const pulled = new Set<string>()
    // 中宫在飞行序中的位次（0-based，自青龙宫起沿 p→p+1 计，含中宫）
    const centerStep = (((5 - qinglongPalace) % 9) + 9) % 9
    // 五合恒为「相隔五位」：序号 0–4 之干的合干在后（不影响前段），5–9 之干的合干在前（会被抽出，后段整体顺延一位）
    const ganIdx = (g: string) => (GANS as readonly string[]).indexOf(g)
    const centerGan =
      ganIdx(WUHE[GANS[centerStep]]) > centerStep ? GANS[centerStep] : GANS[centerStep + 1]
    pulled.add(WUHE[centerGan])

    let cursor = qinglongPalace
    for (const g of GANS) {
      if (pulled.has(g)) continue
      ganAt[cursor].push(g)
      if (cursor === 5) ganAt[5].push(WUHE[g])
