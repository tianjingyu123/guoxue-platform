...
describe("InstituteAssessmentService（T9-P1 考核制度 V5）", () => {
  ...
  function primeLecturePass() {
    ...
    const oldPosts = Array.from({ length: 3 }, (_, i) => ({
      id: `op${i}`,
  ...
  describe("getMyAssessment — 四档判定（V5）", () => {
    ...
    it("全返 FULL_REFUND：积分≥100 且线下三选一达标（旗舰驿站≥1场）", async () => {
      prisma.instituteSharePoint.findMany.mockResolvedValue([
        ...
        {
          id: "off1",
          pointType: "OFFLINE_STATION",
          points: 25,
          refId: null,
          remark: null,
          createdAt: new Date(year, 0, 20),
        },
      ]);
    ...
    it("pointItems 最多返回近 20 条流水", async () => {
      prisma.instituteSharePoint.findMany.mockResolvedValue(
        Array.from({ length: 30 }, (_, i) => ({
          ...
          refId: null,
          remark: null,
          createdAt: new Date(year, 0, 1 + (i % 28)),
        })),
      );
      const res = await svc.getMyAssessment("u1");
  ...
  describe("awardEventPointsForCompletedEvent — 活动自动记分（幂等）", () => {
    ...
    it("无讲师/非记分活动类型（COURSE）/讲师非在册成员 → 均不记分", async () => {
      expect(
        await svc.awardEventPointsForCompletedEvent({ id: "e1", type: "SALON", lecturerId: null }),
      ).toBeNull();
      expect(
        await svc.awardEventPointsForCompletedEvent({ id: "e1", type: "COURSE", lecturerId: "uL" }),
      ).toBeNull();
      prisma.instituteMember.findFirst.mockResolvedValue(null);
      expect(
        await svc.awardEventPointsForCompletedEvent({ id: "e1", type: "SALON", lecturerId: "uL" }),
      ).toBeNull();
      expect(prisma.instituteSharePoint.create).not.toHaveBeenCalled();
    });
  });
  ...
  describe("addManualPoints — 管理层人工记分", () => {
    it("非管理层 → FORBIDDEN", async () => {
      ...
      await expect(svc.addManualPoints("u-normal", "m2", { points: 10 })).rejects.toThrow(
        "仅研究院管理层可操作",
      );
      expect(prisma.instituteSharePoint.create).not.toHaveBeenCalled();
    });
    ...
    it("管理层记分成功：缺省 MANUAL 类型 + verifiedBy 记录操作者（可负分纠错）", async () => {
      ...
      const res: any = await svc.addManualPoints("u-mgr", "m2", {
        points: -10,
        remark: "重复记分纠错",
      });
    ...
    it("成员不存在 → NOT_FOUND；零分/无效类型 → BAD_REQUEST", async () => {
      ...
      await expect(svc.addManualPoints("u-mgr", "mx", { points: 10 })).rejects.toThrow(
        "成员不存在",
      );
      ...
      await expect(svc.addManualPoints("u-mgr", "m2", { points: 0 })).rejects.toThrow(
        BusinessException,
      );
      await expect(
        svc.addManualPoints("u-mgr", "m2", { points: 10, pointType: "HACK" }),
      ).rejects.toThrow(/无效积分类型/);
      expect(prisma.instituteSharePoint.create).not.toHaveBeenCalled();
    });
  });
  describe("addManualPoints — 跨院隔离", () => {
    it("管理层不能调整其他研究院成员积分", async () => {
      prisma.instituteMember.findFirst.mockResolvedValue({ id: "mgr1", instituteId: "i1" });
      prisma.instituteMember.findUnique.mockResolvedValue({
        id: "m2",
        instituteId: "i2",
      ...
      await expect(svc.addManualPoints("u-mgr", "m2", { points: 10 })).rejects.toThrow(/本研究院/);
      expect(prisma.instituteSharePoint.create).not.toHaveBeenCalled();
    });
  });
});
