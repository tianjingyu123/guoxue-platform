...
const CARD_LABELS: Record<AiGuideCardType, string> = {
  classic: '古籍', article: '文章', course: '课程', circle: '圈子', content: '内容', video: '视频', product: '商品',
}
...
function openCard(card: AiGuideCard) {
  if (!/^\/(pkg-classics\/detail\/index|pkg-circle\/(articles|circles)\/detail|pkg-course\/detail\/index|pkg-video\/detail\/index|pkg-mall\/product\/detail)\?id=[^&#]+$/.test(card.target)) return
