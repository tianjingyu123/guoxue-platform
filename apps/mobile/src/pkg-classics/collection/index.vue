...
const collection = ref<CollectionDetail>({
  id: '', title: '', description: '', cover: 'amber', curator: '',
  bookCount: 0, viewCount: 0, tags: [], books: [],
})
const relatedCollections = ref<CollectionDetail[]>([])

onLoad(async (options) => {
  if (options?.id) collectionId.value = options.id
  const res = await classicsApi.collection(collectionId.value)
  collection.value = res.detail
})
