...
const recommendBooks = ref<SearchResult[]>([])
...
async function loadInitialData() {
  ...
  error.value = ''
  try {
    const res = await classicsApi.search('')
    hotSearchData.value = res.hotSearches || []
    searchHistory.value = res.history || []
    if (res.results && res.results.length > 0) {
      recommendBooks.value = res.results.slice(0, 3) as SearchResult[]
    }
  } catch (e: any) {
...
async function onInput(e: any) {
  ...
  if (value.trim()) {
    try {
      const res = await classicsApi.search(value)
      suggestions.value = (res.suggestions || []).map((s: string) => ({ text: s }))
...
async function handleSearch(keyword?: string) {
  ...
  try {
    const res = await classicsApi.search(kw)
    results.value = (res.results || []) as SearchResult[]
    searchState.value = results.value.length > 0 ? 'results' : 'empty'
  } catch (e: any) {
