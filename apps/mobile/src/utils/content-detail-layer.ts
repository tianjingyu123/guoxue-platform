...
export function tryOpenContentDetailLayer(target: string, source?: unknown): boolean {
  ...
  // H5 没有系统返回手势时，从屏幕左缘右滑也能收起详情。
  // 仅占用 28px 边缘热区，避免和正文横向操作、轮播以及 iframe 内滚动争抢手势。
  const edgeGesture = document.createElement("div");
  edgeGesture.className = "gx-content-detail-layer__edge";
  edgeGesture.setAttribute("aria-hidden", "true");
  edgeGesture.title = "向右滑动收起";
  Object.assign(edgeGesture.style, {
    position: "absolute",
    zIndex: "4",
    left: "0",
    top: "0",
  ...
  const restoreEdgeGesture = () => {
    edgePointerId = null;
    edgeDragging = false;
    edgeOffset = 0;
    layer.style.transition = `transform ${Math.max(1, Math.round(duration * 0.72))}ms ${easing}`;
    layer.style.transform = "translateX(0px)";
    closeButton.style.opacity = frame.style.opacity === "1" ? "0.78" : "0";
    window.setTimeout(() => {
      if (!closing) layer.style.transition = layerTransition;
    }, duration);
  };
  ...
  const onEdgePointerDown = (event: PointerEvent) => {
    ...
    edgeStartAt = performance.now();
    edgeDragging = false;
    edgeOffset = 0;
    edgeGesture.setPointerCapture?.(event.pointerId);
  };

  const onEdgePointerMove = (event: PointerEvent) => {
    if (closing || edgePointerId !== event.pointerId) return;
    const deltaX = Math.max(0, event.clientX - edgeStartX);
    const deltaY = Math.abs(event.clientY - edgeStartY);
    if (!edgeDragging && deltaX >= 8 && deltaX > deltaY * 1.25) {
      edgeDragging = true;
    ...
    edgeOffset = Math.min(shellRect.width * 0.42, deltaX * 0.78);
    layer.style.transform = `translateX(${edgeOffset}px)`;
    closeButton.style.opacity = String(Math.min(0.98, 0.78 + edgeOffset / 360));
  };

  const onEdgePointerEnd = (event: PointerEvent) => {
    if (edgePointerId !== event.pointerId) return;
    const elapsed = Math.max(1, performance.now() - edgeStartAt);
    const velocity = edgeOffset / elapsed;
    const shouldClose =
      edgeDragging &&
      (edgeOffset >= Math.max(72, shellRect.width * 0.18) || (edgeOffset >= 42 && velocity > 0.55));

    edgeGesture.releasePointerCapture?.(event.pointerId);
    if (shouldClose) {
      edgePointerId = null;
      edgeDragging = false;
      layer.style.transition = layerTransition;
      layer.style.transform = "translateX(0px)";
      requestClose();
      return;
    }
  ...
  edgeGesture.addEventListener("pointerdown", onEdgePointerDown);
  edgeGesture.addEventListener("pointermove", onEdgePointerMove);
  edgeGesture.addEventListener("pointerup", onEdgePointerEnd);
  edgeGesture.addEventListener("pointercancel", onEdgePointerEnd);
  document.addEventListener("keydown", onKeyDown);
  window.addEventListener("message", onMessage);
  window.addEventListener("popstate", onPopState);
  window.history.pushState(
    { ...(window.history.state || {}), gxContentLayer: true },
    "",
  ...
  layer.append(cardClone, frame, loadingLine, edgeGesture, closeButton);
