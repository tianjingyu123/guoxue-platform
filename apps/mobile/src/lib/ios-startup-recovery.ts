...
let startupHomeReady = false;

// #ifdef APP-PLUS
export function shouldRecoverIosStartupRoute(routes: string[], homeReady = false): boolean {
  if (homeReady) return false;
...
/** 首页 Vue 页面已经真实挂载；取消尚未执行的冷启动恢复，避免正常首页被二次重建。 */
export function markIosStartupHomeReady(): void {
  startupHomeReady = true;
 ...
 * 不能证明页面可见。首页真实挂载后会主动上报就绪并取消看门狗；只有页面栈为空，
 * 或首页 route 存在但始终没有真实挂载时，才重建一次首页页面栈。
...
export function repairIosStartupRoute(): void {
  ...
  const recover = () => {
    ...
    try {
      ...
      if (!shouldRecoverIosStartupRoute(routes, startupHomeReady)) return;
    } catch {
