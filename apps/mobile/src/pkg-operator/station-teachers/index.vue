...
onMounted(async () => {
  try {
    const data = await operatorApi.teacherInvitations()
    invitations.value = data.invitations || []
    demands.value = data.demands || []
  } catch (e) {}
})

const confirmedList = computed(() => invitations.value.filter((i) => i.status === 'confirmed'))
const completedList = computed(() => invitations.value.filter((i) => i.status === 'completed'))
