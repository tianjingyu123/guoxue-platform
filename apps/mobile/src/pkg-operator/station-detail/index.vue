...
function backToTop() {
  scrollTarget.value = "";
  nextTick(() => {
    scrollTarget.value = "station-scroll-top";
  });
}

function goWorkbench() {
  navigateTo("/operator/station-master-panel");
}
function goNotifications() {
  navigateTo("/notifications");
}
function goFeaturedHome() {
  navigateTo("/pkg-operator/station-home/index");
}
...
<template>
  <view class="sd">
      ...
      <view class="sd-brandbar" :style="{ background: theme }">
        <view class="sd-brandbar-left">
          <view class="sd-brand-logo"
            ><text class="sd-brand-logo-txt">{{ station.name?.charAt(0) }}</text></view
          >
          <text class="sd-brand-name">{{ station.name }}</text>
          <text class="sd-brand-tag">{{ BRAND.name }}</text>
        </view>
        <view class="sd-brandbar-my" hover-class="tap-press" @tap="goWorkbench"
          ><text class="sd-brandbar-my-txt">经营工作台 ›</text></view
        >
      </view>
      <!-- 搜索 Header -->
    ...
    <scroll-view
      ...
      @scroll="onScroll"
    >
      <view id="station-scroll-top" class="scroll-anchor" />
      <!-- 三态：加载中 -->
      <view v-if="loading" class="state-loading"
        ><text class="state-loading-text">加载中...</text></view
      >
      <!-- 三态：错误 -->
        ...
        <view class="sd-hero">
          <view class="sd-hero-card">
            ...
            <view class="sd-hero-master">
              ...
              <view class="sd-hero-meta">
                ...
                <text
                  v-if="station.memberCount != null || station.contentCount != null"
                  class="sd-hero-stat"
                >
                  <text v-if="station.memberCount != null">{{ station.memberCount }} 成员</text>
                  <text v-if="station.memberCount != null && station.contentCount != null">
                    ·
                  </text>
          ...
          <view class="sd-welcome" :style="{ background: theme + '1a' }">
            <view class="sd-welcome-left">
              ...
              <text class="sd-welcome-txt" :style="{ color: theme }">{{
                station.masterIntro || `欢迎来到${station.masterName}的国学小站`
              }}</text>
            </view>
          </view>
        </view>
                ...
                <view class="sd-fcard-info">
                  <view class="sd-fcard-tags">
                    <view class="sd-fcard-type" :style="{ background: theme + '26' }">
                      <text class="sd-fcard-type-txt" :style="{ color: theme }">{{
                        featuredTypeConfig[item.type].label
                      }}</text>
                    </view>
                    <text v-if="item.price !== undefined && item.price > 0" class="sd-fcard-price"
                      >¥{{ formatPrice(item.price) }}</text
                    >
                    <text v-if="item.originalPrice" class="sd-fcard-original"
      ...
      <view v-if="feedLoading" class="sd-feed skeleton">
        <view
          class="sk-block"
          style="height: 400rpx; width: 50%; flex-shrink: 0; border-radius: 16rpx"
        />
        <view
          class="sk-block"
          style="
      ...
      <view class="sd-end">
        <view class="sd-end-line" /><text class="sd-end-text">已经到底了</text
        ><view class="sd-end-line" />
      </view>
    </scroll-view>
...
.sd-header {
  ...
  padding: 0 32rpx;
  background: var(--bg-paper, #faf8f5);
  border-bottom: 1rpx solid var(--line, #e8e0d5);
}
.sd-search {
  flex: 1;
  height: 64rpx;
...
.sd-scroll {
  position: absolute;
  top: calc(176rpx + var(--status-bar-height, 0px));
  bottom: 112rpx;
  left: 0;
  right: 0;
}
