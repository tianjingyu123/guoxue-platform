...
const error = ref('')
const overview = ref<TeamMgmtOverview>(null!)
...
async function loadData() {
  ...
  try {
    const res = await operatorApi.team()
    overview.value = res.overview
    teamMgmtMembers.value = res.members
    leaderboard.value = res.leaderboard
    myRank.value = res.myRank
    activities.value = res.activities
    successCases.value = res.successCases
    activityIconMap.value = res.activityIconMap
...
async function openMemberDetail(m: TeamMgmtMember) {
  ...
  try {
    const res = await operatorApi.teamMemberDetail(String(m.id))
    recentOrders.value = res.orders
    invitedMembers.value = res.invitedMembers
  } catch { /* 详情弹窗仍然可以展示基本信息 */ }
}
function copyLink() {
  uni.setClipboardData({ data: inviteLink.value, success: () => uni.showToast({ title: '链接已复制', icon: 'none' }) })
}
</script>
