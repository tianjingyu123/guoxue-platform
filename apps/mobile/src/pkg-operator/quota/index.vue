  ...
  <view v-else class="quota-page">
    ...
    <view class="quota-sec quota-overview-wrap">
      <view class="quota-overview-card">
        ...
        <view class="quota-ov-grid">
          ...
          <view class="quota-ov-item"><text class="quota-ov-num">{{ data.used }}</text><text class="quota-ov-sub">自用</text></view>
          <view class="quota-ov-item"><text class="quota-ov-num c-success">{{ data.sold }}</text><text class="quota-ov-sub">已售</text></view>
          <view class="quota-ov-item"><text class="quota-ov-num c-gold">{{ data.gifted }}</text><text class="quota-ov-sub">已赠</text></view>
    ...
    <view class="quota-sec">
      <view class="quota-card">
        <text class="quota-card-title">分配名额</text>
        <view class="quota-actions">
          ...
          <view class="quota-action-btn" :class="data.available > 0 ? 'gold-btn' : 'disabled-btn'" @tap="onGiftClick">
            ...
            <text class="quota-action-label" :class="{ disabled: data.available === 0 }">免费赠送</text>
            <text class="quota-action-desc">{{ data.available > 0 ? `剩余${data.available}个可赠送` : '暂无可用名额' }}</text>
          </view>
        </view>
      </view>
    </view>
    ...
    <view class="quota-sec">
      <view class="quota-tabs">
        <view class="quota-tab" :class="{ active: activeTab === 'manage' }" @tap="activeTab = 'manage'"><text class="quota-tab-txt">名额记录</text></view>
        <view class="quota-tab" :class="{ active: activeTab === 'rules' }" @tap="activeTab = 'rules'"><text class="quota-tab-txt">使用规则</text></view>
      </view>
    </view>

    <!-- 名额记录 -->
    <view v-if="activeTab === 'manage'" class="quota-sec quota-list">
      <view v-for="r in records" :key="r.id" class="quota-card quota-record">
        <view class="quota-record-icon" :class="recordIconBg(r.type)">
          <app-icon :name="recordIcon(r.type)" :size="30" :color="recordIconColor(r.type)" />
        </view>
        <view class="quota-record-info">
          <view class="quota-record-name-row">
            <text class="quota-record-name">{{ r.name }}</text>
            <text class="quota-badge" :class="recordBadgeClass(r.type)">{{ recordTypeLabel(r.type) }}</text>
          </view>
      ...
      <view v-if="data.available > 0" class="quota-empty-card">
        <text class="quota-empty-txt">剩余 {{ data.available }} 个名额待分配</text>
      </view>
    </view>

    <!-- 使用规则 -->
    <view v-if="activeTab === 'rules'" class="quota-sec">
      <view class="quota-card">
        <view class="quota-rule">
          <view class="quota-rule-num bg-operator"><text class="quota-rule-num-txt c-operator">1</text></view>
          <view class="quota-rule-body"><text class="quota-rule-title">名额来源</text><text class="quota-rule-desc">成为运营商时获赠6个分站名额，其中1个自用，5个可分配给他人</text></view>
        </view>
        <view class="quota-rule">
        ...
        <view class="quota-rule">
          ...
          <view class="quota-rule-body"><text class="quota-rule-title">免费赠送</text><text class="quota-rule-desc">可选择免费赠送给指定用户，用于团队激励或合作伙伴</text></view>
        </view>
        <view class="quota-rule">
    ...
    <view v-if="showQrDialog" class="quota-mask" @tap.self="showQrDialog = false">
      <view class="quota-dialog">
        ...
        <view class="quota-dialog-footer">
          ...
          <view class="quota-dialog-btn operator" @tap="toastShare"><app-icon name="send" :size="26" color="#ffffff" /><text class="quota-dialog-btn-txt light">分享</text></view>
        </view>
      </view>
    </view>

    <!-- 免费赠送弹窗 -->
    <view v-if="showGiftDialog" class="quota-mask" @tap.self="closeGift">
      <view class="quota-dialog">
        <text class="quota-dialog-title">免费赠送名额</text>
        <text class="quota-dialog-desc">输入用户手机号，将站长名额免费赠送给TA</text>
        ...
        <view v-if="searchResult" class="quota-result">
          <view class="quota-result-row">
            <view class="quota-result-avatar"><app-icon name="user" :size="32" color="#C9A96E" /></view>
            <view class="quota-result-info"><text class="quota-result-name">{{ searchResult.name }}</text><text class="quota-result-phone">{{ searchResult.phone }}</text></view>
            <text class="quota-badge badge-success">已找到</text>
          </view>
          <view class="quota-form-field" style="margin-top: 20rpx;">
            <text class="quota-form-label sm">赠送备注（选填）</text>
            <input class="quota-input full" v-model="giftName" placeholder="例如：合作伙伴激励" placeholder-class="quota-input-ph" />
          </view>
        </view>
        <view class="quota-warn">
          <app-icon name="alert-circle" :size="28" color="#b45309" />
          <text class="quota-warn-txt">赠送后名额无法收回，对方将立即获得1年站长权益</text>
        </view>
        <view class="quota-dialog-footer">
          <view class="quota-dialog-btn outline" @tap="closeGift"><text class="quota-dialog-btn-txt">取消</text></view>
          <view class="quota-dialog-btn gold" :class="{ disabled: !searchResult }" @tap="doGift"><app-icon name="gift" :size="26" color="#ffffff" /><text class="quota-dialog-btn-txt light">确认赠送</text></view>
        </view>
      </view>
    </view>
  </view>
