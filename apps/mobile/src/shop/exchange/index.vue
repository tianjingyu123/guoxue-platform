...
import { shopApi, shopExchangeReasons, type ShopExchangeAddress } from '@/lib/shop-data'
...
const availableSkus = computed(() => selectedProduct.value?.skus.filter((s: any) => s.id !== selectedProduct.value?.skuId) || [])

async function loadExchangeData() {
  loading.value = true
  error.value = ''
  try {
    const [addrList, orderData] = await Promise.all([
      shopApi.getAddresses(),
      shopApi.getOrderDetail(orderId.value),
    ])
    addresses.value = Array.isArray(addrList) ? addrList : (addrList as any)?.items || []
    const def = addresses.value.find((a: ShopExchangeAddress) => a.isDefault)
    if (def) selectedAddress.value = def
    if (orderData?.items) {
      products.value = orderData.items.map((item: any) => ({
        id: item.productId || item.id,
        name: item.productName || item.name,
        cover: item.cover || item.image,
        skuId: item.skuId,
        skuName: item.skuName || item.spec,
        price: item.price,
...
async function handleSubmit() {
  ...
  try {
    await shopApi.submitAfterSale({
      orderId: orderId.value,
      productId: selectedProduct.value?.id,
      skuId: exchangeType.value === 'different' ? newSkuId.value : selectedProduct.value?.skuId,
      reason: reason.value,
    ...
    setTimeout(() => navigateTo('/shop/my-after-sales'), 800)
  } catch (e: any) {
