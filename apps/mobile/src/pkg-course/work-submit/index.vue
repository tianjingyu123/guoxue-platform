...
<template>
  <view class="page">
    <!-- 顶部导航 -->
    <app-nav-bar title="提交作业" background="#fff" color="#2C2C2C" />

    <view v-if="loading" class="body">
      <!-- 骨架屏：2个白色卡片 + 1个输入区占位 -->
      <view class="pl-skeleton">
        <view class="sk-card"><view class="sk-row"><view class="sk-icon" /><view class="sk-info"><view class="sk-title" /><view class="sk-desc" /></view></view></view>
        <view class="sk-card"><view class="sk-title" /><view class="sk-paragraph" /><view class="sk-paragraph sk-short" /></view>
        <view class="sk-card"><view class="sk-textarea" /></view>
      </view>
    </view>
...
.sk-icon { width: 80rpx; height: 80rpx; border-radius: 16rpx; background: #E8E3DB; flex-shrink: 0; }
.sk-info { flex: 1; display: flex; flex-direction: column; gap: 12rpx; }
.sk-desc { height: 24rpx; width: 60%; background: #E8E3DB; border-radius: 8rpx; }
.sk-paragraph { height: 28rpx; background: #E8E3DB; border-radius: 8rpx; margin-bottom: 16rpx; }
.sk-paragraph.sk-short { width: 70%; }
.sk-textarea { height: 384rpx; background: #F5F0E8; border-radius: 16rpx; }
</style>
