...
function decodeWechatPaymentState(value: unknown): WechatPaymentState | undefined {
  ...
  try {
    ...
    if (parsed.scene === 'recharge') return /^\d+$/.test(String(parsed.amountCoin || '')) ? parsed : undefined
...
/**
 * 微信网页授权偶发在 H5 history 路由完成初始化前返回，uni-app 会落到默认页。
 * 优先使用会话中的固定回跳地址；会话丢失时只从受限 state 恢复支付页。
 */
