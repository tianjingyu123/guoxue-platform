...
import { shopApi, type ProductDetail } from '@/lib/shop-data'

const product = ref<ProductDetail>({
  id: 0, title: '', subtitle: '', images: [], hasVideo: false,
  price: 0, originalPrice: 0, coupon: { value: 0, threshold: 0 },
  sales: 0, stock: 0, specs: [], rating: 0, reviewCount: 0,
  tags: [], reviews: [], description: '',
})
...
onLoad(async () => {
  try {
    const data: any = await shopApi.getProductDetail('1')
    if (data && data.id != null) product.value = data as ProductDetail
  } catch { /* keep default */ }
  try {
    const cart: any = await shopApi.getCart()
...
const currentPrice = computed(() => {
  if (!product.value.specs) return 0
  const v = product.value.specs.find((s) => s.name === '版本')?.options.find((o) => o.id === selectedSpecs.value['版本'])
...
const selectedSpecLabels = computed(() =>
  Object.entries(selectedSpecs.value)
    .map(([k, v]) => product.value.specs?.find((s) => s.name === k)?.options?.find((o) => o.id === v)?.label)
    .filter(Boolean)
    .join('、'),
)
