...
import { circleBotsApi, formatUsageCount, type CircleSummary, type SortBy, type CircleBotItem } from '@/lib/circle-bots-data'

const circleId = ref(1)
const circle = ref<CircleSummary>({ id: 0, name: '', icon: '', description: '', memberCount: 0, isAdmin: false, isOwner: false })
const keyword = ref('')
const inputVal = ref('')
const sortBy = ref<SortBy>('hot')
const bots = ref<CircleBotItem[]>([])
...
async function loadData() {
  const [summary, botList] = await Promise.all([
    circleBotsApi.summary(circleId.value),
    circleBotsApi.list(circleId.value, keyword.value, sortBy.value),
  ])
  if (summary) circle.value = summary as CircleSummary
  bots.value = botList as CircleBotItem[]
}

watch(sortBy, () => loadData())

function doSearch() { keyword.value = inputVal.value; loadData() }
