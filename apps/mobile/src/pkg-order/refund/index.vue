...
import { orderApi, type RefundDetail } from '@/lib/order-data'

const safeBottom = ref(0)
const data = ref<RefundDetail>({
  id: '', orderId: '', orderNo: '', type: 'refund_only',
  status: 'submitted', reason: '', amount: 0, description: '',
...
const refundId = ref('')
const refundMethod = ref('微信支付')
const estimatedDate = ref('2024年1月18日')
...
onLoad(async (opts: any) => {
  if (opts?.id) refundId.value = opts.id
  try {
  ...
  try {
    const res = await orderApi.getRefundDetail(refundId.value || '1')
    data.value = res
    if (res.refundMethod) refundMethod.value = res.refundMethod
    if (res.estimatedDate) estimatedDate.value = res.estimatedDate
  } catch (e) {
