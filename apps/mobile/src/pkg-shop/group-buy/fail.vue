    ...
    <template v-else-if="info">
      ...
      <view class="card">
        ...
        <view class="row">
          ...
          <view class="members">
            ...
            <text class="member-text">{{ info.currentMembers }}/{{ info.minMembers }}人</text>
          </view>
        </view>
        <view class="row">
      ...
      <view class="card">
        <view class="refund-head">
          <app-icon name="wallet" :size="36" color="#c41e3a" />
        ...
        <view class="steps">
          ...
          <view class="steps-dots">
            <view class="dot" :class="{ 'dot--on': info.refundStatus !== 'pending' }">
              <app-icon v-if="info.refundStatus !== 'pending'" name="check" :size="18" color="#fff" />
            </view>
            <view class="dot" :class="{ 'dot--on': info.refundStatus === 'processing' || info.refundStatus === 'completed' }">
              <app-icon v-if="info.refundStatus === 'completed'" name="check" :size="18" color="#fff" />
            </view>
            <view class="dot" :class="{ 'dot--on': info.refundStatus === 'completed' }">
              <app-icon v-if="info.refundStatus === 'completed'" name="check" :size="18" color="#fff" />
            </view>
          </view>
        </view>
      ...
      <view class="card card--row">
        ...
        <view class="order-id">
          <text class="order-mono">{{ info.orderId }}</text>
          <view class="copy-btn" @tap="copy(info.orderId)">
            <app-icon :name="copied ? 'check' : 'copy'" :size="28" :color="copied ? '#2e8b57' : '#999'" />
          </view>
        </view>
      </view>
      ...
      <!-- 提示 -->
      <view class="tips">
        <text class="tips-text">温馨提示：拼团失败不影响您再次参与，我们为您推荐了更多热门拼团商品，快去看看吧！</text>
      </view>
    </view>
