<template>
  <view class="ebs-page">
    ...
    <view class="ebs-header">
      <view class="ebs-bar">
        <view class="ebs-iconbtn" @tap="goBack">
          <app-icon name="arrow-left" :size="40" color="#1e293b" />
        </view>
        <text class="ebs-title">我的书架</text>
        <view class="ebs-iconbtn" @tap="goStore">
          <app-icon name="book-open" :size="40" color="#2563eb" />
        </view>
      </view>
      ...
      <view class="ebs-filters">
        <view
          ...
          @tap="filter = f.id"
        >
          <text class="ebs-filter-txt" :class="{ 'ebs-filter-txt-active': filter === f.id }">{{ f.label }}</text>
        </view>
      </view>
      ...
      <view class="ebs-toolbar">
        ...
        <view class="ebs-vt">
          ...
          <view class="ebs-vt-btn" :class="{ 'ebs-vt-on': viewMode === 'list' }" @tap="viewMode = 'list'">
            <app-icon name="list" :size="30" :color="viewMode === 'list' ? '#2563eb' : '#64748b'" />
          </view>
        </view>
      </view>
    </view>
    ...
    <view class="ebs-main">
      ...
      <view v-else-if="sortedBooks.length === 0" class="ebs-empty">
        ...
        <view class="ebs-empty-btn" @tap="goStore">
          <text class="ebs-empty-btn-txt">浏览电子书</text>
        </view>
      </view>

      <!-- 网格视图 -->
      <view v-else-if="viewMode === 'grid'" class="ebs-grid">
        <view v-for="book in sortedBooks" :key="book.id" class="ebs-grid-item">
          <view class="ebs-cover-wrap" @tap="goReader(book)">
            <flat-book-cover :color="book.coverColor" :title="book.title" class="ebs-cover" />
            <!-- 进度条 -->
            <view v-if="book.progress > 0 && book.progress < 100" class="ebs-cover-progress">
              <view class="ebs-cover-progress-fill" :style="{ width: book.progress + '%' }" />
            </view>
            <!-- 已读完标 -->
            <view v-if="book.progress === 100" class="ebs-done-badge">
              <text class="ebs-done-txt">已读完</text>
            </view>
            <!-- 已下载标 -->
            <view v-if="book.isDownloaded" class="ebs-dl-dot">
              <app-icon name="download" :size="20" color="#ffffff" />
            </view>
            <!-- 菜单触发 -->
            <view class="ebs-grid-menu-btn" @tap.stop="toggleMenu(book.id)">
              <app-icon name="more-vertical" :size="24" color="#ffffff" />
            </view>
          </view>
          <view class="ebs-grid-info">
            <text class="ebs-grid-title">{{ book.title }}</text>
            <view class="ebs-grid-meta">
              <text class="ebs-grid-pct">{{ progressLabel(book) }}</text>
              <view class="ebs-grid-time">
                <app-icon name="clock" :size="20" color="#64748b" />
                <text class="ebs-grid-time-txt">{{ book.lastReadAt }}</text>
              </view>
            </view>
          </view>
          <!-- 网格菜单 -->
      ...
      <!-- 列表视图 -->
      <view v-else class="ebs-list">
        <view v-for="book in sortedBooks" :key="book.id" class="ebs-list-card">
          <view class="ebs-list-main" @tap="goReader(book)">
            <flat-book-cover :color="book.coverColor" :title="book.title" class="ebs-list-cover" />
            <view class="ebs-list-info">
              <view>
                <text class="ebs-list-title">{{ book.title }}</text>
                <text class="ebs-list-author">{{ book.author }} · {{ book.category }}</text>
                <view class="ebs-list-sub">
                  <app-icon name="clock" :size="24" color="#94a3b8" />
                  <text class="ebs-list-time">{{ book.lastReadAt }}</text>
                  <view v-if="book.isDownloaded" class="ebs-dl-badge">
                    <app-icon name="download" :size="20" color="#2563eb" />
                    <text class="ebs-dl-badge-txt">已下载</text>
                  </view>
                </view>
              </view>
              <!-- 进度 -->
              <view class="ebs-list-progress">
                <view class="ebs-list-progress-head">
                  <text class="ebs-list-progress-label">{{ listProgressLabel(book) }}</text>
                  <text class="ebs-list-progress-total">{{ book.totalChapters }}章</text>
                </view>
            ...
            <view class="ebs-list-menu-btn" @tap.stop="toggleMenu(book.id)">
              <app-icon name="more-vertical" :size="32" color="#94a3b8" />
            </view>
          </view>
          <!-- 列表展开操作栏 -->
