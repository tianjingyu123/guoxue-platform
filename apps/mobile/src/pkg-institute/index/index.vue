    ...
    <scroll-view scroll-y class="scroll" :style="{ paddingTop: navHeight + 'px' }">
      ...
      <view class="section">
        ...
        <view class="teacher-grid">
          <view v-for="t in instructors.slice(0, 4)" :key="t.id" class="teacher-card" @tap="goInstructor(t.id)">
            <view class="teacher-head">
              <view class="avatar-wrap">
                <view class="avatar"><app-icon name="user" :size="22" color="#9ca3af" /></view>
                <view v-if="t.verified" class="avatar-badge">
                  <app-icon name="badge-check" :size="14" color="#c41e3a" />
                </view>
              </view>
              <view class="teacher-info">
                <text class="teacher-name">{{ t.name }}</text>
                <text class="teacher-title">{{ t.title }}</text>
              </view>
            </view>
            <view class="teacher-tags">
              <text v-for="s in t.specialties.slice(0, 2)" :key="s" class="teacher-tag">{{ s }}</text>
            </view>
            <view class="teacher-foot">
              <view class="foot-item">
                <app-icon name="users" :size="12" color="#9ca3af" />
                <text class="foot-text">{{ t.studentCount }}</text>
              </view>
              <view class="foot-item">
                <app-icon name="star" :size="12" color="#f59e0b" />
                <text class="foot-text">{{ t.rating }}</text>
              </view>
            </view>
          </view>
        </view>
        ...
        <view class="event-list">
          <view v-for="e in enrollingEvents" :key="e.id" class="event-card" @tap="goEvent(e.id)">
            <view class="event-cover">
              ...
              <view class="event-status" :style="{ color: eventStatusColor[e.status].color, background: eventStatusColor[e.status].bg }">
            ...
            <view class="event-body">
              ...
              <view class="event-meta">
                ...
                <text class="event-meta-text">{{ e.startTime.split(' ')[0] }}</text>
              </view>
              <view class="event-meta">
              ...
              <view class="event-foot">
                <text class="event-price">{{ e.price === 0 ? '免费' : '¥' + e.price }}</text>
                <text class="event-enrolled">{{ e.currentParticipants }}人已报名</text>
              </view>
            </view>
          </view>
        </view>
...
function goInstructors() {
  navigateTo('/institute/instructors')
}
...
.stat-num { font-size: 20px; font-weight: 700; color: #c41e3a; }
...
.mission { margin-top: 12px; padding: 12px; background: rgba(196,30,58,0.05); border-left: 2px solid #c41e3a; border-radius: 8px; }
.mission-text { font-size: 13px; color: #c41e3a; font-weight: 500; }
...
.more-text { font-size: 13px; color: #c41e3a; }
...
.event-price { font-size: 14px; font-weight: 600; color: #c41e3a; }
.event-enrolled { font-size: 12px; color: #9ca3af; }

.apply-banner { position: relative; overflow: hidden; border-radius: 12px; background: linear-gradient(to right, #c41e3a, #d4445c); padding: 16px; }
...
.apply-btn-text { font-size: 13px; font-weight: 500; color: #c41e3a; }
