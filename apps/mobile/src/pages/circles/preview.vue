...
const circle = ref<any>({ id: '', name: '', cover: '', description: '', category: '', members: 0, posts: 0, isJoined: false, todayActive: 0, createdAt: '', owner: { id: '', name: '', avatar: '' }, rules: [], tags: [] })
const featuredPosts = ref<any[]>([])
const joinStatus = ref({ isJoined: false, isPaid: false, price: 0, originalPrice: 0, membershipDays: 0, discount: '', joinMethod: 'free' as 'free' | 'paid' | 'approval' })
...
async function loadData() {
  ...
  try {
    ...
    const detail = await circleDetailApi.detail(circleId.value)
    circle.value = {
      id: detail.id,
      name: detail.name,
      cover: detail.cover,
      ...
      todayActive: detail.todayActive || 0,
      createdAt: detail.createdAt,
      owner: detail.owner,
      tags: detail.tags || [],
      rules: detail.rules || [],
    }
    if (detail.todayActive) joinStatus.value = { ...joinStatus.value, isJoined: detail.isJoined }
  } catch (e: any) {
