    ...
    <template v-if="!loading && question">
      ...
      <view class="question-card">
        <view class="q-header">
          <image
            class="q-avatar"
            :src="question.asker?.avatar || '/static/default-avatar.png'"
            mode="aspectFill"
          />
          <text class="q-nick">
            {{ question.asker?.nickname || '匿名' }}
          </text>
          <view
            class="q-status"
        ...
        <view class="q-footer">
          <text class="q-price">
            {{ question.priceCoin }}币
          </text>
          <text class="q-circle">
            {{ question.circle?.name || '' }}
          </text>
        </view>
      </view>
      ...
      <!-- 回答卡片 -->
      <view
        v-if="question.status === 'ANSWERED' && question.answer"
        class="answer-card"
      >
        <view class="a-header">
          <image
            class="a-avatar"
            :src="question.answerer?.avatar || '/static/default-avatar.png'"
            mode="aspectFill"
          />
          <text class="a-nick">
            {{ question.answerer?.nickname || '匿名' }}
          </text>
          <text class="a-label">
            回答
          </text>
        ...
        <view class="a-footer">
          <text class="a-peek">
            {{ question.peekCount || 0 }}人围观
          </text>
          <text
            v-if="question.peekPriceCoin > 0"
            class="a-peek-price"
          >
            围观价 {{ question.peekPriceCoin }}币
          </text>
        </view>
      </view>
      ...
      <!-- 回答按钮（回答者自己） -->
      <view
        v-if="isAnswerer && question.status === 'PENDING'"
