<template>
  <view class="page">
    <view class="nav-bar" :style="{ paddingTop: statusBarHeight + 'px' }">
      ...
      <text class="nav-title">退款进度</text>
    ...
    <scroll-view scroll-y class="scroll-area" :style="{ paddingTop: navHeight + 'px' }">
      ...
      <view class="status-banner">
        <view class="status-icon">
          <app-icon name="refresh-cw" :size="56" color="#FFFFFF" />
        </view>
        <text class="status-title">退款处理中</text>
        <text class="status-sub">预计 {{ data.estimatedDate }} 前到账</text>
      </view>
        ...
        <view class="timeline">
          ...
          >
            <view class="tl-col">
              ...
              <view v-if="idx < data.timeline.length - 1" class="tl-line" :class="{ done: isDone(idx + 1) }" />
            </view>
            <view class="tl-body">
              <text class="tl-title" :class="{ active: node.isCurrent || isDone(idx) }">{{ node.title }}</text>
        ...
        <view class="product-row">
          ...
          <view class="product-info">
            ...
            <view class="product-foot">
              ...
              <text class="product-qty">x{{ data.product.quantity }}</text>
            </view>
          </view>
        </view>
      </view>
      ...
      <view class="card">
        ...
        <view class="info-row">
          ...
          <view class="info-copy">
            ...
            <view class="copy-btn" @tap="copyId">
              <app-icon name="copy" :size="24" color="#9A2D2D" />
            </view>
          </view>
        </view>
        <view class="info-row">
        ...
        <view class="info-row">
          ...
          <text class="info-val amount">¥{{ data.amount }}</text>
        </view>
        <view class="info-row">
          <text class="info-label">退款方式</text>
          <text class="info-val">原路退回（{{ data.refundMethod }}）</text>
        </view>
        <view class="info-row">
    ...
    <view class="action-bar" :style="{ paddingBottom: safeBottom + 'px' }">
      ...
      <view class="action-btn primary" @tap="viewOrder">
        <text class="action-text-primary">查看订单</text>
      </view>
    </view>
  </view>
</template>
...
import { refundProgress } from '@/lib/order-data'
