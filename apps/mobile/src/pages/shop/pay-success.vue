...
const orderId = ref('')
const orderInfo = ref<{ orderId: string; amount: number; payMethod: string; paidAt: string; itemCount: number } | null>(null)
...
function goOrder() { if (orderInfo.value) uni.navigateTo({ url: `/pages/orders/order-detail?id=${orderInfo.value.orderId}` }) }
function goHome() { uni.switchTab({ url: '/pages/index/index' }) }
function goShop() { uni.navigateTo({ url: '/pages/shop/shop' }) }
</script>

<style scoped>
.page { background: linear-gradient(to bottom, #4CAF50, #45a049); min-height: 100vh; }

.success-header-bg { padding: 128rpx 32rpx 64rpx; display: flex; flex-direction: column; align-items: center; }

.check-animation { opacity: 0; transform: scale(0.5); transition: all 0.5s; margin-bottom: 48rpx; }
.check-animation.visible { opacity: 1; transform: scale(1); }

.check-circle { position: relative; width: 192rpx; height: 192rpx; }
.check-circle::before { content: ''; position: absolute; inset: 0; background: #fff; border-radius: 50%; box-shadow: 0 8rpx 32rpx rgba(0,0,0,0.15); }
.check-inner { position: absolute; inset: 0; display: flex; align-items: center; justify-content: center; opacity: 0; transform: scale(0); transition: all 0.7s 0.3s; }
.check-inner.visible { opacity: 1; transform: scale(1); }
.check-icon { font-size: 128rpx; color: #4CAF50; font-weight: bold; }

.check-ring { position: absolute; inset: 0; border-radius: 50%; border: 8rpx solid rgba(255,255,255,0.3); opacity: 1; transform: scale(1); transition: all 1s; }
.check-ring.visible { transform: scale(1.5); opacity: 0; }

.success-title { font-size: 40rpx; font-weight: bold; color: #fff; margin-bottom: 24rpx; opacity: 0; transform: translateY(32rpx); transition: all 0.5s 0.2s; }
.success-title.visible { opacity: 1; transform: translateY(0); }

.success-amount-area { text-align: center; opacity: 0; transform: translateY(32rpx); transition: all 0.5s 0.3s; }
.success-amount-area.visible { opacity: 1; transform: translateY(0); }
.success-amount { font-size: 72rpx; font-weight: bold; color: #fff; display: block; margin-bottom: 8rpx; }
.success-pay-info { font-size: 26rpx; color: rgba(255,255,255,0.8); }

/* 白色卡片 */
.content-cards { background: #FAF8F5; border-radius: 48rpx 48rpx 0 0; min-height: 60vh; padding: 32rpx; }

.card { background: #fff; border-radius: 24rpx; padding: 32rpx; margin-bottom: 24rpx; box-shadow: 0 4rpx 16rpx rgba(0,0,0,0.04); opacity: 0; transform: translateY(32rpx); transition: all 0.5s 0.4s; }
.card.visible { opacity: 1; transform: translateY(0); }

.card-row { display: flex; justify-content: space-between; align-items: center; padding: 20rpx 0; border-bottom: 2rpx solid #E8E3DB; }
...
.action-buttons { display: flex; flex-direction: column; gap: 16rpx; margin-bottom: 24rpx; opacity: 0; transform: translateY(32rpx); transition: all 0.5s 0.5s; }
.action-buttons.visible { opacity: 1; transform: translateY(0); }

.btn-primary-action { height: 96rpx; line-height: 96rpx; text-align: center; background: #C41E3A; color: #fff; border-radius: 16rpx; font-weight: 500; display: flex; align-items: center; justify-content: center; gap: 12rpx; font-size: 30rpx; }
.btn-secondary-action { height: 96rpx; line-height: 96rpx; text-align: center; background: #fff; border: 2rpx solid #E8E3DB; color: #2C2C2C; border-radius: 16rpx; font-weight: 500; display: flex; align-items: center; justify-content: center; gap: 12rpx; font-size: 30rpx; }
.action-icon { font-size: 36rpx; }

.recommend-card { opacity: 0; transform: translateY(32rpx); transition: all 0.5s 0.6s; }
...
.recommend-item { background: #fff; border-radius: 24rpx; padding: 32rpx; display: flex; align-items: center; gap: 20rpx; }
.recommend-icon-wrap { width: 80rpx; height: 80rpx; background: linear-gradient(135deg, #C41E3A, #e85a6b); border-radius: 16rpx; display: flex; align-items: center; justify-content: center; flex-shrink: 0; }
.recommend-icon { font-size: 36rpx; color: #fff; }
.recommend-info { flex: 1; }
.recommend-title { font-size: 28rpx; font-weight: 500; color: #2C2C2C; display: block; margin-bottom: 4rpx; }
