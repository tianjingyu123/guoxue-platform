...
import { reviewTags, reviewSortOptions, shopApi, type ShopProductReview } from '@/lib/shop-data'
...
const reviewSummary = computed(() => {
  ...
  if (!list.length) return { goodRatePercent: 0, rating: 0, total: 0 }
  const good = list.filter((r: any) => r.rating >= 4).length
  const avg = list.reduce((s: number, r: any) => s + r.rating, 0) / list.length
  return { goodRatePercent: Math.round((good / list.length) * 100), rating: Math.round(avg * 10) / 10, total: list.length }
})

const sortLabel = computed(() => reviewSortOptions.find((o) => o.id === sortBy.value)?.label)
const sortedReviews = computed(() => {
  const list = reviews.value.filter((r) => selectedTag.value === 'all' || (r.tags || []).includes(selectedTag.value))
...
const previewReview = computed(() => reviews.value.find((r) => r.id === previewImage.value?.reviewId) || null)

function mapReview(r: ShopProductReview): any {
  return {
    id: Number(r.id) || 0,
    user: { name: r.userName, avatar: r.avatar, level: '' },
    rating: r.rating,
    content: r.content,
    images: r.images || [],
    spec: r.skuName || '',
...
async function loadReviews() {
  ...
  error.value = ''
  try {
    const res = await shopApi.getProductReviews(productId.value)
    reviews.value = (res.items || []).map(mapReview)
  } catch (e: any) { error.value = e?.message || '加载失败' } finally { loading.value = false }
}

onLoad((q: any) => {
  if (q?.id) { productId.value = String(q.id); loadReviews() }
...
<template>
  <view class="page">
    ...
    <view v-if="loading" class="sk-wrap">
      <view class="sk-overview"><view class="sk-big" /><view class="sk-stars" /></view>
      <view class="sk-tags"><view v-for="i in 5" :key="i" class="sk-tag" /></view>
...
.sk-wrap { padding: 0 0 28rpx; }
.sk-overview { display: flex; align-items: center; gap: 48rpx; padding: 36rpx 28rpx; background: #fff; }
.sk-big { width: 120rpx; height: 80rpx; background: #f0ece4; border-radius: 12rpx; }
.sk-stars { flex: 1; height: 40rpx; background: #f0ece4; border-radius: 8rpx; }
.sk-tags { display: flex; gap: 12rpx; padding: 20rpx 28rpx; background: #fff; }
.sk-tag { width: 120rpx; height: 48rpx; background: #f0ece4; border-radius: 24rpx; }
.sk-card { background: #fff; margin: 20rpx 28rpx 0; padding: 28rpx; border-radius: 24rpx; display: flex; flex-direction: column; gap: 16rpx; }
