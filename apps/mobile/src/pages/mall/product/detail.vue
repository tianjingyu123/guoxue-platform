...
import { shopApi, shopProductDetail, cartCount as initialCart } from '@/lib/shop-data'

const productId = ref('')
const loading = ref(true)
const error = ref('')
const product = ref<any>({ ...shopProductDetail })
...
async function loadDetail() {
  ...
  error.value = ''
  try {
    const data = await shopApi.getProductDetail(productId.value)
    product.value = { ...shopProductDetail, ...data }
  } catch (e: any) { error.value = e?.message || '加载失败' } finally { loading.value = false }
}

onLoad((q: any) => {
  if (q?.id) { productId.value = String(q.id); loadDetail() }
...
<template>
  <view class="page">
    ...
    <view v-if="loading" class="sk-wrap">
      <view class="sk-carousel" />
      <view class="sk-card"><view class="sk-line" /><view class="sk-line sk-short" /><view class="sk-line w3" /></view>
      <view class="sk-card"><view class="sk-line w4" /><view class="sk-line w6" /></view>
      <view class="sk-card"><view class="sk-line w5" /><view class="sk-line w3" /><view class="sk-line w4" /></view>
    </view>
...
.sk-wrap { padding-bottom: 130rpx; }
.sk-carousel { width: 100%; height: 750rpx; background: #e8e3db; }
.sk-card { margin: 24rpx 28rpx 0; padding: 28rpx; background: #fff; border-radius: 24rpx; display: flex; flex-direction: column; gap: 16rpx; }
.sk-line { height: 24rpx; background: #f0ece4; border-radius: 8rpx; }
.sk-short { width: 60%; }
.sk-line.w3 { width: 30%; } .sk-line.w4 { width: 40%; } .sk-line.w5 { width: 50%; } .sk-line.w6 { width: 60%; }
</style>
