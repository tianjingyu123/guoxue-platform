...
import { categoryTabs, categorySortOptions, shopApi } from '@/lib/shop-data'
...
async function loadProducts() {
  ...
  error.value = ''
  try {
    const res = await shopApi.getCategoryProducts(activeCategory.value === 'all' ? '' : activeCategory.value)
...
<template>
  <view class="page">
    ...
    <view class="body">
      ...
      <view class="main">
        ...
        <view v-if="loading" class="sk-grid">
          <view v-for="i in 6" :key="i" class="sk-card"><view class="sk-img" /><view class="sk-line" /><view class="sk-line sk-short" /></view>
        </view>

        <error-state v-else-if="error" :message="error" @retry="loadProducts" />
    ...
    <!-- 筛选面板 -->
    <view v-if="showFilter" class="mask" @tap="showFilter = false" />
...
.sk-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 20rpx; padding: 20rpx; }
.sk-card { background: #fff; border-radius: 20rpx; padding: 20rpx; display: flex; flex-direction: column; gap: 12rpx; }
.sk-img { width: 100%; aspect-ratio: 1; background: #f0ece4; border-radius: 12rpx; }
