<template>
  <view class="page v0-page" data-v0-route="bounty/create">
        <view class="min-h-screen bg-[#FAF8F5]">
          ...
          <view class="sticky top-0 z-20 bg-white border-b border-[#E8E3DB]">
            <view class="flex items-center justify-between px-4 h-14">
              ...
              <text class="h1" class="text-base font-semibold text-[#2C2C2C]" :style="{ fontFamily: 'Noto Serif SC, serif' }}>
                发布悬赏
              </text>
              <view class="w-7" />
            </view>
          ...
          <view class="px-4 py-4 space-y-4 pb-36">
            ...
            <view class="bg-white rounded-2xl p-4 shadow-[0_2px_16px_rgba(0,0,0,0.06)]">
              ...
              <input
                class="v0-class" transition-colors`}
            ...
            <view class="bg-white rounded-2xl p-4 shadow-[0_2px_16px_rgba(0,0,0,0.06)]">
              ...
              <view class="grid grid-cols-3 gap-2 mb-3">
                <view v-for="amount in AMOUNT_PRESETS" :key="amount.id || index">
                  <view class="v0-btn"
                    key={amount}
              ...
              {{ isCustom && (
                <view class="v0-class" transition-colors` }}>
            ...
            <view class="bg-white rounded-2xl p-4 shadow-[0_2px_16px_rgba(0,0,0,0.06)]">
              ...
              <view class="grid grid-cols-4 gap-2">
                <view v-for="opt in EXPIRE_OPTIONS" :key="opt.id || index">
                  <view class="v0-btn"
                    key={opt.value}
            ...
            <view class="bg-white rounded-2xl p-4 shadow-[0_2px_16px_rgba(0,0,0,0.06)]">
              ...
              <view class="flex flex-wrap gap-2 mb-3">
                <view v-for="cat in CATEGORY_OPTIONS" :key="cat.id || index">
                  <view class="v0-btn"
                    key={cat}
              ...
              {tags.length > 0 && (
                <view class="flex flex-wrap gap-2 mb-2">
                  <view v-for="tag in tags" :key="tag.id || index">
                    <text
                      key={tag}
              ...
              <view class="flex gap-2">
                <input
                  ...
                  @keydown={e => e.key === 'Enter' && handleAddTag()}
          ...
          {showPayConfirm && (
            <view class="fixed inset-0 z-50 flex items-end">
              ...
              <view class="relative w-full bg-white rounded-t-3xl px-6 pt-6 pb-safe-or-8 animate-in slide-in-from-bottom">
                ...
                <text class="h3" class="text-center text-lg font-bold text-[#2C2C2C] mb-2" :style="{ fontFamily: 'Noto Serif SC, serif' }}>
                  确认支付
                </text>
...
async function fetchData() {
  ...
  error.value = null
  try {
    // TODO: 集成真实 API - V0 路由: bounty/create
    loading.value = false
  } catch (e: any) {
