...
/**
 * 共用电子罗盘：第三方排盘和自研排盘均可调用，不属于自研专属权限。
 *
 * 相对 V0 的取舍：
...
const readingSource = ref<'none' | 'sensor' | 'manual'>('none')
...
const compass = createCompass({
  onHeading: deg => {
    if (!Number.isFinite(deg) || locked.value) return
    applyHeading(deg)
    readingSource.value = 'sensor'
  },
  onStatus: (s) => {
    sensorStatus.value = s
    if (s !== 'active' && readingSource.value === 'sensor' && !locked.value) readingSource.value = 'none'
  },
})

function resetReading() {
  locked.value = false
  readingSource.value = 'none'
  showPicker.value = false
}
onShow(() => { resetReading(); compass.start() })
onHide(() => { compass.stop(); resetReading() })
onUnload(() => { compass.stop(); resetReading() })
onUnmounted(() => compass.stop())

function onSlider(e: { detail: { value: number } }) {
  if (locked.value || !Number.isFinite(e.detail.value)) return
  applyHeading(e.detail.value)
  readingSource.value = 'manual'
}

function toggleLock() {
  if (readingSource.value === 'none') return
  locked.value = !locked.value
  // 锁定时可保留历史判读；解锁后若传感器已断流，旧角度不能继续标成实时读数。
  if (!locked.value && readingSource.value === 'sensor' && sensorStatus.value !== 'active') {
    readingSource.value = 'none'
  }
}
  ...
  <view class="page">
    ...
    <scroll-view scroll-y class="scroll">
      <view class="content">
        <!-- 顶部大度数读数（对应固定指针指向） -->
        <view class="readout">
          <text class="readout-deg">{{ readingSource === 'none' ? '—' : `${sf.deg.toFixed(1)}°` }}</text>
          <text class="readout-dir">{{ readingSource === 'none' ? '等待有效方向读数' : readingSource === 'manual' ? `手动设定 · ${directionText}` : locked ? `已锁定 · ${directionText}` : directionText }}</text>
        </view>
        ...
        <paper-card padding="md">
          <view class="panel">
            ...
            <view class="row-top">
              <view class="sf-block">
                <text class="sf-label">{{ readingSource === 'none' ? '盘面示意，尚未定向' : sf.label }}</text>
                <text v-if="readingSource !== 'none'" class="sf-deg">{{ sf.deg }}°</text>
              </view>
            ...
            <view v-else-if="sensorStatus !== 'active'" class="sensor-block">
              <text class="sensor-hint">{{
                sensorStatus === 'unavailable'
                  ? '暂未收到有效方向读数，可先手动定向；手动读数不代表手机实际朝向'
                  : '正在启动罗盘感应…'
              }}</text>
