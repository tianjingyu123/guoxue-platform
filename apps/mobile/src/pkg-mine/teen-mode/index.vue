...
import { defaultTeenModeSettings, teenTimeLimitOptions, teenFilterLevels, mineSettingsApi, type TeenModeSettings } from '@/lib/mine-data'
...
onMounted(async () => {
  try {
    const data = await mineSettingsApi.getTeenMode()
...
async function save() {
  ...
  saving.value = true
  try {
    await mineSettingsApi.updateTeenMode({
      enabled: settings.enabled,
      timeLimit: settings.dailyLimit,
  ...
  <view class="page">
    ...
    <scroll-view scroll-y class="scroll">
      ...
      <view class="card">
        <view class="row">
          <view class="row-icon" :class="settings.enabled ? 'icon-on' : 'icon-off'">
            <AppIcon name="shield" :size="20" :color="settings.enabled ? '#C41E3A' : '#9b948a'" />
          </view>
          <view class="row-body">
      ...
      <view class="settings" :class="{ disabled: !settings.enabled }">
        ...
        <!-- 提示 -->
        <view class="tips">
          <AppIcon name="alert-circle" :size="18" color="#C9A96E" />
    ...
    <!-- 密码设置弹窗 -->
    <view v-if="pwdModal" class="mask center">
      <view class="modal">
    ...
    <!-- 验证密码弹窗 -->
    <view v-if="verifyModal" class="mask center">
      <view class="modal">
    ...
    <!-- 重置密码弹窗 -->
    <view v-if="resetModal" class="mask center">
      <view class="modal">
    ...
    <!-- 时长选择 -->
    <view v-if="timeLimitSheet" class="mask" @tap="timeLimitSheet = false">
    ...
    <!-- 时段选择 -->
    <view v-if="timeRangeSheet" class="mask" @tap="timeRangeSheet = false">
      <view class="sheet" @tap.stop>
        <view class="sheet-bar" />
    ...
    <!-- 过滤级别选择 -->
    <view v-if="filterSheet" class="mask" @tap="filterSheet = false">
      <view class="sheet" @tap.stop>
        <view class="sheet-bar" />
...
.banner { background: #C41E3A; border-radius: 24rpx; padding: 32rpx; display: flex; gap: 20rpx; margin-bottom: 24rpx; }
...
.tips { display: flex; gap: 16rpx; background: #FBF4E6; border-radius: 24rpx; padding: 28rpx; }
