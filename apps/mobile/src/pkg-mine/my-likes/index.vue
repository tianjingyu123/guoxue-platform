...
import { mineContentApi, likeTypeNames, likeTypeStyles, likeFilterOptions, type LikeItem, type LikeTargetType } from '@/lib/mine-data'
...
onMounted(async () => {
  try {
    const data = await mineContentApi.getMyLikes()
    list.value = (data.items || data || []).map((i: any) => ({ ...i }))
  } catch (e) {
    list.value = []
  } finally {
    loading.value = false
