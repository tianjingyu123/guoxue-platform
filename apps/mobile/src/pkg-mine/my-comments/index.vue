...
import { mineContentApi, commentTypeNames, commentTypeStyles, type MyCommentItem } from '@/lib/mine-data'
...
onMounted(async () => {
  try {
    const data = await mineContentApi.getMyComments()
    list.value = (data.items || data || []).map((c: any) => ({ ...c }))
  } catch (e) {
    list.value = []
  } finally {
    loading.value = false
