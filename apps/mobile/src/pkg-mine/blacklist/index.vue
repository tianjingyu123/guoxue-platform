...
import { mineSettingsApi, blacklistUsers, blacklistSearchPool, type BlacklistItem, type SearchUserItem } from '@/lib/mine-data'
...
onMounted(async () => {
  ...
  } catch { list.value = [...blacklistUsers] }
...
async function confirmRemove() {
  ...
  removing.value = true
  try { await mineSettingsApi.removeFromBlocklist(selected.value.userId) } catch { }
...
async function addToBlacklist(u: SearchUserItem) {
  ...
  adding.value = u.id
  try { await mineSettingsApi.addToBlocklist(u.id) } catch { }
...
<template>
  <view class="page">
    ...
    <!-- 移除确认弹窗 -->
    <view v-if="removeDialog" class="mask center" @tap="removeDialog = false">
    ...
    <!-- 添加黑名单底部弹窗 -->
    <view v-if="addSheet" class="mask" @tap="addSheet = false">
