...
/**
 * 六爻装卦记录（自 V0 app/liuyao/history/page.tsx 还原）
...
import { loadLiuyaoHistory, removeLiuyaoHistory, pinLiuyaoHistory } from '../liuyao-history'
...
<template>
  <HistoryPage
    ...
    :records="vms"
    search-placeholder="搜索所占之事 / 卦名"
    empty-text="暂无起卦记录，起卦后自动保存"
    @pin="onPin"
    @delete="onDelete"
    @open="open"
  >
    <template #card="{ rec }">
      <view class="line1">
        <text class="title">{{ rec.params.matter || '未填所占之事' }}</text>
