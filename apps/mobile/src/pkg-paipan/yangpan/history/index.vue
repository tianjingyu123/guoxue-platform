...
function onPin(ids: string[]) {
  const target = [...ids]
  return preview.run(() => { pinYangpanHistory(target); reload() })
}
function onDelete(ids: string[]) {
  const target = [...ids]
  return preview.run(() => { removeYangpanHistory(target); reload() })
}
function onGroup(p: { ids: string[]; group: string }) {
  const target = [...p.ids]; const group = p.group
  return preview.run(() => { groupYangpanHistory(target, group); reload() })
}
