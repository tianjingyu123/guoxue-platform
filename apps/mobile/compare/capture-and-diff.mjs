...
async function main() {
  ...
  if (!args.skipVue) await ensureVueReady(vuePage)
  const results = []
  ...
  for (const pair of pairs) {
    ...
    try {
      ...
      // --skipProto / --skipVue：内存受限时分两阶段跑（任一时刻只需一个 dev server），复用已存截图
      if (!args.skipProto) {
      ...
      if (args.skipProto || args.skipVue) {
        // 分阶段：缺另一端截图时只截图不 diff（diff 留到两端都就绪的那一阶段）
        if (!fs.existsSync(protoFile) || !fs.existsSync(vueFile)) {
          rec.pass = null
          rec.partial = true
          console.log(`[CAP ] ${args.skipProto ? 'vue' : 'proto'} only: ${pair.proto}`)
          results.push(rec)
          continue
        }
      }
