<template>
  <view class="min-h-screen bg-background pb-6">
    ...
    <view class="px-4 py-4">
      <view class="bg-gradient-to-br from-primary/10 via-[#C9A96E]/5 to-[#FAF8F5] rounded-xl p-4 border border-primary/20">
        <view class="flex items-start justify-between mb-3">
          ...
          <text class="text-xs px-1.5 py-0.5 bg-accent/20 text-accent rounded">进行中</text>
        </view>
        ...
        <!-- 进度条 -->
        <view class="mt-3 h-2 bg-[#E8E0D5] rounded-full overflow-hidden">
          <view
    ...
    <view class="px-4 pb-4">
      <view class="bg-white rounded-xl p-6 text-center border border-border">
        <view
          @click="handleStartScan"
          :class="['relative w-48 h-48 mx-auto rounded-2xl border-2 border-dashed flex items-center justify-center mb-4 transition-all', isScanning ? 'border-accent bg-accent/5' : 'border-border bg-background']"
        >
          <!-- 扫描框角标 -->
          ...
          <template v-if="isScanning">
            <view class="flex flex-col items-center">
              <view class="w-16 h-16 border-4 border-accent border-t-transparent rounded-full mb-3" style="animation: spin 1s linear infinite;" />
        ...
        <view v-if="showManualInput" class="mt-4 text-left">
          <view class="relative">
            ...
            <input
              ...
              class="w-full h-10 pl-10 pr-4 rounded-lg border border-border bg-background text-sm"
            />
          </view>
          <!-- 搜索结果 -->
          <view v-if="searchResults.length > 0" class="mt-2 space-y-2">
            <view
              ...
              class="flex items-center justify-between p-3 bg-white rounded-xl border border-border"
            >
              <view class="flex items-center gap-3">
                <view class="w-10 h-10 rounded-full bg-background flex items-center justify-center text-foreground font-medium">
                  {{ student.name[0] }}
                </view>
                <view>
    ...
    <view class="px-4 space-y-4">
      ...
      <view class="bg-white rounded-xl overflow-hidden border border-border">
        <view @click="showNotCheckedIn = !showNotCheckedIn" class="flex items-center justify-between w-full p-4">
          <view class="flex items-center gap-2">
            <text class="text-xs px-2 py-0.5 bg-orange-500/10 text-orange-500 border border-orange-500/30 rounded">未签到</text>
            <text class="text-sm text-muted-foreground">{{ notCheckedInStudents.length }}人</text>
          </view>
          <text :class="['text-sm text-muted-foreground', showNotCheckedIn ? 'rotate-180' : '']">▼</text>
        </view>
        <view v-if="showNotCheckedIn && notCheckedInStudents.length > 0" class="border-t border-border">
          <view v-for="student in notCheckedInStudents" :key="student.id" class="flex items-center justify-between px-4 py-3 border-b border-[#F5F0E8] last:border-b-0">
            <view class="flex items-center gap-3">
              <view class="w-10 h-10 rounded-full bg-background flex items-center justify-center text-foreground font-medium">
                {{ student.name[0] }}
              </view>
            ...
            <view @click="handleManualCheckin(student)" class="px-3 py-1.5 border border-primary text-primary text-xs rounded-full">
              手动签到
            </view>
          </view>
        </view>
      ...
      <view class="bg-white rounded-xl overflow-hidden border border-border">
        <view @click="showCheckedIn = !showCheckedIn" class="flex items-center justify-between w-full p-4">
          <view class="flex items-center gap-2">
            <text class="text-xs px-2 py-0.5 bg-green-500/10 text-green-500 border border-green-500/30 rounded">已签到</text>
            <text class="text-sm text-muted-foreground">{{ checkedInStudents.length }}人</text>
          </view>
          <text :class="['text-sm text-muted-foreground', showCheckedIn ? 'rotate-180' : '']">▼</text>
        </view>
        <view v-if="showCheckedIn && checkedInStudents.length > 0" class="border-t border-border">
          <view v-for="student in checkedInStudents" :key="student.id" class="flex items-center justify-between px-4 py-3 border-b border-[#F5F0E8] last:border-b-0">
            <view class="flex items-center gap-3">
              <view class="w-10 h-10 rounded-full bg-background flex items-center justify-center text-foreground font-medium">
                {{ student.name[0] }}
              </view>
            ...
            <view class="flex items-center gap-1 text-xs text-green-500">
              <text>✓</text>
    ...
    <view v-if="showConfirmModal && scannedStudent" class="fixed inset-0 z-50 flex items-center justify-center bg-black/60">
      <view class="w-[85%] max-w-sm bg-white rounded-2xl overflow-hidden">
        <view class="p-6 text-center">
          <view class="w-20 h-20 rounded-full bg-background flex items-center justify-center mx-auto mb-4 ring-4 ring-[#C9A96E]/30">
            <text class="text-2xl text-foreground font-bold">{{ scannedStudent.name[0] }}</text>
          </view>
        ...
        <view class="flex border-t border-border">
          <view @click="showConfirmModal = false" class="flex-1 py-4 text-sm text-muted-foreground text-center">
            取消
          </view>
          <view @click="handleConfirmCheckin" class="flex-1 py-4 text-sm font-medium text-accent text-center border-l border-border">
            确认签到
          </view>
        </view>
      </view>
    ...
    <view v-if="showSuccessAnimation" class="fixed inset-0 z-50 flex items-center justify-center bg-black/40 pointer-events-none">
      <view class="w-32 h-32 rounded-full bg-green-500 flex items-center justify-center" style="animation: zoomIn 0.3s ease-out;">
        <text class="text-white text-4xl font-bold">✓</text>
      </view>
    </view>
  </view>
</template>
...
interface Student {
  ...
  enrollTime: string; checkedIn: boolean; checkTime: string | null
}
...
function handleConfirmCheckin() {
  if (scannedStudent.value) {
    ...
    students.value = students.value.map(s =>
      s.id === scannedStudent.value!.id
        ? { ...s, checkedIn: true, checkTime: timeStr }
        : s
    )
...
function handleSearch(query: string) {
  ...
  if (query.length >= 2) {
    searchResults.value = notCheckedInStudents.value.filter(s =>
