<template>
  <view class="min-h-screen bg-background">
    <header class="sticky top-0 z-10 bg-background border-b border-border flex items-center px-4 h-12 gap-3">
      ...
      <text class="text-base font-semibold text-foreground">诗词分类</text>
    </header>

    <view class="px-4 pt-4 pb-20 space-y-3">
      <view
        v-for="cat in categories"
        :key="cat.id"
        @click="goCategory(cat.id)"
        class="w-full flex items-center gap-4 p-4 bg-card border border-border rounded-xl"
      >
        <text class="text-3xl shrink-0">{{ cat.icon }}</text>
        <view class="flex-1 min-w-0">
          <view class="flex items-center justify-between gap-2 mb-0.5">
            <text class="text-sm font-semibold text-foreground">{{ cat.name }}</text>
            <text class="text-xs text-muted-foreground">{{ cat.count.toLocaleString() }} 首</text>
          </view>
          <text class="text-xs text-muted-foreground block mb-2">{{ cat.desc }}</text>
          <view class="flex gap-1.5 flex-wrap">
            <text
              v-for="s in cat.subCategories"
              :key="s"
              class="text-[10px] bg-muted text-muted-foreground px-2 py-0.5 rounded-full"
            >{{ s }}</text>
          </view>
        </view>
        <text class="text-base text-muted-foreground shrink-0">›</text>
      </view>
    </view>
  </view>
</template>
...
function goCategory(id: string) {
  uni.navigateTo({ url: `/pages/poetry/categories/${id}/index` })
}
</script>
