  ...
  <view class="min-h-screen bg-background">
    ...
    <view class="sticky top-0 z-10 bg-white border-b border-border">
      <view class="flex items-center justify-between px-4 h-12">
        ...
        <view @click="handleExport" class="flex items-center gap-1 text-xs text-muted-foreground">
          <text>⬇</text>
          <text>导出</text>
        </view>
      </view>
    ...
    <view v-if="loading" class="p-4 space-y-4">
      ...
      <view class="bg-white rounded-xl p-4 border border-border">
        ...
        <view class="grid grid-cols-2 gap-3">
          <view class="h-16 bg-secondary rounded-lg animate-pulse" />
          <view class="h-16 bg-secondary rounded-lg animate-pulse" />
        </view>
      </view>
      <view class="h-16 bg-white rounded-xl animate-pulse border border-border" />
      <view v-for="i in 3" :key="'r'+i" class="h-20 bg-white rounded-xl animate-pulse border border-border" />
    </view>
    ...
    <view v-else class="p-4 space-y-4">
      <!-- 时间范围选择 -->
      <view class="flex gap-2">
        <view v-for="r in RANGES" :key="r.key" @click="range = r.key" :class="['flex-1 py-1.5 rounded-full text-xs font-medium text-center', range === r.key ? 'bg-primary text-white' : 'bg-muted text-foreground']">
          <text>{{ r.label }}</text>
        </view>
      </view>
      ...
      <view class="bg-white rounded-xl p-4 border border-border">
        ...
        <view class="grid grid-cols-2 gap-3">
          <view class="bg-secondary rounded-lg p-3">
      ...
      <!-- 提现入口 -->
      <view @click="goWithdraw" class="w-full flex items-center justify-between bg-primary/5 border border-primary/20 rounded-xl p-4">
        <view class="flex items-center gap-2.5">
          <view class="w-9 h-9 rounded-full bg-primary/10 flex items-center justify-center">
            <text class="text-primary"></text>
          </view>
          <view class="text-left">
            <text class="text-sm font-semibold text-foreground">可提现金额</text>
            <text class="text-xs text-muted-foreground">T+1 结算，最低100元可提</text>
          </view>
        </view>
        <view class="flex items-center gap-1.5">
          <text class="text-lg font-bold text-primary">¥{{ Math.round(stats.total * 0.7) }}</text>
      ...
      <view>
        ...
        <view class="flex gap-2 mb-3">
          <view v-for="f in typeFilters" :key="f.key" @click="typeFilter = f.key" :class="['px-3 py-1.5 rounded-full text-xs font-medium', typeFilter === f.key ? 'bg-primary text-white' : 'bg-muted text-foreground']">
            <text>{{ f.label }}</text>
          </view>
        </view>

        <!-- 空状态 -->
        <view v-if="filtered.length === 0" class="flex flex-col items-center justify-center py-16 text-muted-foreground">
        ...
        <view v-else class="space-y-2">
          <view v-for="record in filtered" :key="record.id" class="bg-white rounded-xl p-3.5 border border-border">
            <view class="flex items-start justify-between gap-2">
              <view class="flex items-start gap-2.5 min-w-0">
                ...
                <view class="min-w-0">
                  <text class="text-sm font-medium text-foreground block truncate">{{ record.desc }}</text>
                  <text class="text-xs text-muted-foreground block truncate">{{ record.live }}</text>
