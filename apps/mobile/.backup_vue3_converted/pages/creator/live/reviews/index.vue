<template>
  <view class="min-h-screen bg-background">
    ...
    <view v-if="loading" class="p-4 space-y-4">
      ...
      <view v-for="i in 3" :key="'r'+i" class="h-32 bg-white rounded-xl animate-pulse border border-border" />
    </view>
    ...
    <view v-else class="p-4 space-y-4">
        ...
        <view class="flex gap-4">
          <view class="text-center flex-shrink-0">
            ...
            <view class="flex gap-0.5 justify-center">
              <text v-for="s in 5" :key="s" :class="s <= Math.round(Number(avgRating)) ? 'text-accent' : 'text-muted-foreground'"></text>
            </view>
      ...
      <scroll-view scroll-x class="flex-row" style="white-space:nowrap">
        <view v-for="f in FILTERS" :key="f.key" @click="filter = f.key" :class="['inline-flex px-3 py-1.5 rounded-full text-xs font-medium mr-2 flex-shrink-0', filter === f.key ? 'bg-primary text-white' : 'bg-muted text-foreground']">
          <text>{{ f.label }}</text>
        </view>
      </scroll-view>
        ...
        <view v-for="review in filtered" :key="review.id" :class="['bg-white rounded-xl p-4 border', review.flagged ? 'border-accent/60' : 'border-border']">
          ...
          <view class="flex items-start justify-between gap-2 mb-2">
            <view class="flex items-center gap-2">
              <view class="w-8 h-8 rounded-full bg-muted flex items-center justify-center text-sm font-bold text-muted-foreground flex-shrink-0">
                <text>{{ review.user[0] }}</text>
              </view>
              <view>
                <text class="text-sm font-medium text-foreground block">{{ review.user }}</text>
                <view class="flex gap-0.5">
                  <text v-for="s in 5" :key="s" :class="s <= review.rating ? 'text-accent' : 'text-muted-foreground'"></text>
                </view>
              </view>
            </view>
            <text class="text-xs text-muted-foreground flex-shrink-0">{{ review.time }}</text>
          </view>
          ...
          <!-- 已有回复 -->
          <view v-if="replies[review.id]" class="bg-muted rounded-lg p-2.5 mb-2">
          ...
          <view v-if="replyId === review.id" class="mt-2 space-y-2">
            ...
            <view class="flex gap-2">
              <view @click="cancelReply" class="flex-1 py-1.5 text-xs text-muted-foreground bg-muted rounded-lg text-center">
                <text>取消</text>
              </view>
              <view @click="submitReply(review.id)" class="flex-1 py-1.5 text-xs text-white bg-primary rounded-lg text-center">
                <text>发布回复</text>
              </view>
            </view>
          </view>
