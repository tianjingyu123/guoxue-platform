  ...
  <view class="min-h-screen bg-background">
    ...
    <view class="sticky top-0 z-10 bg-white border-b border-border">
      <view class="flex items-center justify-between px-4 h-12">
        ...
        <view @click="goAddProduct" class="flex items-center gap-1 text-xs text-primary font-medium">
          <text>+</text>
          <text>添加商品</text>
        </view>
      </view>
    ...
    <view class="p-4 space-y-3">
      ...
      <view class="relative">
        ...
        <input
          v-model="search"
          placeholder="搜索商品名称..."
          class="w-full pl-9 pr-3 py-2 text-sm bg-muted border-0 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary/30"
        />
      </view>

      <!-- 状态筛选 -->
      <view class="flex gap-2">
        <view v-for="f in FILTERS" :key="f.key" @click="filter = f.key" :class="['px-3 py-1.5 rounded-full text-xs font-medium', filter === f.key ? 'bg-primary text-white' : 'bg-muted text-foreground']">
          <text>{{ f.label }}</text>
        </view>
      </view>
      ...
      <view v-if="filtered.length === 0" class="flex flex-col items-center justify-center py-16 text-muted-foreground">
        ...
        <view @click="goAddProduct" class="mt-3 px-4 py-2 text-xs text-white bg-primary rounded-full">
          <text>添加第一件商品</text>
        </view>
      </view>
      ...
      <view v-else class="space-y-3">
        <view v-for="product in filtered" :key="product.id" class="bg-white rounded-xl p-3.5 border border-border">
          <view class="flex gap-3">
            <!-- 封面 -->
            <view class="w-16 h-16 rounded-lg overflow-hidden bg-muted flex-shrink-0">
              <image :src="product.cover" mode="aspectFill" class="w-full h-full" />
            </view>

            <!-- 信息 -->
            <view class="flex-1 min-w-0">
              <text class="text-sm font-medium text-foreground block truncate mb-1">{{ product.name }}</text>
            ...
            <view class="flex flex-col items-end gap-2 flex-shrink-0">
              <view @click="toggleStatus(product.id)" :class="['relative w-10 h-5 rounded-full transition-colors', product.status === 'on' ? 'bg-green-500' : 'bg-muted']">
          ...
          <view class="flex items-center gap-4 mt-3 pt-3 border-t border-border">
            <view @click="handleEdit(product)" class="flex items-center gap-1 text-xs text-muted-foreground">
              <text>›</text>
              <text>编辑</text>
            </view>
            <view @click="remove(product.id)" class="flex items-center gap-1 text-xs text-red-600">
              <text>🗑</text>
              <text>删除</text>
            </view>
            <text v-if="product.stock === 0" class="ml-auto text-xs text-red-600 font-medium">库存不足</text>
          </view>
        </view>
      </view>
    </view>
...
const FILTERS = [
  { key: 'all' as Status, label: '全部' },
  { key: 'on' as Status, label: '已上架' },
  { key: 'off' as Status, label: '已下架' },
]
...
function goAddProduct() {
  uni.navigateTo({ url: '/pages/creator/live/products/add' })
}

function handleEdit(product: Product) {
  uni.navigateTo({ url: '/pages/creator/live/products/edit?id=' + product.id })
}
</script>
