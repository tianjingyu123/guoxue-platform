<template>
  <view class="min-h-screen bg-background pb-20">
    ...
    <view class="sticky top-0 z-50 bg-background/95 backdrop-blur-md border-b border-border">
      <view class="flex items-center justify-between px-4 py-3">
        ...
        <view class="flex items-center gap-2">
          <view class="h-8 text-xs px-3 rounded-lg border border-border flex items-center">
            <text>⬇ 导出报告</text>
          </view>
          <view class="h-8 text-xs px-3 rounded-lg border border-border flex items-center">
            <text> 分享</text>
    ...
    <!-- Tab切换 -->
    <view class="w-full">
      <scroll-view scroll-x class="w-full px-4 h-10 bg-transparent border-b border-border flex items-center gap-0">
        <view
          v-for="tab in tabs"
          :key="tab.value"
          class="text-xs px-3 py-2 whitespace-nowrap"
          :class="activeTab === tab.value ? 'text-primary font-medium border-b-2 border-primary' : 'text-muted-foreground'"
      ...
      <view v-if="activeTab === 'overview'" class="px-4 py-4 space-y-4">
        <!-- 直播信息卡片 -->
        <view class="p-4 bg-white rounded-2xl border border-border">
          <view class="flex gap-3">
            ...
            <view class="flex-1">
              <view class="flex items-center gap-2">
                <text class="text-[10px] bg-secondary px-1.5 py-0.5 rounded">
              ...
              <text class="text-xs text-muted-foreground mt-0.5 block">时长：{{ liveData.duration }}</text>
            </view>
          </view>
        </view>
        ...
        <view class="grid grid-cols-2 gap-3">
          <view v-for="stat in coreStats" :key="stat.label" class="p-3 bg-white rounded-xl border border-border">
            ...
            <view class="flex items-center gap-1 mt-2 text-xs" :class="stat.trend === 'up' ? 'text-green-500' : stat.trend === 'down' ? 'text-red-500' : 'text-muted-foreground'">
      ...
      <view v-if="activeTab === 'traffic'" class="px-4 py-4 space-y-4">
        ...
        <view class="p-4 bg-white rounded-2xl border border-border">
          <view class="flex items-center justify-between mb-4">
            ...
            <text class="text-[10px] text-muted-foreground">峰值 3,256</text>
          </view>
          <view class="h-40 flex items-end gap-1.5">
            <view v-for="(item, index) in trafficData" :key="index" class="flex-1 flex flex-col items-center gap-1">
              <view class="w-full rounded-t transition-all" :class="item.value === maxTraffic ? 'bg-primary' : 'bg-primary/40'" :style="{ height: (item.value / maxTraffic) * 100 + '%' }" />
              <text class="text-[9px] text-muted-foreground">{{ item.time.split(':')[1] }}</text>
            </view>
          </view>
            ...
            <view v-for="(item, index) in keyMoments" :key="index" class="flex gap-3">
              ...
              <view class="flex-1 pb-3">
                <view class="flex items-center gap-2">
                  ...
                  <text class="text-[10px] bg-secondary px-1.5 py-0.5 rounded">{{ item.event }}</text>
                </view>
      ...
      <view v-if="activeTab === 'audience'" class="px-4 py-4 space-y-4">
        ...
        <view class="p-4 bg-white rounded-2xl border border-border">
          ...
          <view class="space-y-2">
            <view v-for="(item, index) in audienceRegion.slice(0, 5)" :key="item.name" class="flex items-center gap-3">
              <view class="w-5 h-5 rounded-full flex items-center justify-center text-[10px] font-bold text-white" :class="index === 0 ? 'bg-amber-500' : index === 1 ? 'bg-gray-400' : index === 2 ? 'bg-amber-700' : 'bg-secondary text-muted-foreground'">
                <text>{{ index + 1 }}</text>
              </view>
        ...
        <view class="p-4 bg-white rounded-2xl border border-border">
          ...
          <view class="grid grid-cols-2 gap-2">
            <view v-for="item in audienceSource" :key="item.label" class="flex items-center gap-2 p-2.5 rounded-lg bg-secondary/50">
      ...
      <view v-if="activeTab === 'interaction'" class="px-4 py-4 space-y-4">
        ...
        <view class="grid grid-cols-4 gap-2">
          <view class="p-2.5 text-center bg-white rounded-xl border border-border">
            <text></text>
            <text class="text-sm font-bold mt-1 block">{{ interactionData.danmaku.toLocaleString() }}</text>
            <text class="text-[10px] text-muted-foreground">弹幕</text>
          </view>
          <view class="p-2.5 text-center bg-white rounded-xl border border-border">
            <text></text>
            <text class="text-sm font-bold mt-1 block">{{ interactionData.likes.toLocaleString() }}</text>
            <text class="text-[10px] text-muted-foreground">点赞</text>
          </view>
          <view class="p-2.5 text-center bg-white rounded-xl border border-border">
            <text>💭</text>
        ...
        <view class="p-4 bg-white rounded-2xl border border-border">
          ...
          <view class="space-y-2">
            <view v-for="(gift, index) in interactionData.gifts" :key="gift.name" class="flex items-center gap-3 p-2 rounded-lg bg-secondary/50">
      ...
      <view v-if="activeTab === 'replay'" class="px-4 py-4 space-y-4">
        ...
        <!-- 回放设置 -->
        <view class="p-4 bg-white rounded-2xl border border-border space-y-4">
        ...
        <!-- 上架操作 -->
        <view class="p-4 bg-white rounded-2xl border border-border space-y-3">
          <text class="font-semibold text-sm block">上架至</text>
          <view class="w-full p-3 rounded-xl border border-border flex items-center">
        ...
        <view class="bg-white rounded-2xl border border-border overflow-hidden">
          ...
          <view class="p-3">
            ...
            <text class="text-xs text-muted-foreground mt-1 block">{{ liveData.startTime }}</text>
          </view>
        </view>
      </view>
    </view>
...
const audienceSource = [
  { label: '首页推荐', value: 35, icon: '🏠' }, { label: '关注列表', value: 28, icon: '' },
