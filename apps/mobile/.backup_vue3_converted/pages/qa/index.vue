  ...
  <view class="min-h-screen bg-background">
    ...
    <view class="sticky top-0 z-20 bg-white border-b border-border">
      <view class="flex items-center justify-between px-4 py-3">
        ...
        <text class="text-lg font-semibold text-foreground">付费问答</text>
        <view
          class="flex items-center gap-1 px-3 py-1.5 bg-primary text-white text-sm font-medium rounded-full"
          @click="goToAsk"
        >
      ...
      <view class="flex px-4 gap-6">
        <view
          ...
          :class="activeTab === tab.key ? 'text-primary border-primary' : 'text-muted-foreground border-transparent'"
    ...
    <view class="p-4 space-y-3">
      ...
      <template v-else-if="filteredQuestions.length === 0">
        <view class="flex flex-col items-center justify-center py-20">
          ...
          <text class="text-muted-foreground">暂无问答内容</text>
          <view
        ...
        >
          ...
          <view class="flex items-center justify-between mb-3">
            <view class="flex items-center gap-3">
              ...
              <view>
                <text class="text-sm font-medium text-foreground">{{ question.asker.name }}</text>
                <text class="text-xs text-muted-foreground block">{{ formatTime(question.createdAt) }}</text>
              </view>
            </view>
            <view
          ...
          <!-- Question Title -->
          <text class="text-base font-medium text-foreground mb-2 line-clamp-2 block">
            {{ question.title }}
          </text>

          <!-- Price & Circle Tags -->
          <view class="flex items-center gap-2 mb-3">
            <view class="px-2 py-0.5 bg-[#FFF4E5] text-accent text-xs font-medium rounded">
              ¥{{ question.price }}
            </view>
            <view
              v-if="question.circleName"
              class="px-2 py-0.5 bg-[#F5F5F5] text-ink-soft text-xs rounded"
            >
              {{ question.circleName }}
            </view>
            <view
              v-if="!question.isPublic"
              class="px-2 py-0.5 bg-gray-100 text-muted-foreground text-xs rounded"
            >
              私密
            </view>
          </view>
          ...
          <view
            ...
            class="flex items-center gap-2 p-3 bg-background rounded-xl mb-3"
          >
            <view
            ...
            <view class="flex-1 min-w-0">
              <view class="flex items-center gap-2">
                <text class="text-sm font-medium text-foreground">{{ question.answerer.name }}</text>
                <text v-if="question.answerer.title" class="text-xs text-accent">
                  {{ question.answerer.title }}
                </text>
              </view>
              <text
                v-if="question.answerPreview"
                class="text-xs text-ink-soft line-clamp-1 block mt-0.5"
              >
                {{ question.answerPreview }}
              </text>
            </view>
            <text class="text-muted-foreground text-lg flex-shrink-0">›</text>
          </view>
