<template>
  <view class="min-h-screen bg-background">
    ...
    <view class="sticky top-0 z-10 bg-background border-b border-border flex items-center px-4 h-12 gap-3">
      ...
      <text class="text-base font-semibold text-foreground">邮箱找回密码</text>
    </view>
    ...
    <view v-else class="px-6 pt-6">
      <view class="flex items-center justify-center mb-8">
        <view v-for="(step, index) in steps" :key="step.key" class="flex items-center">
          ...
          <view
            ...
            class="w-12 h-[2px] mx-1 mt-[-18px] transition-colors duration-300"
      ...
      <view v-if="currentStep === 0">
        ...
        <text class="text-xl font-bold text-foreground text-center block mb-2">验证您的邮箱</text>
        <text class="text-sm text-muted-foreground text-center block mb-8">输入注册邮箱并完成验证</text>

        <view class="space-y-4">
          <view>
            <label class="text-sm font-medium text-foreground mb-1.5 block">邮箱地址</label>
            <input
              ...
              @input="validateEmail"
            />
            <text v-if="emailError" class="text-xs text-danger mt-1 block">{{ emailError }}</text>
          </view>
      ...
      <view v-if="currentStep === 1">
        ...
        <text class="text-xl font-bold text-foreground text-center block mb-2">验证邮箱</text>
        <text class="text-sm text-muted-foreground text-center block mb-2">验证码已发送至 {{ email }}</text>
        <text class="text-xs text-accent text-center block mb-8">请检查收件箱和垃圾邮件</text>
        ...
        <view class="space-y-4">
          <view>
            ...
            <view class="flex gap-2">
              ...
              <view
                ...
                @click="resendCode"
              >
                <text>{{ cd > 0 ? cd + 's后重发' : '重新发送' }}</text>
              </view>
            </view>
            <text class="text-xs text-muted-foreground mt-1 block">验证码有效期为15分钟</text>
          </view>
          ...
          <view class="flex justify-center">
            <text class="text-xs text-muted-foreground" @click="currentStep = 0">← 返回修改邮箱</text>
          </view>
        </view>
      </view>
      ...
      <view v-if="currentStep === 2">
        ...
        <view class="space-y-4">
          <view>
            ...
            />
            <text v-if="passwordStrength" class="text-xs mt-1 block" :class="passwordStrengthColor">{{ passwordStrength }}</text>
          </view>
          <view>
            ...
            />
            <text v-if="confirmError" class="text-xs text-danger mt-1 block">{{ confirmError }}</text>
          </view>
      ...
      <view v-if="currentStep === 3">
        <view class="flex flex-col items-center pt-8">
          <view class="w-20 h-20 rounded-full bg-green-50 border-4 border-green-200 flex items-center justify-center mb-6 animate-bounce">
            <text class="text-4xl"></text>
          </view>
          <text class="text-2xl font-bold text-foreground mb-2">密码重置成功</text>
          <text class="text-sm text-muted-foreground text-center mb-8">请使用新密码登录您的账号</text>
    ...
    <!-- 发送成功提示 -->
    <view v-if="showSentToast" class="fixed top-16 left-4 right-4 z-50 bg-white rounded-xl shadow-lg border border-border p-4 flex items-start gap-3 animate-slide-down">
      <text class="text-green-500 text-lg flex-shrink-0"></text>
      <view class="flex-1">
        <text class="text-sm font-medium text-foreground block">邮件已发送</text>
        <text class="text-xs text-muted-foreground mt-0.5 block">验证邮件已发送至 {{ email }}，请查看并按指引操作</text>
      </view>
      <text class="text-muted-foreground text-sm flex-shrink-0" @click="showSentToast = false">✕</text>
    </view>
...
function validateEmail() {
  ...
  emailError.value = emailRegex.test(email.value) ? '' : '请输入有效的邮箱地址'
}
...
const passwordStrength = computed(() => {
  ...
  if (/^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[!@#$%])/.test(pwd)) return '强密码'
  if (/^(?=.*[a-zA-Z])(?=.*\d)/.test(pwd)) return '中等级别'
  return '弱密码，建议包含字母和数字'
})

const passwordStrengthColor = computed(() => {
  const s = passwordStrength.value
  if (s === '强密码') return 'text-green-500'
  if (s === '中等级别') return 'text-accent'
  if (s) return 'text-danger'
  return ''
})

const confirmError = computed(() => {
  if (!confirmPassword.value) return ''
  return confirmPassword.value === newPassword.value ? '' : '两次输入的密码不一致'
})
...
function sendCode() {
  ...
  setTimeout(() => {
    ...
    setTimeout(() => { showSentToast.value = false }, 4000)
    startCountdown()
  }, 1000)
}
