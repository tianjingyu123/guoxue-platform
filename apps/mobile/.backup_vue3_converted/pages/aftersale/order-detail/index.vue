<template>
  <!-- 提交成功页 -->
  <view v-if="submitSuccess" class="min-h-screen bg-background flex flex-col items-center justify-center p-8">
    ...
    <text class="text-xl font-bold text-foreground mb-2">提交成功</text>
    <text class="text-sm text-muted-foreground text-center mb-8">售后申请已提交，商家将在24小时内审核</text>
    <view class="w-full max-w-xs space-y-3">
      <view class="w-full py-3 bg-primary text-white text-center rounded-xl text-sm font-medium" @click="viewProgress">查看进度</view>
      <view class="w-full py-3 border border-border text-foreground text-center rounded-xl text-sm" @click="goBack">返回订单</view>
    </view>
  </view>

  <!-- 进度跟踪页 -->
  <view v-else-if="viewingProgress" class="min-h-screen bg-background pb-24">
    <view class="sticky top-0 z-50 bg-background/95 backdrop-blur-lg border-b border-border" style="padding-top: var(--status-bar-height);">
      <view class="flex items-center justify-between px-4 h-14">
        ...
        <text class="font-semibold text-base text-foreground">售后进度</text>
        <view class="w-9" />
      </view>
    </view>
    ...
    <view class="p-4 space-y-4">
      ...
      <view class="p-4 rounded-xl bg-white border border-border">
        <view class="flex items-center gap-3">
          ...
          <view>
            <text class="font-medium text-foreground block">商家审核中</text>
            <text class="text-xs text-muted-foreground">预计24小时内完成审核</text>
          </view>
        </view>
      </view>
        ...
        <view class="space-y-0">
          <view v-for="(step, idx) in progressSteps" :key="idx" class="flex gap-3">
            <view class="flex flex-col items-center">
              <view :class="['w-6 h-6 rounded-full flex items-center justify-center text-xs', step.completed ? 'bg-primary text-white' : step.active ? 'bg-blue-500 text-white' : 'bg-secondary text-muted-foreground']">
                <text v-if="step.completed">✓</text>
                <text v-else-if="step.active">●</text>
                <text v-else>○</text>
              </view>
              <view v-if="idx < progressSteps.length - 1" class="w-0.5 flex-1 min-h-6" :class="step.completed ? 'bg-primary' : 'bg-[#E8E0D5]'" />
            </view>
            <view class="flex-1 pb-4">
              <text :class="['text-sm block', step.completed || step.active ? 'text-foreground font-medium' : 'text-muted-foreground']">{{ step.title }}</text>
              <text class="text-xs text-muted-foreground block mt-0.5">{{ step.time || step.desc }}</text>
            </view>
          </view>
        </view>
      </view>
      ...
      <view class="p-4 rounded-xl bg-white border border-border">
        ...
        <view class="space-y-2 text-sm">
          <view class="flex justify-between">
            ...
            <text class="text-foreground">{{ aftersaleTypes.find(t => t.id === selectedType)?.label }}</text>
          </view>
          <view class="flex justify-between">
            <text class="text-muted-foreground">申请原因</text>
            <text class="text-foreground">{{ aftersaleReasons.find(r => r.id === selectedReason)?.label }}</text>
          </view>
          <view class="flex justify-between">
          ...
          <view class="flex justify-between">
            <text class="text-muted-foreground">申请单号</text>
            <view class="flex items-center gap-1">
              <text class="text-foreground">{{ afterSaleNo }}</text>
              <text class="text-primary text-xs" @click="copyNo">复制</text>
            </view>
          </view>
        </view>
      </view>
      ...
      <view class="flex gap-3 pt-2">
        <view class="flex-1 py-3 border border-border text-foreground text-center rounded-xl text-sm" @click="cancelAfterSale">撤销申请</view>
        <view class="flex-1 py-3 bg-primary text-white text-center rounded-xl text-sm" @click="contactService">联系客服</view>
      </view>
    </view>
  </view>
  ...
  <view v-else class="min-h-screen bg-background pb-24">
    ...
    <view class="pt-14 p-4 space-y-4">
        ...
        <view class="space-y-2">
          ...
          >
            <view :class="['w-10 h-10 rounded-full flex items-center justify-center', selectedType === type.id ? 'bg-primary/10' : 'bg-secondary']">
              <text :class="['w-5 h-5', selectedType === type.id ? 'text-primary' : 'text-muted-foreground']">{{ type.id === 'refund_only' ? '' : type.id === 'return_refund' ? '📦' : '🚚' }}</text>
            </view>
      ...
      <view class="flex items-start gap-2 px-2">
        <text class="text-muted-foreground text-sm mt-0.5"></text>
        <text class="text-xs text-muted-foreground">提交申请后，商家将在24小时内审核。如审核通过，请按指引操作。</text>
      </view>
    </view>
    ...
    <view class="fixed bottom-0 left-0 right-0 bg-white/95 backdrop-blur-lg border-t border-border px-4 py-3" style="padding-bottom: env(safe-area-inset-bottom);">
      <view
        @click="handleSubmit"
        :class="['w-full py-3.5 rounded-xl font-medium text-base text-center transition-all', selectedType && selectedReason && !isSubmitting ? 'bg-primary text-white' : 'bg-muted text-muted-foreground']"
      >
