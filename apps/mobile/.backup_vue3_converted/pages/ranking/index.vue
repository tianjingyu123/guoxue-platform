<template>
  <view class="min-h-screen bg-background">
    <!-- Gradient header -->
    <view class="bg-gradient-to-br from-primary to-primary/70 px-4 pt-12 pb-14">
      <view class="flex items-center gap-3 mb-5">
        <view @click="goBack">
          <text class="text-2xl text-white">←</text>
        </view>
        <text class="text-xl font-bold text-white flex-1">排行榜</text>
        <text class="text-2xl text-amber-300"></text>
      </view>
      <view class="flex bg-white/10 rounded-xl p-1">
        <view
          v-for="t in TABS"
          :key="t.key"
          @click="tab = t.key"
          class="flex-1 py-2 text-sm font-medium rounded-lg text-center transition-all"
          :class="tab === t.key ? 'bg-white text-primary' : 'text-white/80'"
        >
          <text>{{ t.label }}</text>
        </view>
      </view>
    ...
    <!-- Content area -->
    <view class="-mt-6 px-4 pb-20 space-y-2">
      <!-- Experts tab -->
      <template v-if="tab === 'experts'">
        <view
          v-for="e in experts"
          :key="e.id"
          class="flex items-center gap-3 p-3 rounded-xl border mb-2"
          :class="e.rank <= 3 ? 'bg-[#FFFBEB] border-[#FDE68A]' : 'bg-white border-border'"
        >
          <text
            class="w-7 text-center font-black text-lg flex-shrink-0"
            :class="e.rank === 1 ? 'text-[#F59E0B]' : e.rank === 2 ? 'text-[#94A3B8]' : e.rank === 3 ? 'text-[#FB923C]' : 'text-muted-foreground text-sm'"
          >
            {{ e.rank <= 3 ? ['', '', ''][e.rank - 1] : e.rank }}
          </text>
          <image class="w-10 h-10 rounded-full flex-shrink-0" :src="e.avatar" />
          <view class="flex-1 min-w-0">
            <view class="flex items-center gap-1.5">
              <text class="text-sm font-medium text-foreground">{{ e.name }}</text>
              <text
                v-if="e.badge"
            ...
            <text class="text-xs text-muted-foreground">{{ e.specialty }}</text>
          </view>
          <view class="text-right flex-shrink-0">
            <text class="text-sm font-bold text-primary">{{ e.score.toLocaleString() }}</text>
            <view class="text-[10px] text-muted-foreground">积分</view>
          </view>
        </view>
      </template>

      <!-- Circles tab -->
      <template v-if="tab === 'circles'">
        <view
          v-for="c in circles"
          :key="c.id"
          class="flex items-center gap-3 p-3 rounded-xl border mb-2"
          :class="c.rank <= 3 ? 'bg-[#FFFBEB] border-[#FDE68A]' : 'bg-white border-border'"
        >
          <text
            class="w-7 text-center font-black text-lg flex-shrink-0"
            :class="c.rank === 1 ? 'text-[#F59E0B]' : c.rank === 2 ? 'text-[#94A3B8]' : c.rank === 3 ? 'text-[#FB923C]' : 'text-muted-foreground text-sm'"
          >
            {{ c.rank <= 3 ? ['', '', ''][c.rank - 1] : c.rank }}
          </text>
          <image class="w-10 h-10 rounded-xl flex-shrink-0" :src="c.cover" />
          <view class="flex-1 min-w-0">
            <text class="text-sm font-medium text-foreground">{{ c.name }}</text>
            <text class="text-xs text-muted-foreground">{{ c.members.toLocaleString() }} 成员</text>
          </view>
          <view class="text-right flex-shrink-0">
            <text class="text-sm font-bold text-primary">{{ c.score.toLocaleString() }}</text>
            <view class="text-[10px] text-muted-foreground">热度值</view>
          </view>
        </view>
      </template>
      ...
      <template v-if="tab === 'content'">
        <view
          v-for="c in content"
          :key="c.id"
          class="flex items-start gap-3 p-3 rounded-xl border mb-2"
          :class="c.rank <= 3 ? 'bg-[#FFFBEB] border-[#FDE68A]' : 'bg-white border-border'"
        >
          <text
            class="w-7 text-center font-black text-lg flex-shrink-0 mt-1"
            :class="c.rank === 1 ? 'text-[#F59E0B]' : c.rank === 2 ? 'text-[#94A3B8]' : c.rank === 3 ? 'text-[#FB923C]' : 'text-muted-foreground text-sm'"
          >
            {{ c.rank <= 3 ? ['', '', ''][c.rank - 1] : c.rank }}
          </text>
          <view class="flex-1 min-w-0">
            <text class="text-sm font-medium text-foreground line-clamp-2">{{ c.title }}</text>
            <view class="flex items-center gap-2 mt-1.5 text-xs text-muted-foreground">
              <image class="w-4 h-4 rounded-full" :src="c.avatar" />
              <text>{{ c.author }}</text>
              <text>· {{ c.views }} 浏览</text>
              <text>· {{ c.likes }} 赞</text>
            </view>
          </view>
        </view>
