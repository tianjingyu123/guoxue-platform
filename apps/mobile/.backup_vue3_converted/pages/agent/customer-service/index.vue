<template>
  <view class="flex flex-col h-screen bg-background">
    ...
    <scroll-view scroll-y class="flex-1 px-4 py-4 space-y-4" :scroll-into-view="scrollToId">
      <view v-for="msg in messages" :key="msg.id" :class="['flex gap-3 mb-4', msg.role === 'user' ? 'flex-row-reverse' : '']">
        ...
        >
          ...
          <text :class="['text-[10px] mt-1 block', msg.role === 'assistant' ? 'text-muted-foreground' : 'text-white/70']">{{ msg.time }}</text>
        </view>
      </view>
    ...
    <!-- 输入区域 -->
    <view class="flex-shrink-0 px-4 pb-6 pt-2 border-t border-border flex items-end gap-2">
      <textarea
        v-model="inputText"
