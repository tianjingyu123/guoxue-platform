<template>
  <view class="min-h-screen bg-background">
    ...
    <header class="sticky top-0 z-10 bg-background border-b border-border flex items-center px-4 h-12 gap-3">
      ...
      <text class="text-base font-semibold text-foreground">主播列表</text>
    </header>
    ...
    <view class="px-4 pt-4 pb-20">
      ...
      <view class="relative mb-4">
        ...
        <input
          v-model="search"
          placeholder="搜索主播"
          class="w-full pl-9 pr-3 py-2 rounded-lg bg-muted text-sm text-foreground placeholder:text-muted-foreground outline-none border border-border"
        />
      </view>
      ...
      <view class="flex gap-2 mb-4">
        <view
          ...
          :key="f.key"
          @click="filter = f.key"
          :class="['px-3 py-1.5 rounded-full text-sm font-medium', filter === f.key ? 'bg-primary text-white' : 'bg-muted text-foreground']"
        >
          <text>{{ f.label }}</text>
        </view>
      </view>
      ...
      <view class="flex flex-col gap-3">
        ...
        >
          <!-- 封面 -->
          <view class="relative h-32 bg-muted">
          ...
          <view class="flex items-center gap-3 p-3">
            <view class="w-12 h-12 rounded-full flex items-center justify-center overflow-hidden flex-shrink-0 border-2 border-background -mt-6 bg-secondary">
              <image v-if="host.avatar" :src="host.avatar" mode="aspectFill" class="w-full h-full" />
              <text v-else class="text-sm text-ink-soft">{{ host.name[0] }}</text>
            </view>
            <view class="flex-1 min-w-0">
              <view class="flex items-center gap-1 mb-0.5">
                <text class="text-sm font-semibold text-foreground">{{ host.name }}</text>
                <view v-if="host.verified" class="text-[10px] bg-primary/10 text-primary px-1.5 py-0.5 rounded-full">
                  <text>认证</text>
                </view>
              </view>
              <text class="text-xs text-muted-foreground truncate block mb-1">{{ host.specialty }}</text>
              <view class="flex gap-2 text-xs text-muted-foreground flex-wrap">
                <text class="flex items-center gap-0.5"> {{ (host.followers / 1000).toFixed(0) }}k 粉丝</text>
                <text class="flex items-center gap-0.5"> {{ (host.likes / 10000).toFixed(0) }}w 获赞</text>
