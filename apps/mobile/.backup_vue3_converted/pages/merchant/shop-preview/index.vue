  ...
  <view class="min-h-screen bg-background pb-20">
    ...
    <view class="mx-4 -mt-3 relative z-10 bg-white rounded-2xl p-4 shadow-sm">
      ...
      <view class="flex gap-3 mt-4 pt-4 border-t border-border">
        <view class="flex-1 py-2 border border-border rounded-xl text-center text-xs"> 关注店铺</view>
        <view class="flex-1 py-2 border border-border rounded-xl text-center text-xs"> 联系客服</view>
      </view>
    </view>
    ...
    <!-- 商品列表 -->
    <view class="mt-4">
      <view class="px-4 sticky top-14 z-40 bg-background py-2">
        <view class="grid grid-cols-4 h-9 bg-muted rounded-lg p-0.5">
          <view v-for="t in productTabs" :key="t.key" :class="['inline-flex items-center justify-center whitespace-nowrap rounded-md text-xs font-medium transition-all', activeProductTab === t.key ? 'bg-white text-foreground shadow-sm' : 'text-muted-foreground']">
            <text>{{ t.label }}</text>
          </view>
        </view>
      </view>
      ...
      <view class="px-4 mt-3">
        <view class="grid grid-cols-2 gap-3">
          <view v-for="product in filteredProducts" :key="product.id" class="bg-white rounded-2xl overflow-hidden">
            <view class="aspect-square bg-background flex items-center justify-center">
    ...
    <view class="fixed bottom-0 left-0 right-0 p-4 bg-white border-t border-border">
      <view class="flex items-center gap-3">
        <view class="relative w-10 h-10 border border-border rounded-xl flex items-center justify-center">
          <text class="text-lg"></text>
          <text class="absolute -top-1 -right-1 w-4 h-4 rounded-full bg-primary text-[10px] text-white flex items-center justify-center">2</text>
        </view>
        <view class="flex-1 py-3 bg-primary text-white rounded-xl text-center text-sm font-medium">去购物车结算</view>
      </view>
    </view>
  </view>
</template>
