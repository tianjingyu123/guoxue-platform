<template>
  <view class="min-h-screen bg-background">
    ...
    <view class="sticky top-0 z-10 bg-background border-b border-border flex items-center px-4 h-12 gap-3">
      ...
      <text class="text-base font-semibold text-foreground flex-1">圈子活动</text>
    ...
    <view class="flex gap-2 px-4 pt-4 pb-2 overflow-x-auto">
      <view
        ...
        :class="[
          ...
          currentFilter === f.value ? 'bg-primary text-white' : 'bg-muted text-foreground'
        ]"
      >
        <text>{{ f.label }}</text>
      </view>
    ...
    <view class="px-4 pb-20 space-y-4 pt-2">
      ...
      <view v-for="act in filtered" :key="act.id" class="bg-white border border-border rounded-xl overflow-hidden">
        <view class="relative">
        ...
        <view class="p-4">
          <text class="font-semibold text-foreground mb-1 block">{{ act.title }}</text>
          <text class="text-xs text-primary mb-3 block">{{ act.circleName }}</text>
          <view class="space-y-1.5 text-xs text-muted-foreground mb-4">
            ...
            <view class="flex items-center gap-1.5">
              ...
              <view class="flex-1 h-1 bg-muted rounded-full overflow-hidden ml-1">
                <view
