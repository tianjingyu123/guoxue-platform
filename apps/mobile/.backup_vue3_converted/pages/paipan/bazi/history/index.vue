  ...
  <view class="min-h-screen bg-background flex flex-col">
    ...
    <header class="bg-white border-b border-border sticky top-0 z-20">
      <view class="flex items-center justify-between px-4 py-3">
        <view @click="goBack" class="flex items-center text-muted-foreground p-1 -ml-1">
          <text class="text-xl leading-none">←</text>
        </view>

        <!-- 切换标签 -->
        <view class="flex bg-[#F1EDE8] rounded-full p-0.5">
          <view class="px-5 py-1.5 text-sm font-medium rounded-full bg-white text-foreground shadow-sm">
            <text>用户列表</text>
          </view>
          <view @click="goCelebrities" class="px-5 py-1.5 text-sm font-medium rounded-full text-muted-foreground relative">
            <text>案例库</text>
      ...
      <view v-if="showMenu">
        ...
        <view class="absolute right-3 top-12 bg-white rounded-xl shadow-xl z-20 py-2 min-w-[140px] animate-fade-in-down">
          <view @click="goGroups" class="w-full flex items-center gap-3 px-4 py-2.5 text-foreground/80 hover:bg-secondary/50 transition-colors">
            <text class="text-sm text-muted-foreground"></text>
            <text class="text-sm">分组编辑</text>
          </view>
          <view @click="setSelectMode('group')" class="w-full flex items-center gap-3 px-4 py-2.5 text-foreground/80 hover:bg-secondary/50 transition-colors">
            <text class="text-sm text-muted-foreground">📁</text>
            <text class="text-sm">修改分组</text>
          </view>
          <view @click="setSelectMode('pin')" class="w-full flex items-center gap-3 px-4 py-2.5 text-foreground/80 hover:bg-secondary/50 transition-colors">
            <text class="text-sm text-muted-foreground"></text>
            <text class="text-sm">星标置顶</text>
          </view>
          <view @click="setSelectMode('delete')" class="w-full flex items-center gap-3 px-4 py-2.5 text-foreground/80 hover:bg-secondary/50 transition-colors">
    ...
    <view class="bg-white px-4 py-3 border-b border-border/60">
      <view class="relative">
        ...
        <input
          ...
          class="w-full h-10 pl-9 pr-4 bg-[#F1EDE8] rounded-xl text-sm placeholder:text-muted-foreground/50 box-border"
    ...
    <!-- 分组标签 -->
    <view class="bg-white px-4 py-2 border-b border-border/60 overflow-x-auto">
      <view class="flex gap-2" style="white-space:nowrap">
        <view v-for="g in groups" :key="g"
          ...
          :class="activeGroup === g ? 'bg-primary text-white' : 'bg-[#F1EDE8] text-muted-foreground'"
        >
          <text>{{ g }}</text>
        </view>
      </view>
    ...
    <!-- 批量操作栏 - 删除模式 -->
    <view v-if="selectMode === 'delete'" class="sticky bottom-0 bg-white border-t border-border px-4 py-3 flex items-center gap-3">
      ...
      <view class="flex-1 flex gap-2 justify-end">
        <view @click="cancelSelect" class="px-6 py-2.5 text-sm font-medium text-muted-foreground bg-[#F1EDE8] rounded-full">取消</view>
    ...
    <!-- 批量操作栏 - 置顶模式 -->
    <view v-if="selectMode === 'pin'" class="sticky bottom-0 bg-white border-t border-border px-4 py-3 flex items-center gap-3">
      <view class="flex-1 flex gap-3">
        <view @click="cancelSelect" class="flex-1 py-2.5 text-sm font-medium text-muted-foreground bg-[#F1EDE8] rounded-full text-center">取消</view>
    ...
    <!-- 批量操作栏 - 修改分组模式 -->
    <view v-if="selectMode === 'group'" class="sticky bottom-0 bg-white border-t border-border px-4 py-3 flex items-center gap-3">
      <view @click="selectAll" class="flex items-center gap-2">
        <view class="w-5 h-5 rounded-full border-2 flex items-center justify-center transition-all"
          :class="selectedIds.length === filteredRecords.length && filteredRecords.length > 0 ? 'border-primary bg-primary' : 'border-border'"
        >
      ...
      <view class="flex-1 flex gap-2 justify-end">
        <view @click="cancelSelect" class="px-6 py-2.5 text-sm font-medium text-muted-foreground bg-[#F1EDE8] rounded-full">取消</view>
        <view @click="showGroupPicker = true" class="px-6 py-2.5 text-sm font-medium text-white bg-primary rounded-full"
          :class="selectedIds.length > 0 ? '' : 'opacity-40'"
        >移动到分组</view>
      </view>
    </view>
      ...
      <view class="bg-white w-full rounded-t-2xl overflow-hidden" @click.stop>
        ...
        <scroll-view scroll-y class="max-h-[50vh]">
          <view v-for="g in groups.filter((g:string) => g !== '全部')" :key="g"
            ...
            class="w-full px-4 py-4 text-left text-foreground border-b border-border/60 last:border-b-0 hover:bg-secondary/50 transition-colors"
          >
            <text>{{ g }}</text>
          </view>
        </scroll-view>
        <view class="p-4 border-t border-border">
          <view @click="showGroupPicker = false" class="w-full py-3 bg-[#F1EDE8] text-muted-foreground rounded-full font-medium text-center">取消</view>
        </view>
      </view>
    </view>
  </view>
