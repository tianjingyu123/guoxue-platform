    ...
    <view v-else class="p-4 space-y-4">
        ...
        <view class="grid grid-cols-4 gap-2">
          <view v-for="(pillar, idx) in mockBaziResult.fourPillars" :key="idx" class="text-center">
            ...
            <view class="space-y-1">
              <view class="h-12 rounded-lg flex items-center justify-center text-white text-lg font-bold" :class="elementBgColor(pillar.element)">
      ...
      <view v-if="toolId === 'bazi' && result.result" class="p-4 bg-white rounded-xl border border-border">
        ...
        <view class="space-y-3">
          <view class="flex justify-between"><text class="text-muted-foreground">日主</text><text class="font-medium text-foreground">{{ mockBaziResult.dayMaster }}</text></view>
          <view class="flex justify-between"><text class="text-muted-foreground">格局</text><text class="font-medium text-foreground">{{ mockBaziResult.pattern }}</text></view>
          <view class="flex justify-between"><text class="text-muted-foreground">身强弱</text><text class="font-medium text-foreground">{{ mockBaziResult.strength }}</text></view>
        </view>
      </view>
      ...
      <view class="flex gap-3">
        <view class="flex-1 flex items-center justify-center gap-2 py-3 rounded-xl bg-secondary text-foreground"><text></text><text>分享</text></view>
        <view class="flex-1 flex items-center justify-center gap-2 py-3 rounded-xl bg-secondary text-foreground"><text>💾</text><text>保存</text></view>
        <view class="flex-1 flex items-center justify-center gap-2 py-3 rounded-xl bg-primary text-white"><text></text><text>AI解读</text></view>
      </view>

      <view @click="handleReset" class="w-full py-3 text-primary text-sm text-center"><text>重新排盘</text></view>
    </view>
  </view>
</template>
...
const schema = reactive<any>({
  ...
  properties: {
    ...
    gender: { type: 'enum', label: '性别', values: [{ label: '男', value: 'male' }, { label: '女', value: 'female' }] },
    ...
    genderSelect: { type: 'enum', label: '性别确认', values: [{ label: '男', value: 'male' }, { label: '女', value: 'female' }] },
  }
})
...
const mockBaziResult = {
  ...
  dayMaster: '戊土', pattern: '正官格', strength: '身旺',
}
...
const elementColors: Record<string, string> = {
  wood: 'bg-green-500', fire: 'bg-red-500', earth: 'bg-yellow-600', metal: 'bg-gray-400', water: 'bg-blue-500',
}
...
function handleReset() { result.value = null; error.value = null }
