<template>
  <div class="page">
    ...
    <el-table v-loading="loading" :data="list" stripe>
        ...
        <template #default="{ row }">
          <div class="order-detail">
            <el-descriptions :column="3" border size="small">
              ...
              <el-descriptions-item label="买家">{{ row.buyerName }}</el-descriptions-item>
              <el-descriptions-item label="金额">¥{{ Number(row.totalAmount || 0).toFixed(2) }}</el-descriptions-item>
              <el-descriptions-item label="收件人">{{ row.receiverName }}</el-descriptions-item>
              <el-descriptions-item label="手机号">{{ row.receiverPhone }}</el-descriptions-item>
              <el-descriptions-item label="地址" :span="1">{{ row.receiverAddress }}</el-descriptions-item>
              <el-descriptions-item v-if="row.trackingNo" label="物流">{{ row.shipCompany }} / {{ row.trackingNo }}</el-descriptions-item>
            </el-descriptions>
            <div v-if="row.items?.length" style="margin-top:12px">
              <h5 style="margin:0 0 8px">商品明细</h5>
              <el-table :data="row.items" size="small" border>
                ...
                <el-table-column label="小计" width="100">
                  <template #default="{ row: item }">¥{{ (Number(item.price || 0) * Number(item.quantity || 0)).toFixed(2) }}</template>
                </el-table-column>
              </el-table>
            </div>
          </div>
      ...
      <el-table-column label="操作" width="200" fixed="right">
        <template #default="{ row }">
          ...
          <el-button v-if="row.status === 'REFUNDING'" size="small" text type="success" @click="doApproveRefund(row)">同意退款</el-button>
          <el-button v-if="row.status === 'REFUNDING'" size="small" text type="danger" @click="openRejectRefund(row)">拒绝退款</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <!-- 拒绝退款 -->
    <el-dialog v-model="rejectDialog" title="拒绝退款" width="450px">
      <el-form label-width="80px">
        <el-form-item label="拒绝原因" required>
          <el-input v-model="rejectReason" type="textarea" :rows="3" placeholder="请输入拒绝退款原因" />
        </el-form-item>
      </el-form>
      <template #footer>
        <el-button @click="rejectDialog = false">取消</el-button>
        <el-button type="primary" :loading="saving" @click="doRejectRefund">确认拒绝</el-button>
      </template>
    </el-dialog>
  </div>
</template>
