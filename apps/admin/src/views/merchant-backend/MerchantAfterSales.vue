    ...
    >
      ...
      >
        <template #default="{ row }">
          <el-tag
            size="small"
            :type="row.type === 'refund' ? 'danger' : row.type === 'return' ? 'warning' : ''"
          >
      ...
      >
        <template #default="{ row }">
          ¥{{ Number(row.amount || row.order?.amount || 0).toFixed(2) }}
        </template>
      </el-table-column>
      <el-table-column
        prop="reason"
        ...
        <template #default="{ row }">
          <template v-if="row.status === 'PENDING'">
            <el-button
              ...
              text
              type="success"
              @click="processAfterSale(row, 'approve')"
            >
              同意
            </el-button>
            <el-button
              ...
              text
              type="danger"
              @click="processAfterSale(row, 'reject')"
            >
              拒绝
            </el-button>
          </template>
          <el-button
            ...
            text
            type="primary"
            @click="processAfterSale(row, 'complete')"
          >
            完成
          </el-button>
          <span
