<template>
  <div class="inventory-page">
    <section class="hero">
      <div>
        <p class="eyebrow">MERCHANT SUPPLY CHAIN</p>
        <h1>库存与采购</h1>
        <p class="hero-copy">采购入库、库存盘点、销售发货与退货验收共用一套库存账。</p>
      </div>
    ...
    <section class="metrics">
      <button class="metric" @click="activate('stock')">
        <span>总库存</span><strong>{{ overview.totalStock }}</strong><small>{{ overview.skuCount }} 个商品规格</small>
      </button>
      <button class="metric warning" @click="showLowStock">
        <span>库存预警</span><strong>{{ overview.lowStockCount }}</strong><small>{{ overview.outOfStockCount }} 个规格已售罄</small>
      </button>
      <button class="metric" @click="activate('purchase')">
        <span>待到货</span><strong>{{ overview.pendingPurchaseCount }}</strong><small>支持分批验收入库</small>
      </button>
    ...
    <section class="chain">
      <div class="chain-head">
        ...
        <span class="chain-note">采购 → 入库 → 销售 → 发货 → 售后</span>
      </div>
      <div class="chain-track">
        <button :class="{ on: active === 'purchase' }" @click="activate('purchase')"><i>01</i><b>采购补货</b><span>草稿、确认、分批到货</span></button>
        <button :class="{ on: active === 'stock' }" @click="activate('stock')"><i>02</i><b>库存管理</b><span>预警、盘点、损耗调整</span></button>
        <button @click="router.push('/merchant-backend/shipping')"><i>03</i><b>销售发货</b><span>订单、批量运单、轨迹</span></button>
        <button @click="router.push('/merchant-backend/after-sales')"><i>04</i><b>售后回仓</b><span>验收合格后才回补</span></button>
      </div>
    </section>
    ...
    <section class="workspace">
      ...
      <template v-else>
        ...
        <div v-loading="loading" class="purchase-list">
          <article v-for="order in purchases" :key="order.id" class="purchase-card">
            <header>
              <div><span class="order-no">{{ order.orderNo }}</span><h3>{{ order.supplierName }}</h3></div>
              <el-tag :type="purchaseTag(order.status)">{{ purchaseLabels[order.status] || order.status }}</el-tag>
            </header>
...
type Purchase = { id:string; orderNo:string; supplierName:string; status:string; totalAmount:number|string; expectedAt?:string|null; createdAt:string; items:PurchaseItem[] };
...
const overview = reactive({ totalStock:0, skuCount:0, lowStockCount:0, outOfStockCount:0, movementCount:0, pendingPurchaseCount:0 });
...
<style scoped>
.inventory-page{--red:#c9183d;--ink:#27231f;--paper:#f7f3eb;--line:#e8dfd1;min-height:100%;padding:22px;background:linear-gradient(135deg,#f7f3eb 0,#fff 52%,#f4eee4 100%);color:var(--ink)}
.hero{display:flex;justify-content:space-between;align-items:end;gap:24px;padding:30px 34px;border-radius:22px;color:#fff;background:radial-gradient(circle at 82% 16%,rgba(255,218,150,.34),transparent 23%),linear-gradient(118deg,#8c122b,#c9183d 55%,#7a2132);box-shadow:0 18px 40px rgba(111,26,46,.18)}
.eyebrow{margin:0 0 8px;font-size:11px;letter-spacing:.24em;opacity:.72}.hero h1{margin:0;font-family:"Noto Serif SC","Songti SC",serif;font-size:34px;letter-spacing:.08em}.hero-copy{margin:10px 0 0;opacity:.82}.hero-actions{display:flex;gap:10px;flex-wrap:wrap}.ghost-btn{color:#fff;border-color:rgba(255,255,255,.45);background:rgba(255,255,255,.08)}.ghost-btn:hover{color:#fff;border-color:#fff;background:rgba(255,255,255,.16)}
...
.chain,.workspace{border:1px solid var(--line);border-radius:20px;background:rgba(255,255,255,.93);box-shadow:0 12px 35px rgba(55,40,25,.05)}.chain{padding:20px 24px;margin-bottom:18px}.chain-head{display:flex;align-items:center;justify-content:space-between}.chain-head div{display:flex;gap:12px;align-items:baseline}.chain-head span{color:#8d8275;font-size:13px}.chain-note{letter-spacing:.06em}.chain-track{display:grid;grid-template-columns:repeat(4,1fr);gap:10px;margin-top:18px}.chain-track button{position:relative;display:grid;grid-template-columns:36px 1fr;text-align:left;gap:3px 8px;padding:15px;border:1px solid #ede5d9;border-radius:14px;background:#fbf9f5;cursor:pointer}.chain-track button:not(:last-child)::after{content:"›";position:absolute;right:-9px;top:29px;z-index:2;color:#c3b8a8}.chain-track button.on{border-color:#d9a4ae;background:#fff5f7}.chain-track i{grid-row:1/3;font-style:normal;color:var(--red);font-family:Georgia,serif}.chain-track span{font-size:12px;color:#8b8175}
...
.purchase-list{padding:18px;display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:14px;min-height:180px}.purchase-card{border:1px solid var(--line);border-radius:16px;padding:18px;background:#fff}.purchase-card header,.purchase-card footer,.purchase-meta{display:flex;justify-content:space-between;gap:12px;align-items:center}.purchase-card h3{margin:5px 0 0}.order-no{font:12px ui-monospace,SFMono-Regular,monospace;color:#8b8175}.purchase-meta{margin:14px 0;padding:12px 0;border-block:1px dashed #e7ded2;color:#796f64;font-size:13px}.batch-line{display:grid;gap:10px}.batch-line>div{display:grid;grid-template-columns:1fr 52px 120px;gap:10px;align-items:center}.purchase-card footer{justify-content:flex-end;margin-top:16px}.form-grid{display:grid;grid-template-columns:1fr 1fr;gap:0 16px}.purchase-editor{width:100%;display:flex;flex-direction:column;gap:9px}.purchase-row{display:grid;grid-template-columns:minmax(220px,1fr) 120px 150px 32px;gap:8px}.row-label{order:-1;color:#8b8175;font-size:12px}.purchase-total{margin-right:auto;font-weight:700;color:var(--red)}.receive-row{display:flex;align-items:center;justify-content:space-between;padding:13px 0;border-bottom:1px solid #eee7dc}.receive-row b,.receive-row span{display:block}.receive-row span,.dialog-tip{color:#8a8176;font-size:13px}
@media(max-width:1000px){.metrics{grid-template-columns:repeat(2,1fr)}.chain-track{grid-template-columns:repeat(2,1fr)}.chain-track button::after{display:none}.purchase-list{grid-template-columns:1fr}}
@media(max-width:720px){.inventory-page{padding:12px}.hero{align-items:flex-start;flex-direction:column;padding:22px}.hero h1{font-size:28px}.metrics{gap:8px}.metric{padding:14px}.chain-head,.chain-head div{align-items:flex-start;flex-direction:column}.chain-note{display:none}.filters{align-items:stretch;flex-direction:column}.filters .el-input{max-width:none}.purchase-row{grid-template-columns:1fr 88px 110px 28px}.form-grid{grid-template-columns:1fr}}
@media(prefers-reduced-motion:reduce){.metric{transition:none}.metric:hover{transform:none}}
</style>
