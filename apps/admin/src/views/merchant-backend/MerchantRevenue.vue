<template>
  <div class="page">
    ...
    <el-row :gutter="16" v-loading="loadingOverview" style="margin-bottom:20px">
      <el-col :span="6">
        <el-card class="stat-card" shadow="hover">
          <div class="stat-label">累计收入</div>
          <div class="stat-value">¥{{ Number(overview.totalRevenue || 0).toFixed(2) }}</div>
        </el-card>
      </el-col>
      <el-col :span="6">
        <el-card class="stat-card" shadow="hover">
          <div class="stat-label">待结算金额</div>
          <div class="stat-value" style="color:#E6A23C">¥{{ Number(overview.pendingSettlement || 0).toFixed(2) }}</div>
        </el-card>
      </el-col>
      <el-col :span="6">
        <el-card class="stat-card" shadow="hover">
          <div class="stat-label">已结算金额</div>
          <div class="stat-value" style="color:#67C23A">¥{{ Number(overview.settledAmount || 0).toFixed(2) }}</div>
        </el-card>
      </el-col>
      <el-col :span="6">
        <el-card class="stat-card" shadow="hover">
          <div class="stat-label">平台抽成比例</div>
          <div class="stat-value">{{ ((overview.commissionRate || 0) * 100).toFixed(1) }}%</div>
        </el-card>
      </el-col>
    </el-row>

    <h4 style="margin-bottom:12px">结算记录</h4>
    <el-table v-loading="loading" :data="list" stripe>
      ...
      <el-table-column label="总收入" width="120">
        <template #default="{ row }">¥{{ Number(row.totalRevenue || 0).toFixed(2) }}</template>
      </el-table-column>
      ...
      <el-table-column label="状态" width="90">
        <template #default="{ row }">
          <el-tag :type="({ PENDING: 'warning', PAID: 'success', CANCELLED: 'info' } as any)[row.status] || 'info'" size="small">
