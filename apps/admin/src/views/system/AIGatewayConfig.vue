<template>
  <div class="aigw-page">
    <div class="page-header">
      <h3>AI网关 — 模型路由配置</h3>
      <div>
        <el-button type="primary" :loading="saving" @click="saveConfig">保存全部</el-button>
    ...
    <el-row :gutter="16" style="margin-bottom:16px">
      <el-col :span="6">
        <div class="stat-card"><span class="value">{{ sceneCount }}</span><span class="label">已配置场景</span></div>
      </el-col>
      <el-col :span="6">
        <div class="stat-card"><span class="value">{{ defaultModel }}</span><span class="label">默认模型</span></div>
      </el-col>
      <el-col :span="6">
        <div class="stat-card warn"><span class="value">{{ cappedScenes }}</span><span class="label">超预算场景</span></div>
      </el-col>
      <el-col :span="6">
        <div class="stat-card"><span class="value">{{ sceneBudgets ? Object.keys(sceneBudgets.scenes || {}).length : 0 }}</span><span class="label">有预算配额的场景</span></div>
      </el-col>
    </el-row>
    ...
    <el-tabs v-model="activeTab">
      ...
      <el-tab-pane label="默认配置" name="default">
        <el-card>
          <template #header><span>全局默认路由参数（所有未独立配置的场景使用）</span></template>
          <el-form :model="config.default" label-width="120px" size="small">
            <el-row :gutter="16">
              <el-col :span="8">
                <el-form-item label="默认模型">
                  <el-select v-model="config.default.model" style="width:100%">
              ...
              <el-col :span="8">
                <el-form-item label="备用模型">
                  <el-select v-model="config.default.fallbackModel" clearable style="width:100%">
                    <el-option v-for="m in availableModels" :key="m.value" :label="m.label" :value="m.value" />
                  </el-select>
                </el-form-item>
              </el-col>
              <el-col :span="8">
                <el-form-item label="Temperature">
                  <el-input-number v-model="config.default.temperature" :min="0" :max="2" :step="0.1" :precision="1" style="width:100%" />
                </el-form-item>
              </el-col>
            </el-row>
            <el-row :gutter="16">
              <el-col :span="8">
                <el-form-item label="Max Tokens">
                  <el-input-number v-model="config.default.maxTokens" :min="256" :max="32768" :step="256" style="width:100%" />
                </el-form-item>
              </el-col>
              <el-col :span="8">
                <el-form-item label="Top P">
                  <el-input-number v-model="config.default.topP" :min="0" :max="1" :step="0.05" :precision="2" style="width:100%" />
                </el-form-item>
              </el-col>
              <el-col :span="8">
                <el-form-item label="超时(秒)">
                  <el-input-number v-model="config.default.timeoutSec" :min="5" :max="60" style="width:100%" />
                </el-form-item>
              </el-col>
            </el-row>
          </el-form>
      ...
      <el-tab-pane label="场景配置" name="scenes">
        <div style="margin-bottom:12px;display:flex;gap:8px">
          <el-input v-model="newSceneName" placeholder="新场景名（如 content_generation）" size="small" style="width:280px" />
          <el-button size="small" type="primary" @click="addScene" :disabled="!newSceneName.trim()">添加场景</el-button>
        </div>

        <el-table :data="sceneList" stripe size="small">
          <el-table-column label="场景" width="200">
            <template #default="{ row }">
              <el-tag>{{ row.name }}</el-tag>
            </template>
          </el-table-column>
          <el-table-column label="主模型" width="200">
            <template #default="{ row }">
              <el-select v-model="row.config.model" size="small" style="width:100%">
                <el-option v-for="m in availableModels" :key="m.value" :label="m.label" :value="m.value" />
              </el-select>
            </template>
          </el-table-column>
          <el-table-column label="备用模型" width="180">
            <template #default="{ row }">
              <el-select v-model="row.config.fallbackModel" size="small" clearable style="width:100%">
                <el-option v-for="m in availableModels" :key="m.value" :label="m.label" :value="m.value" />
              </el-select>
            </template>
          </el-table-column>
          <el-table-column label="灰度发布" width="160">
            <template #default="{ row }">
              ...
              <template v-else>
                <el-button size="small" @click="row._showGray = true">启用灰度</el-button>
              </template>
            </template>
          </el-table-column>
          <el-table-column label="月度预算(Token)" width="170">
            <template #default="{ row }">
              ...
              <template v-else>
                <el-button size="small" @click="row._showBudget = true">设置预算</el-button>
              </template>
            </template>
          </el-table-column>
          <el-table-column label="操作" width="160" fixed="right">
            <template #default="{ row, $index }">
              <el-button size="small" @click="editScene(row)">详细配置</el-button>
              <el-button size="small" type="danger" @click="removeScene($index)">删除</el-button>
            </template>
          </el-table-column>
        </el-table>
      </el-tab-pane>
      ...
      <!-- 预算看板 -->
      <el-tab-pane label="预算看板" name="budgets">
        <el-table :data="budgetList" stripe size="small">
          <el-table-column label="场景" width="200">
            <template #default="{ row }"><el-tag>{{ row.scene }}</el-tag></template>
          </el-table-column>
          <el-table-column label="已用Token" width="150">
            <template #default="{ row }">{{ formatTokens(row.used) }}</template>
          </el-table-column>
          <el-table-column label="月度上限" width="150">
            <template #default="{ row }">{{ formatTokens(row.limit) }}</template>
          </el-table-column>
    ...
    <!-- 场景详细编辑对话框 -->
    <el-dialog v-model="editDialog" title="场景详细配置" width="600px">
      <el-form :model="editTarget" label-width="110px" v-if="editTarget">
        <el-form-item label="场景名">
          <el-input v-model="editTarget.name" disabled />
        </el-form-item>
        ...
        <el-row :gutter="12">
          <el-col :span="12">
            <el-form-item label="主模型">
              <el-select v-model="editTarget.config.model" style="width:100%">
                <el-option v-for="m in availableModels" :key="m.value" :label="m.label" :value="m.value" />
              </el-select>
            </el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item label="备用模型">
              <el-select v-model="editTarget.config.fallbackModel" clearable style="width:100%">
                <el-option v-for="m in availableModels" :key="m.value" :label="m.label" :value="m.value" />
              </el-select>
            </el-form-item>
          </el-col>
        ...
        <el-row :gutter="12">
          <el-col :span="8">
            <el-form-item label="Temperature">
              <el-input-number v-model="editTarget.config.temperature" :min="0" :max="2" :step="0.1" :precision="1" />
            </el-form-item>
          </el-col>
          <el-col :span="8">
            <el-form-item label="Max Tokens">
              <el-input-number v-model="editTarget.config.maxTokens" :min="256" :max="32768" :step="256" />
            </el-form-item>
          </el-col>
          <el-col :span="8">
            <el-form-item label="Top P">
              <el-input-number v-model="editTarget.config.topP" :min="0" :max="1" :step="0.05" :precision="2" />
            </el-form-item>
          </el-col>
        </el-row>
        ...
        <el-row :gutter="12">
          <el-col :span="12">
            <el-form-item label="灰度模型">
              <el-select v-model="editTarget.config.grayRelease.newModel" clearable style="width:100%">
                <el-option v-for="m in availableModels" :key="m.value" :label="m.label" :value="m.value" />
              </el-select>
            </el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item label="灰度比例(%)">
              <el-input-number v-model="editTarget.config.grayRelease.percentage" :min="0" :max="100" />
            </el-form-item>
          </el-col>
        </el-row>
        ...
        <el-row :gutter="12">
          <el-col :span="12">
            <el-form-item label="月度Token上限">
              <el-input-number v-model="editTarget.config.budgetControl.monthlyTokenLimit" :min="0" :step="100000" style="width:100%" />
              <div style="font-size:11px;color:#909399">0 表示不限制</div>
            </el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item label="超预算降级模型">
              <el-select v-model="editTarget.config.budgetControl.lightModel" style="width:100%">
                <el-option v-for="m in availableModels" :key="m.value" :label="m.label" :value="m.value" />
              </el-select>
            </el-form-item>
          </el-col>
      ...
      <template #footer>
        ...
        <el-button type="primary" @click="confirmSceneEdit">确定</el-button>
      </template>
    </el-dialog>
  </div>
</template>
