  ...
  <div class="page">
    <div class="toolbar">
      ...
      <div style="display:flex;gap:8px">
        <el-select v-model="filterType" placeholder="按类型筛选" clearable style="width:160px" @change="fetchList">
          <el-option v-for="t in ENTITY_TYPES" :key="t.value" :label="t.label" :value="t.value" />
        </el-select>
        <el-button @click="seedDefaults">初始化默认权重</el-button>
      </div>
    </div>

    <el-table v-loading="loading" :data="list" stripe>
      <el-table-column label="实体类型" width="120">
        <template #default="{ row }">{{ ENTITY_LABELS[row.entityType] || row.entityType }}</template>
      </el-table-column>
      <el-table-column label="字段" width="100">
        <template #default="{ row }">{{ row.fieldName === 'all' ? '综合' : row.fieldName }}</template>
      </el-table-column>
      <el-table-column label="权重值" width="200">
        <template #default="{ row }">
          <el-slider v-model="row.weight" :min="0.1" :max="5.0" :step="0.1" show-input :disabled="!row.enabled" style="width:160px" />
        </template>
      </el-table-column>
    ...
    <el-dialog v-model="addVis" title="添加权重" width="400px">
      <el-form :model="addForm" label-width="80px">
        <el-form-item label="实体类型"><el-select v-model="addForm.entityType" style="width:100%"><el-option v-for="t in ENTITY_TYPES" :key="t.value" :label="t.label" :value="t.value" /></el-select></el-form-item>
        <el-form-item label="字段"><el-select v-model="addForm.fieldName" style="width:100%"><el-option label="综合(all)" value="all" /><el-option label="标题(title)" value="title" /><el-option label="摘要(excerpt)" value="excerpt" /><el-option label="简介(intro)" value="intro" /><el-option label="名称(name)" value="name" /><el-option label="昵称(nickname)" value="nickname" /><el-option label="作者(author)" value="author" /></el-select></el-form-item>
        <el-form-item label="权重值"><el-input-number v-model="addForm.weight" :min="0.1" :max="5.0" :step="0.1" :precision="1" style="width:100%" /></el-form-item>
      </el-form>
      <template #footer><el-button @click="addVis = false">取消</el-button><el-button type="primary" @click="doAdd">添加</el-button></template>
    </el-dialog>

    <div style="margin-top:16px"><el-button @click="addVis = true">+ 添加权重</el-button></div>
  </div>
</template>
