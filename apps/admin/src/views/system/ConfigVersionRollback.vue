...
<template>
  <div class="page">
    <div class="toolbar"><h3>配置版本回滚</h3></div>

    <el-card shadow="never" style="margin-bottom:16px">
      <el-form :model="filters" inline>
        <el-form-item label="配置Key">
          <el-input v-model="filters.configKey" placeholder="输入配置Key搜索" clearable style="width:240px" @change="fetchList" />
        </el-form-item>
        <el-form-item>
    ...
    <el-table v-loading="loading" :data="list" stripe empty-text=" ">
      ...
      <el-table-column prop="configKey" label="配置Key" min-width="200" />
      ...
      <el-table-column label="操作" width="180" fixed="right">
        <template #default="{ row }">
          ...
          <el-button size="small" type="warning" @click="rollback(row)">回滚到此版本</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog v-model="detailVis" title="配置值详情" width="640px">
      <el-descriptions v-if="detailRow" :column="1" border>
        <el-descriptions-item label="配置Key">{{ detailRow.configKey }}</el-descriptions-item>
        <el-descriptions-item label="版本号">
          <el-tag size="small" type="primary">v{{ detailRow.version }}</el-tag>
        </el-descriptions-item>
        <el-descriptions-item label="操作人">{{ detailRow.updatedBy || '-' }}</el-descriptions-item>
        <el-descriptions-item label="更新时间">{{ formatDate(detailRow.updatedAt || detailRow.createdAt) }}</el-descriptions-item>
      </el-descriptions>
      <div style="font-size:14px;font-weight:500;margin:12px 0 8px">配置值（JSON 格式化）：</div>
