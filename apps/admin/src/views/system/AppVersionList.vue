  ...
  <div class="page">
    <div class="toolbar">
      ...
      <div style="display:flex;gap:8px">
        <el-select v-model="platformFilter" placeholder="平台" clearable style="width:120px" @change="fetchList">
    ...
    <el-table v-loading="loading" :data="list" stripe empty-text=" ">
      ...
      <el-table-column prop="platform" label="平台" width="100">
        <template #default="{ row }">
          <el-tag :type="row.platform === 'ios' ? 'success' : ''" size="small">{{ row.platform === 'ios' ? 'iOS' : 'Android' }}</el-tag>
        </template>
      </el-table-column>
      <el-table-column prop="version" label="版本号" width="120" />
      <el-table-column prop="buildNumber" label="构建号" width="100" />
      <el-table-column prop="changelog" label="更新日志" min-width="250" show-overflow-tooltip />
      ...
      <el-table-column label="操作" width="160" fixed="right">
        <template #default="{ row }">
          ...
          <el-button type="danger" size="small" @click="del(row.id, row.version)">删除</el-button>
        </template>
      </el-table-column>
    </el-table>

    <el-dialog v-model="vis" :title="editingId ? '编辑版本' : '发布新版本'" width="500px" :close-on-click-modal="false">
      <el-form label-width="90px">
        <el-form-item label="平台">
          <el-select v-model="form.platform" class="w-full" :disabled="!!editingId">
            <el-option label="Android" value="android" />
            <el-option label="iOS" value="ios" />
          </el-select>
        </el-form-item>
        ...
        <el-form-item label="构建号">
          <el-input v-model="form.buildNumber" placeholder="100" />
        </el-form-item>
