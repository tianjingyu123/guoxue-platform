...
<template>
  <div class="page">
    <div class="toolbar"><h3>操作日志检索</h3></div>

    <el-card shadow="never" style="margin-bottom:16px">
      <el-form :model="filters" inline>
        <el-form-item label="操作人">
          <el-input v-model="filters.operatorId" placeholder="用户ID" clearable style="width:160px" @change="fetchList" />
        </el-form-item>
        <el-form-item label="操作类型">
          <el-select v-model="filters.action" placeholder="全部" clearable style="width:160px" @change="fetchList">
        ...
        <el-form-item label="关键词">
          <el-input v-model="filters.keyword" placeholder="搜索详情内容" clearable style="width:200px" @change="fetchList" />
        </el-form-item>
    ...
    <el-table v-loading="loading" :data="list" stripe empty-text=" ">
      ...
      <el-table-column label="操作类型" width="110">
        <template #default="{ row }">
          <el-tag :type="actionTagMap[row.action] || ''" size="small">{{ row.action }}</el-tag>
        </template>
      </el-table-column>
      <el-table-column label="目标对象" min-width="160">
        <template #default="{ row }">{{ row.targetType || '-' }}{{ row.targetId ? '/' + row.targetId : '' }}</template>
      </el-table-column>
    ...
    <el-dialog v-model="detailVis" title="日志详情" width="640px">
      <el-descriptions v-if="detailRow" :column="1" border style="margin-bottom:16px">
        <el-descriptions-item label="操作人">{{ detailRow.operatorId || '-' }}</el-descriptions-item>
        <el-descriptions-item label="操作类型">{{ detailRow.action || '-' }}</el-descriptions-item>
        <el-descriptions-item label="目标对象">{{ detailRow.targetType || '-' }}{{ detailRow.targetId ? '/' + detailRow.targetId : '' }}</el-descriptions-item>
        <el-descriptions-item label="IP地址">{{ detailRow.ip || '-' }}</el-descriptions-item>
        <el-descriptions-item label="操作时间">{{ formatDate(detailRow.createdAt) }}</el-descriptions-item>
      </el-descriptions>
      <div v-if="detailRow" style="font-size:14px;font-weight:500;margin-bottom:8px">详情数据：</div>
