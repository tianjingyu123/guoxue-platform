<template>
  <div class="page">
    <div class="toolbar"><h3>推荐码管理</h3><el-button type="primary" @click="openCreate">创建临时配置</el-button></div>

    <el-row :gutter="16" style="margin-bottom:16px">
      <el-col :span="6"><el-card shadow="hover"><el-statistic title="配置总数" :value="total" /></el-card></el-col>
      <el-col :span="6"><el-card shadow="hover"><el-statistic title="当前生效" :value="activeConfig ? '1' : '0'" /></el-card></el-col>
    </el-row>

    <!-- 当前生效 -->
    <el-card v-if="activeConfig" shadow="never" style="margin-bottom:16px;border-left:3px solid #67c23a">
      <template #header><b>当前生效的临时配置</b></template>
      <el-descriptions :column="3" border>
        <el-descriptions-item label="分站ID">{{ activeConfig.stationId || '全局' }}</el-descriptions-item>
        <el-descriptions-item label="运营商ID">{{ activeConfig.operatorId || '-' }}</el-descriptions-item>
        <el-descriptions-item label="佣金比例">{{ activeConfig.commissionRate }}%</el-descriptions-item>
        <el-descriptions-item label="有效期">{{ formatDate(activeConfig.validFrom) }} ~ {{ formatDate(activeConfig.validTo) }}</el-descriptions-item>
        <el-descriptions-item label="创建时间">{{ formatDate(activeConfig.createdAt) }}</el-descriptions-item>
      </el-descriptions>
    </el-card>
    ...
    <el-table v-loading="loading" :data="list" stripe>
      <el-table-column prop="stationId" label="分站ID" width="180"><template #default="{ row }">{{ row.stationId || '全局' }}</template></el-table-column>
      <el-table-column prop="operatorId" label="运营商ID" width="180"><template #default="{ row }">{{ row.operatorId || '-' }}</template></el-table-column>
      <el-table-column label="佣金比例" width="100"><template #default="{ row }">{{ row.commissionRate }}%</template></el-table-column>
      <el-table-column label="有效期" width="300"><template #default="{ row }">{{ formatDate(row.validFrom) }} ~ {{ formatDate(row.validTo) }}</template></el-table-column>
      ...
      <el-table-column v-if="tab === 'all'" label="操作" width="160" fixed="right">
    ...
    <el-dialog v-model="vis" :title="editingId ? '编辑配置' : '创建配置'" width="500px">
      <el-form :model="form" label-width="90px">
        <el-form-item label="分站ID"><el-input v-model="form.stationId" placeholder="留空则为全局" /></el-form-item>
        <el-form-item label="运营商ID"><el-input v-model="form.operatorId" placeholder="留空则为全局" /></el-form-item>
        <el-form-item label="佣金比例(%)" required><el-input-number v-model="form.commissionRate" :min="0" :max="100" :precision="1" style="width:100%" /></el-form-item>
        <el-row :gutter="16">
          <el-col :span="12"><el-form-item label="开始时间"><el-date-picker v-model="form.validFrom" type="datetime" value-format="YYYY-MM-DD HH:mm:ss" style="width:100%" /></el-form-item></el-col>
          <el-col :span="12"><el-form-item label="结束时间"><el-date-picker v-model="form.validTo" type="datetime" value-format="YYYY-MM-DD HH:mm:ss" style="width:100%" /></el-form-item></el-col>
        </el-row>
      </el-form>
