<template>
  <div class="page">
    ...
    <el-table v-loading="loading" :data="list" border stripe>
      ...
      <el-table-column prop="luckyDirection" label="幸运方向" width="100" />
      <el-table-column prop="luckyColor" label="幸运颜色" width="100">
        <template #default="{ row }">
          <span :style="{ color: row.luckyColor }">{{ row.luckyColor }}</span>
        </template>
      </el-table-column>
      <el-table-column prop="sentStatus" label="发送状态" width="110">
        <template #default="{ row }">
          <el-tag :type="sentStatusTag(row.sentStatus)">{{ sentStatusLabel(row.sentStatus) }}</el-tag>
        </template>
      </el-table-column>
