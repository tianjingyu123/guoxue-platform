<template>
  <div class="page">
    ...
    >
      ...
      <template #roles="{ row }">
        <el-tag v-for="r in row.roles" :key="r.roleType" size="small" style="margin-right:4px" :type="roleTagType(r.roleType)">
    ...
    <el-dialog v-model="detailVisible" title="用户详情" width="480px">
      <div v-if="detail" class="detail-info">
        ...
        <p>
          <b>角色：</b>
          <el-tag v-for="r in detail.roles" :key="r.roleType" size="small" style="margin-right:4px">
            {{ roleLabel(r.roleType) }}
          </el-tag>
        </p>
...
const columns = [
  ...
  { prop: "memberLevel", label: "会员", width: 90, slot: "memberLevel" },
  { prop: "status", label: "状态", width: 80, slot: "status" },
  { prop: "createdAt", label: "注册时间", width: 170, slot: "createdAt" },
];
...
async function viewDetail(row: any) {
  try { const { data } = await userApi.detail(row.id); detail.value = data; detailVisible.value = true; } catch { /* */ }
}
...
function exportData() {
  exportCSV(
    ...
    [
      ...
      { label: "状态", key: "status" }, { label: "注册时间", key: "createdAt" },
    ],
    list.value.map((u) => ({
      ...u,
