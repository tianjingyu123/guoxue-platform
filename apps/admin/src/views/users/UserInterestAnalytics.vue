<template>
  <div class="interest-page">
    ...
    <el-row :gutter="16" style="margin-bottom:16px">
      <el-col :span="6">
        <div class="stat-card"><span class="value">{{ stats.totalUsers?.toLocaleString() || 0 }}</span><span class="label">活跃用户总数</span></div>
      </el-col>
      <el-col :span="6">
        <div class="stat-card info"><span class="value">{{ stats.usersWithInterests?.toLocaleString() || 0 }}</span><span class="label">有兴趣标签用户</span></div>
      </el-col>
      <el-col :span="6">
        <div class="stat-card"><span class="value">{{ stats.coverageRate || 0 }}%</span><span class="label">兴趣覆盖率</span></div>
      </el-col>
      <el-col :span="6">
        <div class="stat-card warn"><span class="value">{{ stats.distribution?.length || 0 }}</span><span class="label">涉及品类数</span></div>
      </el-col>
    </el-row>
    ...
    <el-row :gutter="16">
      ...
      <el-col :span="14">
        <el-card>
          <template #header><span>兴趣品类分布</span></template>
          <div v-if="stats.distribution?.length" ref="pieChartRef" style="height:380px" />
      ...
      <el-col :span="10">
        <el-card>
          <template #header><span>品类排行</span></template>
          <el-table :data="stats.distribution || []" stripe size="small" max-height="380">
            <el-table-column type="index" label="#" width="45" />
            <el-table-column prop="name" label="品类" min-width="100" />
            <el-table-column label="用户数" width="80" align="center">
              <template #default="{ row }">{{ row.count?.toLocaleString() }}</template>
            </el-table-column>
            <el-table-column label="占比" width="75" align="center">
    ...
    <!-- 行为兴趣标签 -->
    <el-card v-if="stats.behaviorTags?.length" style="margin-top:16px">
      <template #header><span>行为兴趣标签 TOP30</span></template>
      <el-table :data="stats.behaviorTags" stripe size="small" max-height="350">
        <el-table-column type="index" label="#" width="45" />
        <el-table-column prop="tag" label="标签" min-width="160" />
        <el-table-column label="用户数" width="100" align="center">
          <template #default="{ row }">{{ row.userCount?.toLocaleString() }}</template>
        </el-table-column>
        <el-table-column label="平均兴趣分" width="110" align="center">
          <template #default="{ row }">
            <el-progress :percentage="Math.round((row.avgScore || 0) * 100)" :stroke-width="14"
    ...
    <el-card style="margin-top:16px">
      <template #header><span>运营建议</span></template>
      <el-row :gutter="16">
        <el-col :span="8">
          <div class="tip-card">
            <div class="tip-title">📊 品类覆盖</div>
            <div class="tip-body">
