  ...
  <div class="page">
    <div class="toolbar">
      ...
      <div class="toolbar-right">
        ...
        <el-button type="primary" @click="openCreate">新建电子书</el-button>
      </div>
    </div>
    ...
    <el-table v-loading="loading" :data="list" stripe>
      ...
      <el-table-column prop="totalChapters" label="章节数" width="80" />
      <el-table-column prop="purchaseCount" label="购买数" width="80" />
      ...
      <el-table-column label="操作" width="200" fixed="right">
        <template #default="{ row }">
          <el-button size="small" text type="primary" @click="openChapters(row)">章节</el-button>
    ...
    <!-- 电子书对话框 -->
    <el-dialog v-model="dialogVisible" :title="editingId ? '编辑电子书' : '新建电子书'" width="600px">
      <el-form :model="form" label-width="80px">
        ...
        <el-form-item label="文件类型">
          <el-select v-model="form.fileType" style="width:100%">
            <el-option label="PDF" value="PDF" /><el-option label="EPUB" value="EPUB" />
          </el-select>
        </el-form-item>
        ...
        <el-form-item label="状态">
          <el-select v-model="form.status" style="width:100%">
            <el-option label="草稿" value="DRAFT" /><el-option label="已发布" value="PUBLISHED" /><el-option label="已禁用" value="DISABLED" />
          </el-select>
        </el-form-item>
      </el-form>
      <template #footer>
    ...
    <!-- 章节管理对话框 -->
    <el-dialog v-model="chapterDialog" :title="`章节管理 — ${chapterBook?.title || ''}`" width="700px">
      <div style="margin-bottom:12px">
        <el-button type="primary" size="small" @click="openChapterCreate">添加章节</el-button>
      </div>
      <el-table :data="chapterList" stripe max-height="400">
        <el-table-column prop="title" label="章节标题" min-width="150" />
        <el-table-column label="页码" width="120">
          <template #default="{ row }">{{ row.pageStart }} — {{ row.pageEnd }}</template>
        </el-table-column>
        <el-table-column prop="sortOrder" label="排序" width="70" />
        <el-table-column label="试读" width="70">
          <template #default="{ row }">{{ row.freeTrial ? '是' : '否' }}</template>
        </el-table-column>
        <el-table-column label="操作" width="120">
          <template #default="{ row }">
            <el-button size="small" text type="primary" @click="openChapterEdit(row)">编辑</el-button>
            <el-button size="small" text type="danger" @click="delChapter(row.id)">删除</el-button>
          </template>
        </el-table-column>
      </el-table>
      <template #footer>
        <el-button @click="chapterDialog = false">关闭</el-button>
      </template>
    </el-dialog>

    <!-- 章节表单对话框 -->
    <el-dialog v-model="chapterFormDialog" :title="editingChapterId ? '编辑章节' : '新建章节'" width="500px">
      <el-form :model="chapterForm" label-width="80px">
        <el-form-item label="标题" required><el-input v-model="chapterForm.title" /></el-form-item>
        <el-form-item label="内容"><el-input v-model="chapterForm.content" type="textarea" :rows="5" /></el-form-item>
        <el-form-item label="起始页"><el-input-number v-model="chapterForm.pageStart" :min="0" style="width:100%" /></el-form-item>
        <el-form-item label="结束页"><el-input-number v-model="chapterForm.pageEnd" :min="0" style="width:100%" /></el-form-item>
        <el-form-item label="排序"><el-input-number v-model="chapterForm.sortOrder" :min="0" style="width:100%" /></el-form-item>
        <el-form-item label="免费试读"><el-switch v-model="chapterForm.freeTrial" /></el-form-item>
      </el-form>
      <template #footer>
