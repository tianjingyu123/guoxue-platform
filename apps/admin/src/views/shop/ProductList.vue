<template>
  <div class="product-page">
    ...
    >
      ...
      <template #originalPrice="{ row }">¥{{ Number(row.originalPrice).toFixed(2) }}</template>
      <template #status="{ row }">
      ...
      <template #actions="{ row }">
        ...
        <el-button v-if="row.status !== 'ON_SALE'" size="small" type="success" @click="toggleStatus(row)">上架</el-button>
        <el-button v-else size="small" type="warning" @click="toggleStatus(row)">下架</el-button>
    ...
    <el-dialog v-model="dialogVisible" :title="editingId ? '编辑商品' : '添加商品'" width="700px" top="5vh">
      <el-form ref="dialogFormRef" :model="form" :rules="dialogRules" label-width="80px">
        <el-form-item label="商品名称" required><el-input v-model="form.title" placeholder="商品名称" /></el-form-item>
        <el-form-item label="简介"><el-input v-model="form.intro" type="textarea" :rows="2" placeholder="简要描述" /></el-form-item>
        <el-form-item label="详情"><div ref="detailEditorEl" class="editor-box" /></el-form-item>
        <el-row :gutter="16">
          ...
          <el-col :span="8"><el-form-item label="状态"><el-select v-model="form.status" style="width:100%"><el-option label="在售" value="ON_SALE" /><el-option label="待审" value="PENDING" /><el-option label="下架" value="OFF_SHELF" /></el-select></el-form-item></el-col>
        </el-row>
        <el-row :gutter="16">
          ...
          <el-col :span="8"><el-form-item label="分类"><el-input v-model="form.category" placeholder="商品分类" /></el-form-item></el-col>
          <el-col :span="8"><el-form-item label="封面"><ImageUpload v-model="form.cover" /></el-form-item></el-col>
        </el-row>
        <el-form-item label="图片">
          <div class="images-area">
            <div v-for="(img, i) in form.images" :key="i" class="img-item">
              <img :src="img"><el-button size="small" type="danger" circle @click="form.images.splice(i,1)">✕</el-button>
            </div>
            <div class="img-add">
    ...
    <el-dialog v-model="skuVisible" title="SKU 管理" width="600px">
      <div v-if="skuProduct">
        ...
        <el-table :data="skus" border size="small" max-height="300">
          <el-table-column label="规格" min-width="160"><template #default="{ row }">{{ specsText(row.specs) }}</template></el-table-column>
          <el-table-column label="价格" width="100"><template #default="{ row }">¥{{ Number(row.price).toFixed(2) }}</template></el-table-column>
          <el-table-column prop="stock" label="库存" width="70" />
          <el-table-column prop="skuCode" label="编码" width="100" />
          <el-table-column label="操作" width="80"><template #default="{ row }"><el-button size="small" type="danger" @click="delSku(row.id)">删除</el-button></template></el-table-column>
        </el-table>
        <el-divider />
        <el-form :model="skuForm" inline>
          <el-form-item label="规格名"><el-input v-model="skuKey" placeholder="如 颜色" size="small" style="width:100px" /></el-form-item>
          <el-form-item label="规格值"><el-input v-model="skuVal" placeholder="如 红色" size="small" style="width:100px" /></el-form-item>
          <el-form-item label="价格"><el-input-number v-model="skuForm.price" :min="0" :precision="2" size="small" style="width:120px" /></el-form-item>
          <el-form-item label="库存"><el-input-number v-model="skuForm.stock" :min="0" size="small" style="width:80px" /></el-form-item>
          <el-form-item><el-button size="small" type="primary" @click="addSkuAction">添加SKU</el-button></el-form-item>
        </el-form>
      </div>
    </el-dialog>
  </div>
