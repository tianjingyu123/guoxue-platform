  ...
  <div class="page">
    ...
    <el-table v-loading="loading" :data="list" stripe>
      ...
      <el-table-column label="状态" width="90">
        <template #default="{ row }">
          <el-tag :type="row.startAt && new Date(row.startAt) > new Date() ? 'warning' : (row.endAt && new Date(row.endAt) < new Date() ? 'info' : 'success')" size="small">
    ...
    <el-dialog v-model="dialogVisible" :title="editingId ? '编辑规则' : '新建规则'" width="600px">
      <el-form :model="form" label-width="90px">
        <el-form-item label="场景" required><el-input v-model="form.scene" placeholder="如 course_detail, ALL" /></el-form-item>
        <el-form-item label="规则类型" required>
          <el-select v-model="form.ruleType" style="width:100%">
            <el-option label="固定推荐(PIN)" value="PIN" />
            <el-option label="加权(BOOST)" value="BOOST" />
            <el-option label="降权(MUTE)" value="MUTE" />
            <el-option label="屏蔽(BAN)" value="BAN" />
            <el-option label="强插(INSERT)" value="INSERT" />
          </el-select>
        </el-form-item>
        <el-form-item label="目标类型" required>
          <el-select v-model="form.targetType" style="width:100%">
            <el-option label="课程" value="COURSE" /><el-option label="商品" value="PRODUCT" />
            <el-option label="文章" value="ARTICLE" /><el-option label="圈子" value="CIRCLE" />
            <el-option label="视频" value="VIDEO" />
          </el-select>
        </el-form-item>
        <el-form-item label="目标ID" required><el-input v-model="form.targetId" /></el-form-item>
        <el-form-item label="权重值"><el-input-number v-model="form.ruleValue" :precision="2" style="width:100%" /></el-form-item>
        <el-form-item label="强插位置"><el-input-number v-model="form.position" :min="0" style="width:100%" /></el-form-item>
        <el-form-item label="优先级"><el-input-number v-model="form.priority" :min="0" style="width:100%" /></el-form-item>
        <el-form-item label="开始时间"><el-date-picker v-model="form.startAt" type="datetime" style="width:100%" /></el-form-item>
        <el-form-item label="结束时间"><el-date-picker v-model="form.endAt" type="datetime" style="width:100%" /></el-form-item>
