<template>
  <div class="cat-page">
    <el-page-header title="返回" style="margin-bottom:16px" @back="$router.push('/courses')">
      <template #content>课程分类管理</template>
    </el-page-header>

    <el-row :gutter="16" style="margin-bottom:16px">
      <el-col :span="6">
        <div class="stat-card"><span class="value">{{ Object.keys(categoryTree).length }}</span><span class="label">一级分类</span></div>
      </el-col>
      <el-col :span="6">
        <div class="stat-card"><span class="value">{{ totalSub }}</span><span class="label">二级分类总数</span></div>
      </el-col>
      <el-col :span="12" style="display:flex;align-items:center;justify-content:flex-end;gap:8px">
        <el-button type="primary" @click="openEditTree">编辑分类树</el-button>
    ...
    <el-card v-loading="loading">
      <template #header><span>课程品类树</span></template>
      <div v-if="Object.keys(categoryTree).length === 0" style="text-align:center;padding:40px;color:#999">
        暂无分类数据，请点击"编辑分类树"添加
      </div>
      <div v-else class="tree-preview">
        <div v-for="(subs, level1) in categoryTree" :key="level1" class="level1-group">
          <div class="level1-name">
            <el-tag type="danger" size="large">{{ level1 }}</el-tag>
            <span class="level1-count">{{ subs.length }} 个子类</span>
          </div>
          <div class="level2-list">
            <el-tag v-for="(sub, idx) in subs" :key="idx" size="small" type="info" style="margin:2px">{{ sub }}</el-tag>
          </div>
        </div>
      </div>
    </el-card>
    ...
    <!-- 编辑对话框 -->
    <el-dialog v-model="showEditTree" title="编辑课程品类树" width="750px" top="5vh">
      <div v-for="(subs, level1) in editTree" :key="level1" style="margin-bottom:14px;padding:14px;background:#faf8f5;border-radius:8px;border:1px solid #E8E0D5">
        <div style="display:flex;align-items:center;gap:8px;margin-bottom:10px">
          <el-input v-model="editLevel1Names[level1]" size="small" style="width:160px;font-weight:600" placeholder="一级分类名" />
          <el-button size="small" type="danger" circle @click="deleteLevel1(level1)">✕</el-button>
        </div>
        <div style="display:flex;flex-wrap:wrap;gap:6px;align-items:center">
          <el-tag v-for="(sub, idx) in subs" :key="idx" closable size="small" @close="removeSub(level1, idx)">
            <el-input v-model="editTree[level1][idx]" size="small" style="width:110px;border:none;padding:0" placeholder="二级分类" />
          </el-tag>
          <el-button size="small" type="primary" circle @click="addSub(level1)">+</el-button>
        </div>
      </div>
      <div style="margin-top:12px">
        <el-button size="small" @click="addLevel1">+ 添加一级分类</el-button>
        <el-button size="small" type="warning" style="margin-left:8px" @click="resetToDefault">恢复默认</el-button>
      </div>
      <template #footer>
        <el-button @click="showEditTree = false">取消</el-button>
        <el-button type="primary" :loading="savingTree" @click="saveTree">保存分类树</el-button>
      </template>
    </el-dialog>
  </div>
</template>
