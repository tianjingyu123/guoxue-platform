<template>
  <div class="course-edit">
    <el-page-header title="返回" style="margin-bottom:16px" @back="router.push('/courses')">
    ...
    <el-tabs v-model="activeTab" type="border-card">
      ...
      <el-tab-pane label="基本信息" name="basic">
        <div class="edit-body">
          <div class="edit-main">
            <el-card>
              <el-form ref="mainFormRef" :model="form" :rules="mainRules" label-width="90px">
                ...
                <el-row :gutter="16">
                  <el-col :span="8">
                    <el-form-item label="一级分类">
                      <el-select v-model="form.categoryLevel1" placeholder="选择一级分类" clearable style="width:100%" @change="onLevel1Change">
                        <el-option v-for="cat in categoryLevel1List" :key="cat" :label="cat" :value="cat" />
                      </el-select>
                    </el-form-item>
                  </el-col>
                  <el-col :span="8">
                    <el-form-item label="二级分类">
                      <el-select v-model="form.categoryLevel2" placeholder="选择二级分类" clearable style="width:100%">
                        <el-option v-for="cat in categoryLevel2Options" :key="cat" :label="cat" :value="cat" />
                      </el-select>
                    </el-form-item>
                  </el-col>
                  <el-col :span="8">
                    <el-form-item label="有效期(天)">
                      <el-input-number v-model="form.validityDays" :min="0" placeholder="0=永久" style="width:100%" />
                ...
                <el-row :gutter="16">
                  <el-col :span="8">
                    <el-form-item label="所属圈子">
                      <el-input v-model="form.circleId" placeholder="圈子ID" clearable />
                    </el-form-item>
                  </el-col>
                  <el-col :span="8">
                    <el-form-item label="所属分站">
                      <el-input v-model="form.stationId" placeholder="分站ID（空=平台级）" clearable />
                    </el-form-item>
                  </el-col>
                  <el-col :span="8">
                    <el-form-item label="定时发布">
                      <el-date-picker v-model="form.scheduledAt" type="datetime" placeholder="留空=立即发布" style="width:100%" value-format="YYYY-MM-DDTHH:mm:ss.sssZ" />
                    </el-form-item>
                  </el-col>
                </el-row>
                <el-form-item label="标签">
                  <el-tag v-for="(tag, idx) in form.tags" :key="idx" closable style="margin-right:6px" @close="form.tags.splice(idx, 1)">{{ tag }}</el-tag>
                  <el-input
          ...
          <div v-if="isEdit" class="edit-sidebar">
            <div class="sidebar-section">
              <h4>封面图片</h4>
              <div class="cover-upload">
                <div v-if="form.cover" class="cover-preview">
                  <img :src="form.cover" alt="封面">
                  <el-button size="small" type="danger" class="cover-remove" @click="form.cover = ''">移除</el-button>
                </div>
                <div v-else class="cover-placeholder"><span>暂无封面</span></div>
                <div class="cover-input-row">
                  <el-input v-model="coverUrl" placeholder="图片URL" size="small" />
                  <el-button size="small" @click="form.cover = coverUrl">设置</el-button>
                </div>
                <el-upload :show-file-list="false" :http-request="handleCoverUpload" accept="image/*" style="margin-top:8px">
            ...
            <div v-if="isEdit" class="sidebar-section">
              <h4>快速操作</h4>
              <el-select v-model="quickStatus" placeholder="变更状态" size="small" style="width:100%;margin-bottom:8px" @change="changeStatus">
                <el-option label="提交审核" value="PENDING" />
                <el-option label="直接上架" value="APPROVED" />
                <el-option label="驳回" value="REJECTED" />
                <el-option label="下架为草稿" value="DRAFT" />
              </el-select>
            </div>
          </div>
        </div>
      ...
      <!-- Tab 3: 学员管理 -->
      <el-tab-pane v-if="isEdit" label="学员管理" name="students">
        <div class="section-header">
          <span>共 {{ studentTotal }} 名学员</span>
          <el-button size="small" @click="fetchStudents">刷新</el-button>
        </div>
        <el-table :data="students" v-loading="studentLoading" stripe style="margin-top:12px">
          <el-table-column label="学员" min-width="140">
            <template #default="{ row }">
              <div style="display:flex;align-items:center;gap:8px">
                <el-avatar v-if="row.user?.avatar" :src="row.user.avatar" :size="32" />
          ...
          <el-table-column label="已完成章节" width="110">
            <template #default="{ row }">{{ row.progress?.completedChapters || 0 }}</template>
          </el-table-column>
          <el-table-column label="总进度" width="90">
            <template #default="{ row }">
              <el-progress :percentage="row.progress?.totalProgress || 0" :stroke-width="6" />
            </template>
          </el-table-column>
      ...
      <!-- Tab 4: 作业批改 -->
      <el-tab-pane v-if="isEdit" label="作业批改" name="works">
        <div class="section-header">
          <span>共 {{ works.length }} 份作业</span>
          <div style="display:flex;gap:8px">
            <el-button size="small" type="success" :loading="aiBatchLoading" @click="handleAiBatchScore">AI 批量批改</el-button>
            <el-button size="small" @click="fetchWorks">刷新</el-button>
          </div>
        </div>
        <el-table :data="works" stripe style="margin-top:12px">
          <el-table-column label="学员" width="100">
            <template #default="{ row }">{{ row.userId?.slice(0, 8) || '-' }}</template>
          </el-table-column>
          ...
          <el-table-column label="评分" width="90">
            <template #default="{ row }">{{ row.score != null ? row.score + '分' : '未评分' }}</template>
          </el-table-column>
          <el-table-column label="操作" width="200">
            <template #default="{ row }">
              <el-button size="small" @click="openScoreDialog(row)">{{ row.score != null ? '重新评分' : '评分' }}</el-button>
              <el-button size="small" type="success" :loading="aiScoreLoading === row.id" @click="handleAiScore(row)">AI 批改</el-button>
            </template>
          </el-table-column>
        </el-table>
      </el-tab-pane>
      ...
      <!-- Tab 5: 评价管理 -->
      <el-tab-pane v-if="isEdit" label="评价管理" name="reviews">
        <div class="section-header">
          <span>共 {{ reviewTotal }} 条评价</span>
          <el-select v-model="reviewFilter" placeholder="状态筛选" size="small" clearable style="width:120px" @change="fetchReviews">
            <el-option label="已发布" value="PUBLISHED" />
            <el-option label="已隐藏" value="HIDDEN" />
          </el-select>
        </div>
        <el-table :data="reviews" v-loading="reviewLoading" stripe style="margin-top:12px">
          ...
          <el-table-column label="操作" width="200">
            <template #default="{ row }">
              ...
              <el-button size="small" :type="row.status === 'HIDDEN' ? 'success' : 'warning'" @click="toggleReview(row)">
      ...
      <!-- Tab 6: 问答管理 -->
      <el-tab-pane v-if="isEdit" label="问答管理" name="qas">
        <div class="section-header">
          <span>共 {{ qaTotal }} 条问答</span>
          <div style="display:flex;gap:8px">
            <el-select v-model="qaFilter.status" placeholder="状态" size="small" clearable style="width:110px" @change="fetchQas">
              <el-option label="待回答" value="PENDING" />
              <el-option label="已回答" value="ANSWERED" />
              <el-option label="已关闭" value="CLOSED" />
            </el-select>
            <el-select v-if="qaTags.length > 0" v-model="qaFilter.tag" placeholder="标签" size="small" clearable style="width:110px" @change="fetchQas">
              <el-option v-for="t in qaTags" :key="t" :label="t" :value="t" />
            </el-select>
            <el-button size="small" @click="fetchQas">刷新</el-button>
          </div>
        </div>
        <el-table :data="qas" v-loading="qaLoading" stripe style="margin-top:12px">
          <el-table-column label="提问者" width="110">
            <template #default="{ row }">{{ row.user?.nickname || row.userId?.slice(0, 8) || '-' }}</template>
          </el-table-column>
          ...
          <el-table-column label="状态" width="80">
            <template #default="{ row }">
              <el-tag :type="row.status === 'ANSWERED' ? 'success' : row.status === 'CLOSED' ? 'info' : 'warning'" size="small">
          ...
          <el-table-column label="操作" width="200">
            <template #default="{ row }">
              <el-button v-if="row.status === 'PENDING'" size="small" type="primary" @click="openQaAnswerDialog(row)">回答</el-button>
              <el-button v-if="row.status === 'ANSWERED'" size="small" @click="openQaAnswerDialog(row)">编辑</el-button>
              <el-button v-if="row.status === 'PENDING' || row.status === 'ANSWERED'" size="small" type="success" :loading="aiSuggestLoading === row.id" @click="handleAiSuggest(row)">AI辅助</el-button>
              <el-button v-if="row.status !== 'CLOSED'" size="small" type="warning" @click="handleCloseQa(row)">关闭</el-button>
            </template>
          </el-table-column>
        </el-table>
        <el-pagination
      ...
      <!-- Tab 7: 数据统计 -->
      <el-tab-pane v-if="isEdit" label="数据统计" name="stats">
        <div v-loading="statsLoading" style="min-height:200px">
          <el-row v-if="stats" :gutter="16">
            <el-col :span="6"><div class="stat-card"><div class="stat-label">报名人数</div><div class="stat-value">{{ stats.enrollmentCount }}</div></div></el-col>
            <el-col :span="6"><div class="stat-card"><div class="stat-label">完成人数</div><div class="stat-value">{{ stats.completedCount }}</div></div></el-col>
            <el-col :span="6"><div class="stat-card"><div class="stat-label">平均评分</div><div class="stat-value">{{ stats.avgRating }} ⭐</div></div></el-col>
            <el-col :span="6"><div class="stat-card"><div class="stat-label">作业总数</div><div class="stat-value">{{ stats.totalWorks }}</div></div></el-col>
          </el-row>
          <el-empty v-if="!stats && !statsLoading" description="暂无数据" />
        </div>
      </el-tab-pane>
    </el-tabs>
    ...
    <el-dialog v-model="chapterDialog" :title="editingChapter ? '编辑章节' : '添加章节'" width="700px" top="5vh">
      <el-form :model="chapterForm" label-width="80px">
        <el-form-item label="标题" required><el-input v-model="chapterForm.title" placeholder="章节标题" /></el-form-item>
        <el-form-item label="媒体URL"><el-input v-model="chapterForm.mediaUrl" placeholder="视频/音频URL" /></el-form-item>
        <el-form-item label="正文">
          <div ref="chapterEditorEl" class="chapter-editor-box" />
        </el-form-item>
        <el-row :gutter="12">
          <el-col :span="8"><el-form-item label="时长(秒)"><el-input-number v-model="chapterForm.duration" :min="0" /></el-form-item></el-col>
          <el-col :span="8"><el-form-item label="排序"><el-input-number v-model="chapterForm.sortOrder" :min="0" /></el-form-item></el-col>
          <el-col :span="8"><el-form-item label="免费试看"><el-switch v-model="chapterForm.freeTrial" /></el-form-item></el-col>
        </el-row>
      </el-form>
      <template #footer>
    ...
    <!-- 作业评分弹窗 -->
    <el-dialog v-model="scoreDialog" title="作业评分" width="500px">
      <el-form label-width="80px">
        <el-form-item label="作业内容"><div style="max-height:200px;overflow-y:auto;background:#f5f5f5;padding:12px;border-radius:4px">{{ scoringWork?.content }}</div></el-form-item>
        <el-form-item label="评分"><el-input-number v-model="scoreForm.score" :min="0" :max="100" /></el-form-item>
        <el-form-item label="点评"><el-input v-model="scoreForm.feedback" type="textarea" :rows="3" placeholder="可选点评" /></el-form-item>
      </el-form>
      <template #footer>
    ...
    <el-dialog v-model="replyDialog" title="回复评价" width="500px">
      <el-form label-width="80px">
        <el-form-item label="用户评价"><div style="color:#666">{{ replyingReview?.content }}</div></el-form-item>
        <el-form-item label="回复内容"><el-input v-model="replyContent" type="textarea" :rows="3" placeholder="输入回复内容" /></el-form-item>
      </el-form>
      <template #footer>
    ...
    <!-- 问答回答弹窗 -->
    <el-dialog v-model="qaAnswerDialog" title="回答问题" width="550px">
      <el-form label-width="80px">
        <el-form-item label="学生提问"><div style="background:#f5f5f5;padding:12px;border-radius:4px;max-height:150px;overflow-y:auto">{{ answeringQa?.question }}</div></el-form-item>
        <el-form-item label="现有回答" v-if="answeringQa?.answer"><div style="color:#666">{{ answeringQa.answer }}</div></el-form-item>
        <el-form-item label="回答内容"><el-input v-model="qaAnswerContent" type="textarea" :rows="4" placeholder="输入回答内容" /></el-form-item>
      </el-form>
      <template #footer>
        <el-button @click="qaAnswerDialog = false">取消</el-button>
        <el-button type="primary" @click="submitQaAnswer">提交回答</el-button>
      </template>
    </el-dialog>

    <!-- 学员详情弹窗 -->
    <el-dialog v-model="studentDetailDialog" title="学员学习详情" width="700px">
      <div v-if="studentDetail" v-loading="studentDetailLoading">
        <h4 style="margin-bottom:8px">章节进度</h4>
        <el-table :data="studentDetail.progresses || []" stripe size="small">
          <el-table-column label="章节" prop="chapter.title" />
          <el-table-column label="进度" width="100">
            <template #default="{ row }">{{ row.progress }}%</template>
          </el-table-column>
          <el-table-column label="完成" width="80">
            <template #default="{ row }"><el-tag :type="row.completed ? 'success' : 'info'" size="small">{{ row.completed ? '是' : '否' }}</el-tag></template>
          </el-table-column>
          <el-table-column label="更新时间" width="160">
            <template #default="{ row }">{{ row.updatedAt ? new Date(row.updatedAt).toLocaleString() : '-' }}</template>
          </el-table-column>
        </el-table>
        <h4 v-if="(studentDetail.works || []).length > 0" style="margin:16px 0 8px">作业记录</h4>
        <el-table v-if="(studentDetail.works || []).length > 0" :data="studentDetail.works" stripe size="small">
          <el-table-column label="章节" prop="chapter.title" />
