  ...
  <div class="page">
    ...
    <el-table v-loading="loading" :data="list" stripe>
      ...
      <el-table-column label="操作" width="280" fixed="right">
        <template #default="{ row }">
          ...
          <el-button v-if="row.status !== 'PUBLISHED'" size="small" type="success" @click="publishPage(row)">发布</el-button>
    ...
    <el-dialog v-model="compVis" title="页面组件" width="650px">
      ...
      <el-table :data="components" stripe max-height="400">
        ...
        <el-table-column label="配置" min-width="140"><template #default="{ row }">{{ JSON.stringify(row.config).substring(0, 40) }}</template></el-table-column>
    ...
    <el-dialog v-model="compFormVis" :title="compEditingId ? '编辑组件' : '添加组件'" width="500px">
      ...
      <template #footer><el-button @click="compFormVis = false">取消</el-button><el-button type="primary" @click="saveComp">保存</el-button></template>
    </el-dialog>
  </div>
</template>
...
const compVis = ref(false); const compFormVis = ref(false); const compEditingId = ref('')
const components = ref<any[]>([]); const currentPageId = ref('')
const compForm = reactive({ type: 'CAROUSEL', title: '', configStr: '{}' })

const BASE = '/marketing/pages'
...
async function fetchList() {
  loading.value = true
  try { const { data } = await api.get(BASE, { params: { page: page.value, pageSize: 20 } }); list.value = data.pages || data.data || []; total.value = data.total || 0 } catch { list.value = [] } finally { loading.value = false }
}
...
async function save() {
  saving.value = true
  try { if (editingId.value) { await api.put(`${BASE}/${editingId.value}`, form) } else { await api.post(BASE, form) }; ElMessage.success('已保存'); vis.value = false; fetchList() } catch { ElMessage.error('保存失败') } finally { saving.value = false }
}

async function publishPage(row: any) {
  try { await api.post(`${BASE}/${row.id}/publish`); ElMessage.success('已发布'); fetchList() } catch { ElMessage.error('发布失败') }
}

async function del(id: string) {
  try { await ElMessageBox.confirm('确定删除？', '提示', { type: 'warning' }); await api.delete(`${BASE}/${id}`); ElMessage.success('已删除'); fetchList() } catch {}
}

// 组件管理
async function openComponents(row: any) {
  currentPageId.value = row.id; components.value = row.components || []
  compVis.value = true
}

function openCompCreate() { compEditingId.value = ''; Object.assign(compForm, { type: 'CAROUSEL', title: '', configStr: '{}' }); compFormVis.value = true }
function openCompEdit(row: any) { compEditingId.value = row.id; Object.assign(compForm, { type: row.type, title: row.title || '', configStr: JSON.stringify(row.config || {}) }); compFormVis.value = true }
...
async function saveComp() {
  ...
  try {
    if (compEditingId.value) {
      await api.put(`${BASE}/${currentPageId.value}/components/${compEditingId.value}`, payload)
    } else {
      await api.post(`${BASE}/${currentPageId.value}/components`, payload)
    }
    ElMessage.success('已保存'); compFormVis.value = false
    const { data } = await api.get(`${BASE}/${currentPageId.value}`)
    components.value = data.components || []
  } catch { ElMessage.error('保存失败') }
}

async function delComp(compId: string) {
  try { await ElMessageBox.confirm('删除该组件？', '提示', { type: 'warning' }); await api.delete(`${BASE}/${currentPageId.value}/components/${compId}`); ElMessage.success('已删除'); const { data } = await api.get(`${BASE}/${currentPageId.value}`); components.value = data.components || [] } catch {}
}
</script>
