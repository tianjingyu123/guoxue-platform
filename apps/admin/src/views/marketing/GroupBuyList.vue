<template>
  <div class="page">
    ...
    >
      ...
      >
        <template #default="{ row }">
          ¥{{ Number(row.groupPrice).toFixed(2) }}
        </template>
      </el-table-column>
      <el-table-column
        label="有效期"
    ...
    >
      ...
      >
        ...
        <el-form-item label="有效时长(分)">
          <el-input-number
            v-model="form.expireMinutes"
            :min="10"
            :max="43200"
            style="width:100%"
          />
      ...
      >
        ...
        >
          <template #default="{ row }">
            <el-tag
              size="small"
              :type="row.groupStatus === 'COMPLETED' ? 'success' : 'warning'"
            >
              {{ row.groupStatus === 'COMPLETED' ? '已成团' : '拼团中' }}
            </el-tag>
          </template>
        </el-table-column>
        <el-table-column
