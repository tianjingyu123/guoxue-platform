...
const form = reactive({ name: '', discountRate: 0.85, startTime: '', endTime: '' })

const BASE = '/marketing/discounts'

onMounted(() => fetchList())
function formatDate(d: string) { return d ? new Date(d).toLocaleString() : '-' }
async function fetchList() { loading.value = true; try { const { data } = await api.get(BASE, { params: { page: page.value, pageSize: 20 } }); list.value = data.discounts || data.data || []; total.value = data.total || 0 } catch { list.value = [] } finally { loading.value = false } }
function openCreate() { editingId.value = ''; Object.assign(form, { name: '', discountRate: 0.85, startTime: '', endTime: '' }); vis.value = true }
function openEdit(row: any) { editingId.value = row.id; Object.assign(form, { name: row.name, discountRate: Number(row.discountRate), startTime: row.startTime || '', endTime: row.endTime || '' }); vis.value = true }
