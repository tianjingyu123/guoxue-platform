  ...
  <div class="page">
    <div class="header">
      ...
      <div class="search-row">
        ...
        <el-select v-model="roleFilter" placeholder="角色筛选" clearable style="width:140px" @change="fetchList">
          ...
          <el-option label="潜力讲师" value="TYPE_A" />
          <el-option label="深造者" value="TYPE_B" />
        </el-select>
    ...
    <el-table :data="list" border stripe v-loading="loading">
      ...
      <el-table-column label="保证金" width="110" align="right">
        <template #default="{ row }">¥{{ Number(row.deposit || 0).toFixed(2) }}</template>
      </el-table-column>
      <el-table-column label="任务进度" width="130" align="center">
        <template #default="{ row }">
          <el-progress :percentage="row.tasksRequired > 0 ? Math.round(row.tasksCompleted / row.tasksRequired * 100) : 0"
      ...
      <el-table-column prop="joinedAt" label="加入时间" width="170">
        <template #default="{ row }">{{ row.joinedAt?.slice(0,16).replace('T',' ') }}</template>
      </el-table-column>
      <el-table-column label="操作" width="260" fixed="right">
        <template #default="{ row }">
          <el-button size="small" @click="openEdit(row)">编辑</el-button>
          <el-button v-if="row.status === 'ACTIVE'" size="small" type="warning" @click="handleUpdate(row, 'FROZEN')">冻结</el-button>
          <el-button v-if="row.status === 'FROZEN'" size="small" type="success" @click="handleUpdate(row, 'ACTIVE')">解冻</el-button>
          <el-button v-if="row.status === 'ACTIVE'" size="small" type="danger" @click="handleUpdate(row, 'LEFT')">退出</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <!-- 编辑弹窗 -->
    <el-dialog v-model="dialogVisible" title="编辑研究院成员" width="440px">
      <el-form :model="editForm" label-width="100px">
        <p style="margin-bottom:12px;color:#666;font-size:14px"><b>成员：</b>{{ editingRow?.user?.nickname || '-' }}</p>
        ...
        <el-form-item label="保证金(元)">
          <el-input-number v-model="editForm.deposit" :min="0" :step="100" :precision="2" style="width:100%" />
        </el-form-item>
        <el-form-item label="需完成任务">
          <el-input-number v-model="editForm.tasksRequired" :min="1" :max="99" />
        </el-form-item>
      </el-form>
      <template #footer>
