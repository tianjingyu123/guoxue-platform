<template>
  <div class="rag-template">
    ...
    <div class="filter-bar">
      <el-select v-model="filterScene" placeholder="场景筛选" clearable style="width:160px" @change="fetchList">
        <el-option v-for="s in scenes" :key="s.value" :label="s.label" :value="s.value" />
      </el-select>
      <el-select v-model="filterStatus" placeholder="状态筛选" clearable style="width:120px;margin-left:8px" @change="fetchList">
      ...
      <el-table :data="templates" stripe v-loading="loading">
        ...
        <el-table-column prop="variables" label="变量" width="180">
          <template #default="{ row }">
            <el-tag v-for="v in (row.variables || [])" :key="v" size="small" style="margin:2px">{{ v }}</el-tag>
          </template>
        </el-table-column>
    ...
    <!-- 编辑/创建对话框 -->
    <el-dialog v-model="showEdit" :title="editingId ? '编辑模板' : '新建模板'" width="640px" destroy-on-close>
      <el-form :model="editForm" label-width="100px">
        <el-form-item label="模板名称" required>
          <el-input v-model="editForm.templateName" placeholder="如：圈主助理问答模板" />
        </el-form-item>
        ...
        <el-form-item label="系统提示词" required>
          <el-input v-model="editForm.systemPrompt" type="textarea" :rows="4" placeholder="系统级提示词，定义AI角色和行为" />
        </el-form-item>
        <el-form-item label="用户提示词模板" required>
          <el-input v-model="editForm.userPromptTemplate" type="textarea" :rows="4" placeholder="支持 &#123;&#123;variable&#125;&#125; 变量替换，如：请根据&#123;&#123;knowledge&#125;&#125;回答用户问题：&#123;&#123;question&#125;&#125;" />
        </el-form-item>
        <el-form-item label="变量列表">
          <el-select v-model="editForm.variables" multiple allow-create filterable placeholder="输入变量名后回车添加" style="width:100%" />
        </el-form-item>
      ...
      <template #footer>
        ...
        <el-button type="primary" :loading="saving" @click="doSave">{{ editingId ? '保存' : '创建' }}</el-button>
      </template>
    </el-dialog>

    <!-- 预览对话框 -->
    <el-dialog v-model="showPreview" title="模板预览与测试" width="640px">
      <template v-if="previewTpl">
        <el-descriptions :column="1" border size="small">
          <el-descriptions-item label="模板">{{ previewTpl.templateName }}</el-descriptions-item>
          <el-descriptions-item label="场景">{{ sceneLabel(previewTpl.scene) }}</el-descriptions-item>
          <el-descriptions-item label="系统提示词"><pre>{{ previewTpl.systemPrompt }}</pre></el-descriptions-item>
          <el-descriptions-item label="用户提示词"><pre>{{ previewTpl.userPromptTemplate }}</pre></el-descriptions-item>
        </el-descriptions>
        <el-divider>变量填充测试</el-divider>
        <el-form label-width="80px">
          <el-form-item v-for="v in (previewTpl.variables || [])" :key="v" :label="v">
            <el-input v-model="previewVars[v]" :placeholder="`输入 ${v} 的值`" />
          </el-form-item>
        </el-form>
        <el-button type="primary" :loading="previewing" @click="doPreview">生成预览</el-button>
