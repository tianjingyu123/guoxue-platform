<template>
  <div class="competition-detail">
    <el-page-header title="返回" style="margin-bottom:16px" @back="router.push('/competitions')">
      <template #content>{{ detail?.title || '赛事详情' }}</template>
      <template #extra>
        <el-button v-if="detail?.status === 'DRAFT'" type="success" @click="changeStatus('publish')">发布赛事</el-button>
        <el-button v-if="detail?.status === 'PUBLISHED'" type="warning" @click="changeStatus('start')">开始赛事</el-button>
        <el-button v-if="detail?.status === 'IN_PROGRESS'" type="danger" @click="changeStatus('finish')">结束赛事</el-button>
        <el-button @click="router.push(`/competitions/${route.params.id}/edit`)">编辑</el-button>
      </template>
    </el-page-header>
    ...
    <el-tabs v-model="activeTab" type="border-card" @tab-change="onTabChange">
      ...
      <el-tab-pane label="基本信息" name="basic">
        <el-card v-loading="loading">
          <el-descriptions :column="2" border>
            <el-descriptions-item label="赛事标题" :span="2">{{ detail?.title }}</el-descriptions-item>
            <el-descriptions-item label="赛事类型">{{ typeLabels[detail?.type] || detail?.type }}</el-descriptions-item>
            <el-descriptions-item label="赛事级别">
              <el-tag :type="detail?.level === 'S' ? 'danger' : detail?.level === 'A' ? 'warning' : 'info'" size="small">
                {{ detail?.level }}级
              </el-tag>
            ...
            <el-descriptions-item label="状态">
              <el-tag :type="statusLabels[detail?.status]?.type || 'info'" size="small">
                {{ statusLabels[detail?.status]?.text || detail?.status }}
              </el-tag>
            </el-descriptions-item>
            <el-descriptions-item label="报名费">{{ detail?.entryFee > 0 ? (detail.entryFee / 100).toFixed(2) + '元' : '免费' }}</el-descriptions-item>
            <el-descriptions-item label="人数上限">{{ detail?.maxParticipants > 0 ? detail.maxParticipants + '人' : '不限' }}</el-descriptions-item>
            <el-descriptions-item label="邀请制">{{ detail?.isInviteOnly ? '是' : '否' }}</el-descriptions-item>
            <el-descriptions-item label="实名要求">{{ detail?.requireIdentity ? '需要' : '不要求' }}</el-descriptions-item>
            <el-descriptions-item label="最低等级">{{ detail?.minLevel > 0 ? 'Lv.' + detail.minLevel : '不限' }}</el-descriptions-item>
            <el-descriptions-item label="总奖金池">{{ detail?.totalPrize > 0 ? (detail.totalPrize / 100).toFixed(2) + '元' : '未设置' }}</el-descriptions-item>
            <el-descriptions-item label="奖品类型">{{ ({ CASH: "现金奖金", PHYSICAL: "实物奖品", VIRTUAL: "虚拟商品", MIXED: "混合" } as any)[detail?.prizeType] || "现金" }}</el-descriptions-item>
            <el-descriptions-item label="邀请分润">{{ detail?.invitationShare ? (detail.invitationShare / 10).toFixed(1) + '%' : '无' }}</el-descriptions-item>
            <el-descriptions-item v-if="detail?.prizeConfig?.length" label="奖品分配" :span="2">
              <div class="prize-config-list">
                <div v-for="(item, idx) in detail.prizeConfig" :key="idx" class="prize-config-item">
                  <el-tag size="small" type="danger">{{ item.title || '第' + item.rank + '名' }}</el-tag>
                  <template v-if="item.prizeType === 'PHYSICAL' || item.prizeType === 'VIRTUAL'">
                    <span>{{ item.prizeItem || item.description }}</span>
                    <el-tag size="small" :type="item.prizeType === 'PHYSICAL' ? 'warning' : ''">实物</el-tag>
                  </template>
                  <template v-else>
                    <span style="color:#C41E3A;font-weight:bold">¥{{ ((item.prize || 0) / 100).toFixed(0) }}</span>
                  </template>
                  <span v-if="item.description" style="color:#999;font-size:12px">{{ item.description }}</span>
                </div>
              </div>
            </el-descriptions-item>
            <el-descriptions-item label="报名人数">{{ detail?._count?.registrations || 0 }}人</el-descriptions-item>
            <el-descriptions-item label="题目数量">{{ detail?._count?.questions || 0 }}题</el-descriptions-item>
            <el-descriptions-item label="标签" :span="2">
              <el-tag v-for="t in detail?.tags" :key="t" size="small" style="margin-right:4px">{{ t }}</el-tag>
              <span v-if="!detail?.tags?.length" style="color:#ccc">-</span>
            </el-descriptions-item>
            <el-descriptions-item label="创建时间">{{ formatDate(detail?.createdAt) }}</el-descriptions-item>
            <el-descriptions-item label="发布时间">{{ formatDate(detail?.publishedAt) }}</el-descriptions-item>
          </el-descriptions>

          <el-divider>赛事描述</el-divider>
          <div class="markdown-body" v-html="detail?.description || '暂无描述'" style="min-height:40px" />

          <el-divider v-if="detail?.rules">赛事规则</el-divider>
          <div v-if="detail?.rules" class="markdown-body" v-html="detail?.rules" />
        </el-card>
      </el-tab-pane>

      <!-- Tab 2: 赛程管理 -->
      <el-tab-pane label="赛程管理" name="rounds">
        <div class="section-header">
          <span>共 {{ rounds.length }} 个赛程</span>
          <el-button type="primary" size="small" @click="openRoundDialog()">添加赛程</el-button>
        </div>
        <el-table :data="rounds" stripe style="margin-top:12px">
          <el-table-column label="排序" width="60" prop="sortOrder" />
          <el-table-column prop="title" label="标题" min-width="150" />
          <el-table-column label="类型" width="90">
            <template #default="{ row }">
              <el-tag :type="row.roundType === 'FINAL' ? 'danger' : row.roundType === 'SEMIFINAL' ? 'warning' : 'info'" size="small">
          ...
          <el-table-column label="晋级人数" width="90">
            <template #default="{ row }">{{ row.passCount || '-' }}</template>
          </el-table-column>
          <el-table-column label="操作" width="150">
            <template #default="{ row }">
              <el-button size="small" @click="openRoundDialog(row)">编辑</el-button>
              <el-button size="small" type="danger" @click="handleDeleteRound(row.id)">删除</el-button>
            </template>
          </el-table-column>
        </el-table>
      </el-tab-pane>
      ...
      <el-tab-pane label="题库管理" name="questions">
        <div class="section-header">
          ...
          <div style="display:flex;gap:8px">
            <el-select v-model="questionFilter.roundId" placeholder="按赛程筛选" clearable size="small" style="width:160px" @change="fetchQuestions">
              <el-option v-for="r in rounds" :key="r.id" :label="r.title" :value="r.id" />
            </el-select>
            <el-button size="small" @click="fetchQuestions">刷新</el-button>
            <el-button type="primary" size="small" @click="openQuestionDialog()">添加题目</el-button>
            <el-button size="small" @click="openBatchImport">批量导入</el-button>
          </div>
        </div>
        <el-table :data="questions" v-loading="questionLoading" stripe style="margin-top:12px">
          <el-table-column label="排序" width="60" prop="sortOrder" />
      ...
      <el-tab-pane label="报名管理" name="registrations">
        <div class="section-header">
          ...
          <div style="display:flex;gap:8px">
            <el-select v-model="regFilter.status" placeholder="报名状态" clearable size="small" style="width:120px" @change="fetchRegistrations">
              <el-option label="待审核" value="PENDING" />
              <el-option label="已确认" value="QUALIFIED" />
              <el-option label="已拒绝" value="DISQUALIFIED" />
              <el-option label="已取消" value="CANCELLED" />
            </el-select>
            <el-button size="small" @click="fetchRegistrations">刷新</el-button>
          </div>
        </div>
        <el-table :data="registrations" v-loading="regLoading" stripe style="margin-top:12px">
          ...
          <el-table-column label="状态" width="90">
            <template #default="{ row }">
              <el-tag :type="({ PENDING: 'warning', QUALIFIED: 'success', DISQUALIFIED: 'danger', CANCELLED: 'info' } as Record<string, string>)[row.status] || 'info'" size="small">
                {{ ({ PENDING: '待审核', QUALIFIED: '已确认', DISQUALIFIED: '已拒绝', CANCELLED: '已取消' } as Record<string, string>)[row.status] || row.status }}
              </el-tag>
            </template>
          </el-table-column>
          <el-table-column label="报名费" width="80">
            <template #default="{ row }">{{ row.paidFee > 0 ? (row.paidFee / 100).toFixed(2) + '元' : '免费' }}</template>
          </el-table-column>
          <el-table-column label="邀请人" width="100">
            <template #default="{ row }">{{ row.inviterId?.slice(0, 8) || '-' }}</template>
          </el-table-column>
          ...
          <el-table-column label="操作" width="160">
            <template #default="{ row }">
              <el-button v-if="row.status === 'PENDING'" size="small" type="success" @click="handleRegAction(row.id, 'QUALIFIED')">确认</el-button>
              <el-button v-if="row.status === 'PENDING'" size="small" type="danger" @click="handleRegAction(row.id, 'DISQUALIFIED')">拒绝</el-button>
            </template>
          </el-table-column>
        </el-table>
        <el-pagination
      ...
      <!-- Tab 5: 排名管理 -->
      <el-tab-pane label="排名管理" name="rankings">
        <div class="section-header">
          <span>共 {{ rankTotal }} 条排名</span>
          <div style="display:flex;gap:8px">
            <el-select v-model="rankFilter.roundId" placeholder="按赛程筛选" clearable size="small" style="width:160px" @change="fetchRankings">
              <el-option v-for="r in rounds" :key="r.id" :label="r.title" :value="r.id" />
            </el-select>
            <el-button type="warning" size="small" @click="handleCalcRanking">计算排名</el-button>
            <el-button size="small" @click="fetchRankings">刷新</el-button>
          </div>
        </div>
        <el-table :data="rankings" v-loading="rankLoading" stripe style="margin-top:12px">
          <el-table-column label="排名" width="70" prop="rank">
            <template #default="{ row }">
              <el-tag v-if="row.rank === 1" type="danger" size="small">🥇 1</el-tag>
              <el-tag v-else-if="row.rank === 2" type="warning" size="small">🥈 2</el-tag>
              <el-tag v-else-if="row.rank === 3" type="success" size="small">🥉 3</el-tag>
          ...
          <el-table-column label="总分" width="80" prop="score" />
          <el-table-column label="状态" width="80">
            <template #default="{ row }">
              <el-tag :type="({ CHAMPION: 'danger', RUNNER_UP: 'warning', THIRD_PLACE: 'success', ELIMINATED: 'info' } as Record<string, string>)[row.status] || 'info'" size="small">
          ...
          <el-table-column label="证书" width="100">
            <template #default="{ row }">
              <el-button v-if="row.certificateUrl" size="small" @click="viewCert(row)">查看证书</el-button>
    ...
    <!-- 赛程编辑弹窗 -->
    <el-dialog v-model="roundDialog" :title="editingRound ? '编辑赛程' : '添加赛程'" width="600px">
      <el-form :model="roundForm" label-width="100px">
        <el-form-item label="标题" required><el-input v-model="roundForm.title" placeholder="赛程标题" /></el-form-item>
        ...
        <el-form-item label="排序"><el-input-number v-model="roundForm.sortOrder" :min="0" /></el-form-item>
        <el-row :gutter="12">
          <el-col :span="12">
            <el-form-item label="开始时间">
              <el-date-picker v-model="roundForm.startAt" type="datetime" placeholder="选择时间" style="width:100%" value-format="YYYY-MM-DDTHH:mm:ss.sssZ" />
            </el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item label="结束时间">
              <el-date-picker v-model="roundForm.endAt" type="datetime" placeholder="选择时间" style="width:100%" value-format="YYYY-MM-DDTHH:mm:ss.sssZ" />
            </el-form-item>
          </el-col>
        </el-row>
        <el-row :gutter="12">
          <el-col :span="8">
            <el-form-item label="时长(分)"><el-input-number v-model="roundForm.duration" :min="0" /></el-form-item>
          </el-col>
          <el-col :span="8">
            <el-form-item label="晋级人数"><el-input-number v-model="roundForm.passCount" :min="0" /></el-form-item>
          </el-col>
          <el-col :span="8">
            <el-form-item label="晋级比例(%)"><el-input-number v-model="roundForm.passPercent" :min="0" :max="100" /></el-form-item>
          </el-col>
        </el-row>
        <el-form-item label="描述"><el-input v-model="roundForm.description" type="textarea" :rows="2" /></el-form-item>
      </el-form>
      <template #footer>
    ...
    <!-- 题目编辑弹窗 -->
    <el-dialog v-model="questionDialog" :title="editingQuestion ? '编辑题目' : '添加题目'" width="700px">
      <el-form :model="questionForm" label-width="100px">
        <el-form-item label="题干" required>
          <el-input v-model="questionForm.stem" type="textarea" :rows="2" placeholder="题目内容" />
        </el-form-item>
        <el-row :gutter="12">
          <el-col :span="8">
            <el-form-item label="题型">
              <el-select v-model="questionForm.type" style="width:100%">
                ...
                <el-option label="配对题" value="MATCHING" />
                <el-option label="代码题" value="CODE" />
                <el-option label="上传题" value="UPLOAD" />
              </el-select>
            </el-form-item>
          </el-col>
          <el-col :span="8">
        ...
        <el-row :gutter="12">
          <el-col :span="8">
            <el-form-item label="所属赛程">
              <el-select v-model="questionForm.roundId" placeholder="可选" clearable style="width:100%">
                <el-option v-for="r in rounds" :key="r.id" :label="r.title" :value="r.id" />
              </el-select>
            </el-form-item>
          </el-col>
          <el-col :span="8">
            <el-form-item label="排序"><el-input-number v-model="questionForm.sortOrder" :min="0" /></el-form-item>
          </el-col>
          <el-col :span="8">
        ...
        <el-form-item label="选项配置">
          <el-input v-model="optionsJson" type="textarea" :rows="4" placeholder='JSON数组，如 [{"label":"A","value":"a","text":"选项A"}]' />
        </el-form-item>
        <el-form-item label="正确答案">
          <el-input v-model="answerJson" type="textarea" :rows="2" placeholder='JSON，如 {"value":"a"} 或 {"values":["a","b"]}' />
        </el-form-item>
    ...
    <!-- 批量导入题目弹窗 -->
    <el-dialog v-model="batchDialog" title="批量导入题目" width="600px">
      <el-form label-width="80px">
        ...
        <el-form-item label="题目数据">
          <el-input v-model="batchContent" type="textarea" :rows="12" :placeholder="batchPlaceholder" />
        </el-form-item>
      </el-form>
      <template #footer>
        <el-button @click="batchDialog = false">取消</el-button>
        <el-button type="primary" :loading="batchImporting" @click="doBatchImport">导入</el-button>
      </template>
    </el-dialog>

    <!-- 证书预览弹窗 -->
    <el-dialog v-model="certDialog" title="获奖证书" width="850px" top="3vh">
      <iframe v-if="certHtml" :srcdoc="certHtml" style="width:100%;height:600px;border:1px solid #eee" />
    </el-dialog>
  </div>
</template>
