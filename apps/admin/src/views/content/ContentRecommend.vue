<template>
  <div class="recommend-page">
    <div class="toolbar">
      <h3>推荐管理</h3>
      <el-button type="primary" @click="openCreate">添加推荐位</el-button>
    </div>

    <el-table v-loading="loading" :data="list" stripe>
      <el-table-column label="封面" width="70">
        <template #default="{ row }">
          <img v-if="row.cover || row.image" :src="row.cover || row.image" class="thumb">
          <span v-else class="no-img">无</span>
        </template>
      </el-table-column>
      <el-table-column prop="title" label="标题" min-width="180" show-overflow-tooltip />
      <el-table-column label="类型" width="90">
        <template #default="{ row }">{{ typeLabel(row.targetType || row.type) }}</template>
      </el-table-column>
      ...
      <el-table-column label="权重" width="80">
        <template #default="{ row }">
          <el-tag :type="row.weight > 50 ? 'danger' : row.weight > 20 ? 'warning' : ''" size="small">{{ row.weight }}</el-tag>
        </template>
      </el-table-column>
    ...
    <el-dialog v-model="dialogVisible" :title="editingId ? '编辑推荐位' : '添加推荐位'" width="550px">
      <el-form :model="form" label-width="80px">
        ...
        <el-form-item label="内容ID" required>
          <el-input v-model="form.targetId" placeholder="输入内容ID" />
        </el-form-item>
        <el-form-item label="推荐位置" required>
