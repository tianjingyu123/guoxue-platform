<template>
  <div class="huifu-page">
    <div class="page-header">
      <h3>汇付天下支付管理</h3>
      <div>
        <el-tag :type="paymentStatus.enabled ? 'success' : 'danger'" size="small" style="margin-right:8px">
    ...
    <el-tabs v-model="activeTab" @tab-change="onTabChange">
      ...
      <el-tab-pane label="支付配置" name="config">
        <el-table :data="configList" stripe size="small">
          <el-table-column label="配置项" prop="key" width="220" />
          <el-table-column label="值" prop="value">
            <template #default="{ row }">
              <template v-if="editingKey === row.key">
                <el-input v-model="editValue" size="small" style="width:300px" />
                <el-button size="small" type="primary" style="margin-left:8px" @click="saveConfig(row)">保存</el-button>
      ...
      <!-- 分账管理 -->
      <el-tab-pane label="分账管理" name="split">
        <div class="toolbar-row">
          <el-button type="primary" size="small" @click="splitVisible = true">发起分账</el-button>
          <el-input v-model="splitQuery.orderId" placeholder="订单号查询" size="small" style="width:250px;margin-left:8px" />
          <el-button size="small" @click="querySplitResult">查询分账</el-button>
        </div>
        <el-table :data="splitList" stripe size="small" style="margin-top:12px">
          ...
          <el-table-column label="接收方" prop="receiverName" width="150" />
      ...
      <el-tab-pane label="退款管理" name="refund">
        <div class="toolbar-row">
          <el-button type="primary" size="small" @click="refundVisible = true">发起退款</el-button>
        </div>
        <el-table :data="refundList" stripe size="small" style="margin-top:12px">
          <el-table-column label="原交易号" prop="outTradeNo" width="200" />
          <el-table-column label="退款金额" width="120">
            <template #default="{ row }">¥{{ row.amount || row.refundAmount || 0 }}</template>
          </el-table-column>
          <el-table-column label="退款原因" prop="reason" min-width="150" />
          <el-table-column label="状态" width="100">
            <template #default="{ row }">
      ...
      <!-- 账单下载 -->
      <el-tab-pane label="账单下载" name="bill">
        <el-form inline>
          <el-form-item label="日期">
            <el-date-picker v-model="billDate" type="date" placeholder="选择日期" value-format="YYYY-MM-DD" />
          </el-form-item>
          <el-form-item>
            <el-button type="primary" @click="downloadBill">下载账单</el-button>
          </el-form-item>
        </el-form>
        <div class="text-muted" style="margin-top:8px">账单格式为 CSV，可用于对账核对</div>
      </el-tab-pane>
    </el-tabs>

    <!-- 分账弹窗 -->
    <el-dialog v-model="splitVisible" title="发起分账" width="450px">
      <el-form label-width="100px">
        <el-form-item label="订单号" required><el-input v-model="splitForm.orderId" /></el-form-item>
        <el-form-item label="分账金额" required><el-input v-model="splitForm.amount" placeholder="单位：元" /></el-form-item>
        <el-form-item label="接收方ID"><el-input v-model="splitForm.receiverId" /></el-form-item>
        <el-form-item label="接收方名称"><el-input v-model="splitForm.receiverName" /></el-form-item>
      </el-form>
      <template #footer>
        <el-button @click="splitVisible = false">取消</el-button>
        <el-button type="primary" :loading="splitSubmitting" @click="submitSplit">确认分账</el-button>
      </template>
    </el-dialog>

    <!-- 退款弹窗 -->
    <el-dialog v-model="refundVisible" title="发起退款" width="450px">
      <el-form label-width="100px">
        <el-form-item label="原交易号" required><el-input v-model="refundForm.outTradeNo" /></el-form-item>
        <el-form-item label="退款金额" required><el-input v-model="refundForm.amount" placeholder="单位：元" /></el-form-item>
        <el-form-item label="退款原因"><el-input v-model="refundForm.reason" /></el-form-item>
      </el-form>
      <p style="color:#e6a23c; font-size:13px">⚠️ 退款操作不可逆，请仔细核对信息</p>
      <template #footer>
