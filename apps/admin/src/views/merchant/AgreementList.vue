<template>
  <div class="page">
    <div class="toolbar">
      <h3>商家协议</h3>
      <el-button type="primary" @click="openCreate">新建协议版本</el-button>
    </div>
    ...
    <el-table v-loading="loading" :data="list" stripe>
      ...
      <el-table-column label="创建时间" width="180">
        <template #default="{ row }">{{ new Date(row.createdAt).toLocaleDateString('zh-CN', { year: 'numeric', month: '2-digit', day: '2-digit', hour: '2-digit', minute: '2-digit' }) }}</template>
      </el-table-column>
    ...
    <el-dialog v-model="dialogVisible" :title="editingId ? '编辑协议' : '新建协议'" width="700px">
      <el-form :model="form" label-width="80px">
        <el-form-item label="版本号" required>
          <el-input v-model="form.version" :disabled="!!editingId" placeholder="如 1.0.0" />
        </el-form-item>
        <el-form-item label="协议标题" required>
          <el-input v-model="form.title" placeholder="如 商家入驻协议" />
        </el-form-item>
        <el-form-item label="协议内容" required>
          <el-input v-model="form.content" type="textarea" :rows="12" placeholder="HTML 或 Markdown 格式" />
        </el-form-item>
      </el-form>
      <template #footer>
