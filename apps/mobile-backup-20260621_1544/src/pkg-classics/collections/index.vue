            ...
            >
              <view class="cl-cover">
                <flat-cover
                  ...
                  :cover-color="String(coverColorForBook(item.title))"
                  :title-size="'22'"
                />
              </view>
              <view class="cl-info">
...
import { classicsApi, type CollectionItem } from '@/lib/classics-data'

const searchText = ref('')
const filter = ref<string>('all')
const collections = ref<CollectionItem[]>([])
const typeMeta = ref<Record<string, { label: string; icon: string }>>({})
const filters = ref<Array<{ id: string; label: string }>>([])

onMounted(async () => {
  const res = await classicsApi.collections()
  collections.value = res.items as any
  typeMeta.value = res.typeMeta as any
  filters.value = [...res.filters] as any
})
