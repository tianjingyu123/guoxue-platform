...
import { getUserIdFromToken } from '@/lib/profile-data'
import {
...
async function loadData() {
  ...
  try {
    ...
    isOwner.value = (c as any).isOwner || false
...
<template>
  ...
  >
    ...
    >
      <view
        class="cd-checkin-card"
        :class="{ done: hasCheckedIn }"
        @tap="handleCheckIn"
      >
        <view class="cd-checkin-left">
          <app-icon
            ...
            :size="40"
            :color="hasCheckedIn ? '#22c55e' : '#c41e3a'"
          />
          <view class="cd-checkin-info">
            <text class="cd-checkin-title">
              {{ hasCheckedIn ? '今日已签到' : '每日签到' }}
            </text>
            <text class="cd-checkin-sub">
              连续签到 <text class="cd-checkin-streak">
                {{ checkInStreak }}
              </text> 天
            </text>
          </view>
    ...
    <view class="cd-foot">
      ...
      >
        <text
          ...
          :class="{ joined: isJoined }"
        >
          {{ isJoined ? '已加入' : (isPaid ? `${priceLabel} 加入圈子` : '免费加入圈子') }}
        </text>
      </view>
      <view
        v-if="isJoined"
    ...
    <view
      ...
      class="cd-mask"
      @tap="showLeaveConfirm = false"
    >
      <view
        class="cd-leave-sheet"
        @tap.stop
      >
        <view class="cd-leave-icon">
          <app-icon
            name="log-out"
            :size="48"
            color="#EF4444"
        ...
        <text class="cd-leave-title">
          确认退出圈子？
        </text>
        <text class="cd-leave-desc">
          退出后将失去会员权益和所有专属内容
        </text>
        <view class="cd-leave-actions">
          <view
            class="cd-leave-btn cancel"
            @tap="showLeaveConfirm = false"
          >
            <text class="cd-leave-btn-t">
              再想想
            </text>
          </view>
          <view
            class="cd-leave-btn confirm"
            @tap="handleLeave"
          >
            <text class="cd-leave-btn-t">
              确认退出
  ...
  >
    <view class="sk-cover" />
    <view class="sk-info">
      <view class="sk-card" /><view class="sk-card sk-card-sm" /><view class="sk-card sk-card-sm" />
    </view>
  </view>
