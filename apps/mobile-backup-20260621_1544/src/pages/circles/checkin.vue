...
async function loadData() {
  ...
  try {
    ...
    if (actData.status === 'fulfilled' && (actData as any).value) {
      const { todayContent: tc, ...rest } = (actData as any).value as any
      Object.assign(activity.value, rest)
      if (tc) todayContent.value = tc
    }
    if (feedData.status === 'fulfilled') checkinFeed.value = Array.isArray((feedData as any).value) ? (feedData as any).value : (((feedData as any).value as any)?.data || [])
    if (leadData.status === 'fulfilled') leaderboard.value = Array.isArray((leadData as any).value) ? (leadData as any).value : (((leadData as any).value as any)?.data || [])
    if (myData.status === 'fulfilled') myCheckins.value = Array.isArray((myData as any).value) ? (myData as any).value : (((myData as any).value as any)?.data || [])
  } catch (e: any) {
