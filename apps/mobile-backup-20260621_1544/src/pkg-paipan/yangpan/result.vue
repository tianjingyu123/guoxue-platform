...
const q = reactive({
  ...
  panMethod: 'zhuan', jigongMethod: 'kungong', startMethod: 'chaibu', anganMethod: 'dipan',
  trueSolar: false, earlyLateZi: false, daylightSaving: false, place: '',
})
...
onLoad((opts: Record<string, string> = {}) => {
  ...
  if (opts.trueSolar !== undefined) q.trueSolar = opts.trueSolar === 'true'
  if (opts.earlyLateZi !== undefined) q.earlyLateZi = opts.earlyLateZi === 'true'
  if (opts.daylightSaving !== undefined) q.daylightSaving = opts.daylightSaving === 'true'
  if (opts.place) q.place = decodeURIComponent(opts.place)
  fetchYangpanData()
})
...
async function fetchYangpanData() {
  ...
  try {
    ...
    if (result.mingli?.daYun) {
      ...
      // 生成流年数据（基于当前活跃大运，传入出生月日判立春）
      buildLiuNianData(q.month, q.day)
    }
    // 旬首
...
function deriveXunShou(yongShi: string): string {
  ...
  // 六仪：甲子→戊, 甲寅→癸, 甲辰→壬, 甲午→辛, 甲申→庚, 甲戌→己
  const YI_MAP = ['戊','癸','壬','辛','庚','己']
  const riZhi = yongShi[1]
  const zhiIdx = ZHI.indexOf(riZhi)
  if (zhiIdx === -1) return '甲子戊'
  // 旬首地支：每对地支共享同一旬首（子丑→子, 寅卯→寅, ...）
  const xunZhiIdx = Math.floor(zhiIdx / 2) * 2
  const xunYi = YI_MAP[xunZhiIdx / 2]
  return `甲${xunYi}${ZHI[xunZhiIdx]}`
}

function buildLiuNianData(birthMonth?: number, birthDay?: number, riGan?: string) {
  const GAN = ['甲','乙','丙','丁','戊','己','庚','辛','壬','癸']
  const ZHI = ['子','丑','寅','卯','辰','巳','午','未','申','酉','戌','亥']
  ...
  // 十神相对于日干计算
  const riGanIdx = riGan ? GAN.indexOf(riGan) : GAN.indexOf(activeDy.gan)
  // 立春通常在2月4日前后，用于判断出生日期是否在立春前
  const isBeforeLichun = birthMonth !== undefined && (birthMonth < 2 || (birthMonth === 2 && (birthDay || 1) < 4))
  const items: LiuNianItem[] = []
  for (let i = 0; i < 10; i++) {
    const year = startYear + i
    // 公元→干支：立春前出生的，立春前那段时期沿用上一年的干支
    const adjustedYear = (isBeforeLichun && year === startYear) ? year - 1 : year
    const ganzhiIdx = (adjustedYear - 4) % 60
...
// 计算公历日期对应的日干支（1900-01-01 = 甲戌，序数10）
...
// 年上起月（五虎遁）
const YUE_GAN_MAP: Record<string, number> = { 甲: 2, 乙: 4, 丙: 6, 丁: 8, 戊: 0, 己: 2, 庚: 4, 辛: 6, 壬: 8, 癸: 0 }
// 日上起时（五鼠遁）
const SHI_GAN_MAP: Record<string, number> = { 甲: 0, 乙: 2, 丙: 4, 丁: 6, 戊: 8, 己: 0, 庚: 2, 辛: 4, 壬: 6, 癸: 8 }

function calcSizhu(year: number, month: number, day: number, hour: number) {
  // 年柱
  const yearIdx = (year - 4) % 60
  const yearGan = GAN[((yearIdx % 10) + 10) % 10]
  const yearZhi = ZHI[((yearIdx % 12) + 12) % 12]

  // 月柱（简化：公历月近似于节气月）
  const yueZhiIdx = (month + 1) % 12 // 1月≈丑, 2月≈寅, ...
  const yueGanIdx = (YUE_GAN_MAP[yearGan] + yueZhiIdx) % 10
  const yueGan = GAN[yueGanIdx]
  const yueZhi = ZHI[yueZhiIdx]
  ...
  const shiGanIdx = (SHI_GAN_MAP[dayGz.gan] + shiZhiIdx) % 10
  const shiGan = GAN[shiGanIdx]
  const shiZhi = ZHI[shiZhiIdx]

  return {
    sizhu: [
      { label: '年柱', g: yearGan, z: yearZhi },
      { label: '月柱', g: yueGan, z: yueZhi },
      { label: '日柱', g: dayGz.gan, z: dayGz.zhi },
      { label: '时柱', g: shiGan, z: shiZhi },
    ],
    // 空亡：日柱地支的旬空
    kongwang: getKongwang(dayGz.zhi),
  }
}

function getKongwang(dayZhi: string) {
  const zhiIdx = ZHI.indexOf(dayZhi)
  if (zhiIdx === -1) return ['子丑', '子丑', '申酉', '寅卯']
  const xunIdx = Math.floor(zhiIdx / 2) * 2
  const kw1 = ZHI[(xunIdx + 8) % 12]
  const kw2 = ZHI[(xunIdx + 9) % 12]
  const kw = kw1 + kw2
  return [kw, kw, kw, kw]
}

const computedSizhu = computed(() => calcSizhu(q.year, q.month, q.day, q.hour))
const sizhu = computed(() => computedSizhu.value.sizhu)
const kongwangData = computed(() =>
  computedSizhu.value.kongwang.map((zhi, i) => ({ label: ['年', '月', '日', '时'][i], zhi })),
)
...
async function handleSave() {
  ...
  saving.value = true
  try {
    const res = await yangpanApi.save({
      name: q.name, gender: q.gender as 'male' | 'female',
      year: q.year, month: q.month, day: q.day, hour: q.hour, minute: q.minute,
      panMethod: q.panMethod as 'zhuan' | 'fei',
      jigongMethod: q.jigongMethod as 'kungong' | 'yanggenyin',
      startMethod: q.startMethod as 'chaibu' | 'maoshan' | 'zhirun',
      anganMethod: q.anganMethod as 'zhishi' | 'dipan',
      trueSolar: q.trueSolar, earlyLateZi: q.earlyLateZi, daylightSaving: q.daylightSaving,
      place: q.place,
    })
...
const draftOpts = reactive([q.trueSolar, q.earlyLateZi, q.daylightSaving])

function openEdit() {
  Object.assign(draft, q)
  draftOpts[0] = q.trueSolar
  draftOpts[1] = q.earlyLateZi
  draftOpts[2] = q.daylightSaving
  showEditModal.value = true
}
function confirmEdit() {
  Object.assign(q, draft)
  q.trueSolar = draftOpts[0]
  q.earlyLateZi = draftOpts[1]
  q.daylightSaving = draftOpts[2]
      ...
      >
        ...
        <view class="form">
          ...
          <view class="frow">
            ...
            <view class="genders">
              <view class="gender" @tap="draft.gender = 'male'">
                <view class="radio" :class="{ 'radio-on': draft.gender === 'male' }" /><text class="gtext">男</text>
              </view>
              <view class="gender" @tap="draft.gender = 'female'">
                <view class="radio" :class="{ 'radio-on': draft.gender === 'female' }" /><text class="gtext">女</text>
              </view>
            </view>
          </view>
          <!-- 出生时间 -->
          ...
          <!-- 排盘方法 -->
          <view class="fblock">
            <text class="flabel block">排盘方法</text>
            <view class="opt-row">
              <view class="opt-half">
              ...
              <view class="opt-half">
                <text class="tg-lab">寄宫</text>
                <view class="chips">
                  <view class="chip" :class="{ 'chip-on': draft.jigongMethod === 'kungong' }" @tap="draft.jigongMethod = 'kungong'"><text class="chip-t">坤宫</text></view>
                  <view class="chip" :class="{ 'chip-on': draft.jigongMethod === 'yanggenyin' }" @tap="draft.jigongMethod = 'yanggenyin'"><text class="chip-t">阳艮阴坤</text></view>
                </view>
              </view>
            </view>
          </view>
          <!-- 定局/暗干 -->
          <view class="fblock">
            <text class="flabel block">定局与暗干</text>
            <view class="opt-row">
              <view class="opt-half">
        ...
        <!-- 选项 -->
        <view class="opts">
          <view v-for="(opt, i) in ['真太阳时', '早晚子时', '夏令时']" :key="opt" class="opt" @tap="draftOpts[i] = !draftOpts[i]">
            <view class="checkbox" :class="{ 'checkbox-on': draftOpts[i] }">
              <app-icon v-if="draftOpts[i]" name="check" :size="22" color="#ffffff" />
            </view>
