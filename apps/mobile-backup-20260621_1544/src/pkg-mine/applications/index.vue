      ...
      <template v-if="tab === 'feature'">
        ...
        >
          <view class="card-row">
            <view class="icon-box">
              <app-icon
                :name="app.feature === 'ecommerce_live' ? 'shopping-bag' : 'sparkles'"
                :size="20"
                :color="app.feature === 'ecommerce_live' ? '#C41E3A' : '#C9A96E'"
              />
            </view>
            <view class="card-main">
              ...
              >
                <text
                  class="link-muted"
                  @click="rejectDetail = app.rejectReason || '暂不满足开通条件'"
                >
                  驳回原因
                </text>
                <text class="link-red">
          ...
          <view class="card-row">
            ...
            <view class="card-main">
              ...
              <text
                ...
                class="link-muted"
                @click="rejectDetail = app.rejectReason || '申请未通过'"
              >
                查看驳回原因
              </text>
            </view>
      ...
      <template v-if="tab === 'exit'">
        ...
        >
            ...
            <view class="card-main">
              <view class="card-head">
                ...
                >
                  <text
                    class="status-text"
                    :style="{ color: toneColor(app.tone) }"
                  >
                    {{ app.stageLabel }}
                  </text>
                </view>
          ...
          <view class="refund-row">
            ...
            <text class="refund-used">
              （已用 {{ app.usedDays }} 天）
            </text>
          </view>
          <!-- 被驳回 -->
          <view
            v-if="app.stage === 'rejected'"
            class="reject-box"
          >
            <text class="reject-by">
          ...
          >
            <view
              ...
              :key="step.key"
              class="step-item"
              :class="{ 'step-last': idx === flowSteps.length - 1 }"
            >
              <view class="step-col">
                <view
                  class="step-dot"
                  :class="stepDotClass(app, idx)"
                >
                  <app-icon
                    v-if="idx <= app.completedIdx"
                    name="check"
                    :size="12"
                    color="#FFFFFF"
                  />
                  <text
                    v-else
                    class="step-num"
                    :class="{ 'step-num-cur': idx === app.completedIdx + 1 }"
                  >
                    {{ idx + 1 }}
                  </text>
                </view>
                <text
                  class="step-label"
                  :class="{ 'step-label-on': idx <= app.completedIdx || idx === app.completedIdx + 1 }"
                >
                  {{ step.label }}
                </text>
              </view>
              <view
                ...
                class="step-line"
                :class="{ 'step-line-on': idx < app.completedIdx }"
              />
            </view>
          </view>
          <!-- 当前状态提示 -->
          <text
            v-if="app.stage === 'owner_reviewing'"
            class="stage-tip"
          >
            圈主正在审核你的退出申请
          </text>
          <text
            v-else-if="app.stage === 'platform_reviewing'"
            class="stage-tip"
          >
            圈主已同意，平台审核中
          </text>
          <text
            v-else-if="app.stage === 'refunding'"
            class="stage-tip"
          >
            审核通过，退款处理中，预计 1-3 个工作日到账
          </text>
          <text
            v-else-if="app.stage === 'refunded'"
            class="stage-tip stage-tip-green"
          >
            退款 ¥{{ app.refundAmount }} 已于 {{ app.refundedAt }} 到账
          </text>
        </view>
      </template>
    </view>
