...
const domain = reactive({ custom: '', ssl: false })
...
onMounted(async () => {
  try {
    const data = await operatorApi.getStationManage()
    if (data?.basic) Object.assign(basic, data.basic)
    if (data?.features) Object.assign(features, data.features)
    if (data?.domain) Object.assign(domain, data.domain)
...
async function handleSave() {
  ...
  saving.value = true
  try {
    await operatorApi.saveStationManage({
      basic: { ...basic },
      features: { ...features },
      domain: { ...domain },
      notify: { ...notify },
