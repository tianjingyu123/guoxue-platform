...
async function loadData() {
  ...
  try {
    const res = await operatorApi.quota()
    data.value = res.quotaData
    records.value = res.records
    saleLink.value = res.saleLink
...
function copyLink() {
  uni.setClipboardData({ data: saleLink.value, success: () => { copied.value = true; setTimeout(() => (copied.value = false), 2000) } })
}
