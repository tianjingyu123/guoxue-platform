<template>
  <view class="asst-page">
    ...
    <view
      v-if="loading"
      class="asst-body asst-body-center"
    >
    ...
    >
      <app-icon
        ...
        color="#ef4444"
      />
      <text class="asst-error-txt">
        {{ error }}
      </text>
      <view
        class="asst-retry-btn"
        @tap="loadData"
      >
        重试
      </view>
...
const error = ref('')
const config = ref({ name: '站长助理', welcomeMessage: '', suggestions: [] as { id: string; text: string; category: string }[], capabilities: [] as string[] })
...
async function loadData() {
  ...
  try {
    const res = await stationAssistantApi.config()
    config.value = res
