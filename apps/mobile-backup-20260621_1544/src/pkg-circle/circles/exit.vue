...
const membership = ref<any>({})
const today = new Date().toISOString().slice(0, 10)
const breakdown = computed(() => {
  const m = membership.value
  if (!m.paidAmount) return { paidAmount: 0, usedDays: 0, totalDays: 0, dailyRate: 0, deduction: 0, refundAmount: 0 }
  return calcRefund({
...
onMounted(async () => {
  try {
    ...
    const circleId = page?.options?.id || page?.options?.circleId || '1'
    membership.value = await circleExitApi.getMyMembership(circleId)
  } catch (_) {
    membership.value = await circleExitApi.getMyMembership('1')
  } finally {
    loading.value = false
  }
})
...
async function handleSubmit() {
  ...
  try {
    ...
    const page = pages[pages.length - 1] as any
    const circleId = page?.options?.id || page?.options?.circleId || '1'
    await circleExitApi.submitExit(circleId, reason.value)
  } catch (_) {}
