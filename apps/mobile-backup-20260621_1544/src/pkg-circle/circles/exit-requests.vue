...
import { circleExitApi, getExitStageDisplay, type ExitApplication } from '@/lib/circle-exit'
...
onMounted(async () => {
  try {
    ...
    requests.value = await circleExitApi.getExitRequests(circleId) as ExitApplication[]
  } catch (_) {
    requests.value = []
  }
})
