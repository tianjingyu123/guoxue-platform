...
onLoad(async (q: Record<string, string> = {}) => {
  ...
  } catch (e) { /* useMock fallback in API */ }
})

const activeTab = ref('overview')
const replayPublic = computed({ get: () => replay.value.isPublic, set: (v) => { replay.value.isPublic = v } })
const replayPaid = computed({ get: () => replay.value.isPaid, set: (v) => { replay.value.isPaid = v } })
...
const maxTraffic = computed(() => Math.max(...trafficData.value.map((d) => d.value)))

const interStats = computed(() => [
  { label: '弹幕', value: interaction.value.danmaku, icon: 'message-circle' },
  { label: '点赞', value: interaction.value.likes, icon: 'heart' },
  { label: '评论', value: interaction.value.comments, icon: 'message-circle' },
  { label: '分享', value: interaction.value.shares, icon: 'share-2' },
])
