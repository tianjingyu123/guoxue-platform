...
export const operatorApi = {
  /** 加入运营商页 */
  async joinOperator() {
    if (useMock()) return { pricing: operatorPricing, planComparison, benefits: operatorBenefits, earningCases: operatorEarningCases, faqs: operatorFaqs }
    try {
      const data = await apiGet<any>('/operator/join')
      return { pricing: data.pricing || operatorPricing, planComparison: data.planComparison || planComparison, benefits: data.benefits || operatorBenefits, earningCases: data.earningCases || operatorEarningCases, faqs: data.faqs || operatorFaqs }
    } catch { return { pricing: operatorPricing, planComparison, benefits: operatorBenefits, earningCases: operatorEarningCases, faqs: operatorFaqs } }
  },

  /** 加入站长页 */
  async joinStation() {
    if (useMock()) return { pricing: stationPricing, benefits: stationBenefits, earningCases: stationEarningCases, faqs: stationFaqs }
    try {
      const data = await apiGet<any>('/operator/station/join')
      return { pricing: data.pricing || stationPricing, benefits: data.benefits || stationBenefits, earningCases: data.earningCases || stationEarningCases, faqs: data.faqs || stationFaqs }
    } catch { return { pricing: stationPricing, benefits: stationBenefits, earningCases: stationEarningCases, faqs: stationFaqs } }
  },

  /** 运营商协议 */
  async operatorAgreement() {
    if (useMock()) return { sections: operatorAgreementSections, tip: operatorAgreementTip }
    try {
      const data = await apiGet<any>('/operator/agreement')
      return { sections: data.sections || operatorAgreementSections, tip: data.tip || operatorAgreementTip }
    } catch { return { sections: operatorAgreementSections, tip: operatorAgreementTip } }
  },

  /** 站长协议 */
  async stationAgreement() {
    if (useMock()) return { sections: stationAgreementSections, tip: stationAgreementTip }
    try {
      const data = await apiGet<any>('/operator/station/agreement')
      return { sections: data.sections || stationAgreementSections, tip: data.tip || stationAgreementTip }
    } catch { return { sections: stationAgreementSections, tip: stationAgreementTip } }
  },

  /** 运营商面板 */
  async operatorPanel() {
    if (useMock()) return { info: operatorPanelInfo, overview: operatorOverview, teamRanking: operatorTeamRanking, quotaUsage: operatorQuotaUsage, quickActions: operatorQuickActions }
    try {
      const data = await apiGet<any>('/station/my')
      return { info: data.info || operatorPanelInfo, overview: data.overview || operatorOverview, teamRanking: data.teamRanking || operatorTeamRanking, quotaUsage: data.quotaUsage || operatorQuotaUsage, quickActions: data.quickActions || operatorQuickActions }
    } catch { return { info: operatorPanelInfo, overview: operatorOverview, teamRanking: operatorTeamRanking, quotaUsage: operatorQuotaUsage, quickActions: operatorQuickActions } }
  },

  /** 站长面板 */
  async stationMasterPanel() {
    if (useMock()) return { info: stationPanelInfo, overview: stationPanelOverview, trends: stationPanelTrends, balance: stationPanelBalance, quickActions: stationPanelQuickActions, notices: stationPanelNotices, overviewIconMap: stationOverviewIconMap, actionIconMap: stationActionIconMap }
    try {
      const data = await apiGet<any>('/operator/station/panel')
      return { info: data.info || stationPanelInfo, overview: data.overview || stationPanelOverview, trends: data.trends || stationPanelTrends, balance: data.balance || stationPanelBalance, quickActions: data.quickActions || stationPanelQuickActions, notices: data.notices || stationPanelNotices, overviewIconMap: data.overviewIconMap || stationOverviewIconMap, actionIconMap: data.actionIconMap || stationActionIconMap }
    } catch { return { info: stationPanelInfo, overview: stationPanelOverview, trends: stationPanelTrends, balance: stationPanelBalance, quickActions: stationPanelQuickActions, notices: stationPanelNotices, overviewIconMap: stationOverviewIconMap, actionIconMap: stationActionIconMap } }
  },

  /** 运营商工作台 */
  async dashboard() {
    if (useMock()) return { dashboardData: operatorDashboardData, teamMembers: dashboardTeamMembers, quotaRecords: dashboardQuotaRecords, inviteLink: operatorInviteLink }
    try {
      const data = await apiGet<any>('/station/dashboard/overview')
      return { dashboardData: data.dashboardData || operatorDashboardData, teamMembers: data.teamMembers || dashboardTeamMembers, quotaRecords: data.quotaRecords || dashboardQuotaRecords, inviteLink: data.inviteLink || operatorInviteLink }
    } catch { return { dashboardData: operatorDashboardData, teamMembers: dashboardTeamMembers, quotaRecords: dashboardQuotaRecords, inviteLink: operatorInviteLink } }
  },

  /** 团队管理 */
  async team() {
    if (useMock()) return { overview: teamMgmtOverview, members: teamMgmtMembers, leaderboard: teamLeaderboard, myRank: teamMyRank, activities: teamActivities, successCases: teamSuccessCases, activityIconMap: teamActivityIconMap, inviteLink: teamInviteLink }
    try {
      const data = await apiGet<any>('/station/team/members')
      return { overview: data.overview || teamMgmtOverview, members: data.members || teamMgmtMembers, leaderboard: data.leaderboard || teamLeaderboard, myRank: data.myRank ?? teamMyRank, activities: data.activities || teamActivities, successCases: data.successCases || teamSuccessCases, activityIconMap: data.activityIconMap || teamActivityIconMap, inviteLink: data.inviteLink || teamInviteLink }
    } catch { return { overview: teamMgmtOverview, members: teamMgmtMembers, leaderboard: teamLeaderboard, myRank: teamMyRank, activities: teamActivities, successCases: teamSuccessCases, activityIconMap: teamActivityIconMap, inviteLink: teamInviteLink } }
  },

  /** 团队成员详情 */
  async teamMemberDetail(memberId: string) {
    if (useMock()) return { orders: teamMemberRecentOrders, invitedMembers: teamMemberInvitedMembers }
    try {
      const data = await apiGet<any>(`/operator/team/members/${memberId}`)
      return { orders: data.orders || teamMemberRecentOrders, invitedMembers: data.invitedMembers || teamMemberInvitedMembers }
    } catch { return { orders: teamMemberRecentOrders, invitedMembers: teamMemberInvitedMembers } }
  },

  /** 名额管理 */
  async quota() {
    if (useMock()) return { quotaData, records: quotaRecords, saleLink: quotaSaleLink }
    try {
      const data = await apiGet<any>('/operator/quota')
      return { quotaData: data.quotaData || quotaData, records: data.records || quotaRecords, saleLink: data.saleLink || quotaSaleLink }
    } catch { return { quotaData, records: quotaRecords, saleLink: quotaSaleLink } }
  },

  /** 邀请站长 */
  async invite() {
    if (useMock()) return { stations: invitedStations, inviteLink: operatorInviteLinkFull, inviteCode: operatorInviteCode }
    try {
      const data = await apiGet<any>('/operator/invite')
      return { stations: data.stations || invitedStations, inviteLink: data.inviteLink || operatorInviteLinkFull, inviteCode: data.inviteCode || operatorInviteCode }
    } catch { return { stations: invitedStations, inviteLink: operatorInviteLinkFull, inviteCode: operatorInviteCode } }
  },

  /** 沉寂站长预警 */
  async dormant() {
    if (useMock()) return { members: dormantMembers }
    try {
      const data = await apiGet<any>('/operator/dormant')
      return { members: data.members || dormantMembers }
    } catch { return { members: dormantMembers } }
  },

  /** 下线业绩分析 */
  async analysis() {
    if (useMock()) return { members: analysisMembers }
    try {
      const data = await apiGet<any>('/operator/analysis')
      return { members: data.members || analysisMembers }
    } catch { return { members: analysisMembers } }
  },

  /** 推广素材列表 */
  async materials(stationId: string, type?: string) {
    if (useMock()) return {
      posters: [
        { id: 1, title: '八字命理课程推广海报', useCount: 328, tags: ['八字', '课程', '推广'] },
        { id: 2, title: '风水大师直播预告', useCount: 256, tags: ['风水', '直播', '预告'] },
        { id: 3, title: '国学经典诵读活动', useCount: 189, tags: ['经典', '活动', '诵读'] },
        { id: 4, title: '紫微斗数入门课程', useCount: 167, tags: ['紫微', '课程', '入门'] },
      ],
      copywritings: [
        { id: 1, title: '命理课程推广文案', scene: '朋友圈', content: '知命方能改运。八字命理，一窥天机。\n二十年命理研究精华，等你来学。\n扫码加入，开启自我认知之旅。', copyCount: 456, tags: ['八字', '课程'] },
        { id: 2, title: '风水直播预告文案', scene: '微信群', content: '🏠 家居风水决定运势？\n本周六晚8点，风水大师直播答疑\n教你家宅旺财三招，小白也能学会', copyCount: 321, tags: ['风水', '直播'] },
        { id: 3, title: '紫微斗数推广文案', scene: '朋友圈', content: '你的人生格局，星盘早已注定。\n紫微斗数十四主星，解锁命理密码。\n限时特惠，开启奇妙之旅。', copyCount: 289, tags: ['紫微', '课程'] },
      ],
      qrcodes: [
        { id: 1, title: '分站推广二维码', targetUrl: 'https://recob.app/s/station', scanCount: 1520 },
        { id: 2, title: '课程推广二维码', targetUrl: 'https://recob.app/course/1', scanCount: 890 },
        { id: 3, title: '微信群二维码', targetUrl: 'https://recob.app/group/join', scanCount: 2340 },
      ],
    }
    try {
      const data = await apiGet<any>(`/station/promotion/materials?stationId=${stationId}${type ? `&type=${type}` : ''}`)
      return data
    } catch { return { posters: [], copywritings: [], qrcodes: [] } }
  },

  /** 分站直播列表 */
  async stationLiveRooms(stationId: string) {
    if (useMock()) return [
    ...
    try {
      return await apiGet<any>(`/station/${stationId}/live-rooms`)
    } catch { return [] }
  },

  /** 老师邀约列表 */
  async teacherInvitations() {
    if (useMock()) return {
      invitations: [
      ...
      demands: [
        { id: 1, title: '八字命理高级课程', category: '八字命理', date: '2024-04-25', budget: '3000-5000', applications: 5, status: 'open' },
        { id: 2, title: '风水实地考察', category: '风水堪舆', date: '2024-05-01', budget: '5000-8000', applications: 3, status: 'open' },
      ],
    }
    try {
      return await apiGet<any>('/station/teachers/invitations')
    } catch { return { invitations: [], demands: [] } }
  },

  /** 可推荐内容列表（临时推荐用） */
  async recommendableContent() {
    if (useMock()) return [
    ...
    try {
      return await apiGet<any>('/station/promotion/recommendable-content')
    } catch { return [] }
  },

  /** 分站管理 - 获取配置 */
  async getStationManage() {
    if (useMock()) return {
      basic: { name: '儒布命理文化站', slogan: '传承国学智慧，点亮人生方向', intro: '专注于传统命理文化传播与学习，汇聚百位名师，覆盖八字、紫微、风水等多个领域。', contactEmail: 'admin@station.com', contactPhone: '138-0000-1234' },
      features: { comment: true, share: true, community: true, ai: false, offline: true },
      domain: { custom: '', ssl: false },
      notify: { newMember: true, newOrder: true, newPost: false, systemNotice: true },
      liveStatus: 'not_applied' as string,
    }
    try {
      return await apiGet<any>('/station/my')
    } catch { return null }
  },

  /** 分站管理 - 保存配置 */
  async saveStationManage(data: Record<string, unknown>) {
    if (useMock()) return { ok: true }
    try {
      return await apiPut<any>('/station/my', data)
