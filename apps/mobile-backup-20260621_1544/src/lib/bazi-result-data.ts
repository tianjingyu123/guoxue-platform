...
export const classicsContent: Record<string, { title: string; original: string; translation: string }> = {
  qiong: {
    title: '论丁生午月',
...
export const baziApi = {
  /** 排盘预览（八字，无需登录）POST /paipan/bazi/preview */
  async calculate(params: Record<string, any>) {
    if (useMock()) return _mockBaziData
    try {
      const data = await apiPost<any>('/paipan/bazi/preview', params)
      return data.result || data
    } catch { return _mockBaziData }
  },

  /** 获取古籍参考（暂无后端接口，返回内置数据） */
  async getClassics(_toolId?: string) {
    return { list: classics, content: classicsContent }
  },

  /** 保存排盘记录 POST /paipan/bazi */
  async save(params: Record<string, any>) {
    if (useMock()) return { id: 'mock-id', result: _mockBaziData }
    return await apiPost<any>('/paipan/bazi', params)
  },

  /** 获取单条记录 GET /paipan/bazi/:id */
  async detail(id: string) {
    if (useMock()) return { id, clientName: '测试', clientBirth: '1983-6-18 14:31', inputParams: {}, resultData: _mockBaziData, createdAt: new Date().toISOString() }
    return await apiGet<any>(`/paipan/bazi/${id}`)
  },

  /** 获取排盘历史 GET /paipan/bazi */
  async history() {
    if (useMock()) return { records: [{ id: 'mock-1', clientName: '测试', clientBirth: '1983-6-18 14:31', createdAt: new Date().toISOString() }], total: 1, page: 1, pageSize: 20 }
    try {
      return await apiGet<{ records: any[]; total: number; page: number; pageSize: number }>('/paipan/bazi')
    } catch { return { records: [], total: 0, page: 1, pageSize: 20 } }
  },

  /** AI分析 POST /paipan/bazi/analyze */
  async analyze(recordId: string) {
    if (useMock()) return { status: 'completed', summary: '此八字日主丁火生于午月建禄，身旺以金水为用，宜行西北运。', recordId }
    return await apiPost<any>('/paipan/bazi/analyze', { recordId })
  },

  /** 获取分析结果 GET /paipan/bazi/:id/analysis */
  async getAnalysis(recordId: string) {
    if (useMock()) return { status: 'completed', summary: '此八字日主丁火生于午月建禄，身旺以金水为用，宜行西北运。', recordId }
    return await apiGet<any>(`/paipan/bazi/${recordId}/analysis`)
  },
}
