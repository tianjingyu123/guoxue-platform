...
export const classicsApi = {
  ...
  async category(catId: string) {
    ...
    try { const data = await apiGet<any>(`/classic/books?category=${encodeURIComponent(catId)}`); return { config: data.config || CAT_CONFIG[catId as CatId], books: data.books || CAT_BOOKS[catId as CatId] || [], filterTypes: data.filterTypes || filterTypes } }
  ...
  async search(keyword: string) {
    ...
    try { const data = await apiGet<any>(`/classic/books?q=${encodeURIComponent(keyword)}`); return { results: data.results || searchResultsData, suggestions: data.suggestions || searchSuggestionsData, hotSearches: data.hotSearches || hotSearchData, history: data.history || searchHistoryData } }
  ...
  async detail(id: string) {
    ...
    try { const data = await apiGet<any>(`/classic/books/${id}`); return { book: data.book || bookData[id], aiFeatures: data.aiFeatures || AI_FEATURES, discussions: data.discussions || bookDiscussions } }
  ...
  async bookshelf() {
    ...
    try { const data = await apiGet<any>('/classic/my-progress'); return { shelfBooks: data.shelfBooks || bookshelfData, groups: data.groups || groupsData, readingHistory: data.readingHistory || readingHistoryData } }
  ...
  async bookmarks() {
    if (useMock()) return { items: bookmarksData }
    try { const data = await apiGet<any>('/classic/bookmarks'); return { items: data.items || data || bookmarksData } }
    catch { return { items: bookmarksData } }
  },
  async notes() {
    if (useMock()) return { items: notesData }
    try { const data = await apiGet<any>('/classic/notes'); return { items: data.items || data || notesData } }
    catch { return { items: notesData } }
  },
