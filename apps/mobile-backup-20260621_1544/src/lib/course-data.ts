...
export const courseApi = {
  ...
  async saleSessions() {
    ...
    try {
      const data = await apiGet<any>('/marketing/flash-sales/active')
  ...
  async purchaseInfo(courseId: string) {
    ...
    try {
      const data = await apiPost<any>(`/courses/${courseId}/purchase`, {})
  ...
  async player(lessonId: string) {
    ...
    try {
      const data = await apiGet<any>(`/courses/chapters/${lessonId}/content`)
