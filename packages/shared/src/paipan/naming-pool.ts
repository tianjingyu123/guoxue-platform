 ...
 *
 * ══ 2026-09-21 更新：语义层已补齐（在服务端，不在本包）══
 *
 * 决策人拍板「开源库 + 模型生成都可以」。已为本池 6500 字逐字标注宜/可/忌、人名读音、性别倾向、风格、简明字义，
 * 依据开放汉语字典 hyzd（CC BY 3.0）的词例释义与 Unihan（Unicode License V3）的读音，按统一规格生成，
 * 两遍独立标注三档一致 92.4%、宜⇄忌对冲 0，另做「可→宜」复核。结果放在
 * `apps/server/src/modules/paipan/engine/data/naming-lexicon.json`——**只在服务端**，不进本包，
 * 因为本包会打进前端（防抄，见第 4 步引擎迁后端）。服务端起名引擎已接入（候选只取「宜」约 1046 字）。
 * 生成过程与质检记录：`artifacts/naming-lexicon/README.md`。
 */
