...
const NAYIN = [
  ...
  "井泉水", "屋上土", "霹雳火", "松柏木", "长流水", "沙中金", "山下火", "平地木", "壁上土", "金箔金",
  "覆灯火", "天河水", "大驿土", "钗钏金", "桑柘木", "大溪水", "沙中土", "天上火", "石榴木", "大海水",
]
/**
 * 纳音五行。两干支一组，六十甲子对应三十纳音。
 *
 * ⚠️ 2026-09-20 修正：原实现 **60 个错 40 个**（desktop-44 实测报出，我全枚举复核）。
 *
 * 原写法拿 `(zi - gi) mod 12` 当旬号去查偏移表 `[0,10,20,30,40,50]`。
 * 但 `(zi - gi) mod 12` 得到的是**旬首的地支序号**，而地支序号**逆着**旬序走：
 *
 ...
 * 六旬里只有**甲子旬与甲午旬**（两端与中点）碰巧落对，其余四旬整旬皆错。
 *
 * 这个错法值得记住：**乙巳年得「井泉水」（正确为覆灯火）看上去完全正常**——
 * 是个真纳音名、五行也说得通，除非逐个比对否则永远发现不了。
 *
 * 现改为直接按同余求六十甲子序号（孙子定理闭式）：
 * 求 n 使 n ≡ gi (mod 10) 且 n ≡ zi (mod 12)，解为 n = (6·gi − 5·zi) mod 60。
 * 不再有任何需要手写的偏移表——**能写错的那张表被消掉了**。
 *
 * 全枚举回归见 `test/nayin.spec.ts`：60 甲子逐个对 `@guoxue/bazi-engine` 的
 * 独立手写表，两份实现必须一致。
 */
