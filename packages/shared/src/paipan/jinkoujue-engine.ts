 ...
 *
 * 2026-09-18 从前端 `apps/mobile/src/pkg-paipan/lib/jinkoujue-engine.ts` 迁入 shared。
 * 迁移缘由：后端 `tool-registry/calculators/jinkoujue.calculator.ts` 算错了——
 * 拿前端记录的竞品黄金基准一比，**只有日柱对**，月将差了半年
 * （2025-09-29 秋分后该用辰将，它给谷雨后的酉将），月将是「月将加时」的根，
 * 将神、贵神、人元连带全错。那份实现已删除（见接续文档 §2.47、§2.50）。
 * 这里迁的是**前端那份（经竞品实测校准、已用第三方基准逐项验过）**。
 *
 * 分工与小六壬一致：**shared 只管推算，历法数据由调用方算好传入**——
 * 四柱、中气表、节气区间文案、农历标签都走参数，shared 不为历法再背一个依赖。
 *
 * 排课流程：
 * 1. 四柱（调用方给：年柱立春换年、月柱交节换月）
 ...
 *    纯阴课反取阳（将神）、纯阳课反取阴（贵神）
 *
 * **回归基准**（第三方 H5 实测，改这个文件后必须逐项对上）：
 * 2025-09-29 10:10、地分子、中气换将、贵人甲戊庚牛羊、贵神卯酉区分 →
 * 四柱 乙巳/乙酉/辛丑/癸巳，月将辰，人元戊，贵神戊戌(天空)，将神己亥(登明)，
...
/** 十二天将（贵神）序 + 本家支 */
/** 十二天将（与大六壬那张同名不同用，故不对外导出，避免 shared 出口冲突） */
const TIANJIANG = [
...
export interface JkjSizhu {
  /** 年柱干支，如「乙巳」（立春换年，由调用方算好） */
  year: string
  /** 月柱干支（交节换月） */
  month: string
  day: string
...
export interface JkjOptions {
  /** 起课时间 */
  date: Date
  /** 四柱：shared 不算历法，由调用方传入（前端用自己的 lunar 包、服务端用 lunar-javascript） */
  sizhu: JkjSizhu
  /**
   * 中气表：中气名 → 该中气交节的时间戳（毫秒）。
   * 仅 `jiangMethod === "zhong"` 时需要；缺省时退回子将（与原实现一致）。
   */
  zhongqiTable?: Record<string, number>
  /** 节气区间文案（纯展示，如「秋分2025.09.23 02:19 ~ 寒露2025.10.08 08:40」） */
  jieqiRange?: string
  /** 农历标签（纯展示，如「八月初八」） */
  lunarLabel?: string
...
export function computeJinkoujue(opts: JkjOptions): JkjResult {
  const d = opts.date
  // 四柱由调用方算好传入：shared 不背历法依赖（缘由见文件头）
  const yearGZ = opts.sizhu.year
  const monthGZ = opts.sizhu.month
  const dayGZ = opts.sizhu.day
  const timeGZ = opts.sizhu.hour
  ...
  if (opts.jiangMethod === "jie") {
    ...
    /**
     * 🔴 2026-09-20 加的守卫：候选集不全要报错，不能静默给个陈旧的月将。
     *
     * 起因：调用方拿 `lunar.getJieQiTable()` 建表，而那张表是**按农历年**给的——
     * 查 2025-12-25 时表里的「冬至」装的是 **2024-12-21**（上一个），
     * 当年 2025-12-21 那个冬至根本不在表里，要跨年后再查才出现。
     * 于是每年 **12-22 ~ 12-31** 这十天找不到当年冬至，退回小雪，
     * **月将错一位（给寅，应丑）**，而盘面完整、毫无异常可看。实测 2022–2027 稳定复现。
     *
     * 判据是**结构性**的，不依赖任何外部基准：中气每月一个，相邻间隔最长 31.3 天
     * （实测 2020–2030 的真实最大值）。若「最近一个已过中气」距今超过 32 天，
     * 那一定是候选集缺了条目，而不是天上真有 32 天没交中气。
     *
     * 两个调用方（apps/mobile 适配层、apps/server paipan.service）已改为
     * 合并「当日 + 45 天后」两张年表，正常情况下这里不会触发。
     *
     * ⚠️ **这道守卫是安全网，不是修复。** 阈值 32 天卡的是结构上限，
     * 而紧挨冬至那两三天的陈旧度约 30~32 天、落在合法区间内，结构上无从分辨。
     * 全量扫 2022–2027：病日 60 天，守卫只抓得住 44 天，剩下 16 天
     * （每年冬至后头 2~3 日）唯有合并两张表才能根治。别把网当成修。
     * 回归见 `apps/server/test/jinkoujue-zhongqi.spec.ts`。
     */
    if (best) {
      const staleDays = (d.getTime() - best.time) / 86400000
      if (staleDays > 32) {
        throw new Error(
          `金口诀中气法：传入的中气表不完整——最近一个已过中气是「${best.name}」，` +
          `距起课时刻 ${staleDays.toFixed(1)} 天，而中气相邻间隔最长约 31.3 天。` +
          `多半是只取了单张农历年节气表（冬至条目装的是上一个冬至），` +
          `请合并「当日」与「当日+45天」两张表后再传入。`,
        )
      }
    }
